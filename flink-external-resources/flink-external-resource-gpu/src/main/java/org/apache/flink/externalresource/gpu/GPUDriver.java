...
class GPUDriver implements ExternalResourceDriver {
    ...
    @VisibleForTesting
    static final ConfigOption<String> DISCOVERY_SCRIPT_PATH = key("discovery-script.path").stringType()
            .defaultValue(String.format("%s/external-resource-gpu/nvidia-gpu-discovery.sh", ConfigConstants.DEFAULT_FLINK_PLUGINS_DIRS));

    @VisibleForTesting
    static final ConfigOption<String> DISCOVERY_SCRIPT_ARG = key("discovery-script.args").stringType().noDefaultValue();
    ...
    private String executeDiscoveryScript(File discoveryScript, long gpuAmount, String args) throws Exception {
        ...
        try(final BufferedReader stdoutReader = new BufferedReader(
            ...
            if(stdout.length > 1) {
                LOG.warn(
                        ...
                        stdout.length, Arrays.toString(stdout));
            }
            if(stdout.length == 0) {
                return "";
            }
