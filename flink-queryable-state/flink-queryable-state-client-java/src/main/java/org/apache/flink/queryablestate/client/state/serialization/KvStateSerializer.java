...
public final class KvStateSerializer {
    ...
    public static <K, N> Tuple2<K, N> deserializeKeyAndNamespace(byte[] serializedKeyAndNamespace,
        ...
        try {
            ...
            throw new IOException(
                    "Unable to deserialize key " + "and namespace. This indicates a mismatch in the key/namespace " + "serializers used by the KvState instance and this access.",
                    e);
        }
    }
    ...
    public static <T> T deserializeValue(byte[] serializedValue, TypeSerializer<T> serializer) throws IOException {
        if(serializedValue == null) {
            ...
            if(deser.available() > 0) {
                throw new IOException(
                        "Unconsumed bytes in the deserialized value. " + "This indicates a mismatch in the value serializers " + "used by the KvState instance and this access.");
            }
            return value;
        }
    }
    ...
    public static <T> List<T> deserializeList(byte[] serializedValue, TypeSerializer<T> serializer) throws IOException {
        if(serializedValue != null) {
            ...
            try {
                ...
                throw new IOException(
                        "Unable to deserialize value. " + "This indicates a mismatch in the value serializers " + "used by the KvState instance and this access.",
                        e);
            }
        } else {
            return null;
     ...
     * @throws IOException On failure during serialization
     */
    public static <UK, UV> byte[] serializeMap(Iterable<Map.Entry<UK, UV>> entries, TypeSerializer<UK> keySerializer,
