...
public abstract class AbstractServerBase<REQ extends MessageBody, RESP extends MessageBody> {
     ...
     */
    protected AbstractServerBase(final String serverName, final String bindAddress,
    ...
    private ExecutorService createQueryExecutor() {
        ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true)
                .setNameFormat("Flink " + getServerName() + " Thread %d").build();
    ...
    private boolean attemptToBind(final int port) throws Throwable {
        ...
        final ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true)
                .setNameFormat("Flink " + serverName + " EventLoop Thread %d").build();
         ...
         *  注释：
         */
        this.bootstrap = new ServerBootstrap().localAddress(bindAddress, port).group(nioGroup)
                .channel(NioServerSocketChannel.class).option(ChannelOption.ALLOCATOR, bufferPool)
                .childOption(ChannelOption.ALLOCATOR, bufferPool).childHandler(new ServerChannelInitializer<>(handler));

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 配置水位线，确保不往网络中写入太多数据
         * 	当输出缓冲中的字节数超过高水位值, 则 Channel.isWritable() 会返回 false
         * 	当输出缓存中的字节数低于低水位值, 则 Channel.isWritable() 会重新返回 true
         */
    ...
    public CompletableFuture<Void> shutdownServer() {
        ...
        if(serverShutdownFuture.compareAndSet(null, shutdownFuture)) {
            ...
            CompletableFuture.allOf(queryExecShutdownFuture, groupShutdownFuture, handlerShutdownFuture)
