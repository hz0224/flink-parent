...
public class GroupAggFunction extends KeyedProcessFunction<RowData, RowData, RowData> {
     ...
     */
    public GroupAggFunction(GeneratedAggsHandleFunction genAggsHandler, GeneratedRecordEqualiser genRecordEqualiser,
            LogicalType[] accTypes, int indexOfCountStar, boolean generateUpdateBefore, long stateRetentionTime) {
    ...
    public void processElement(RowData input, Context ctx, Collector<RowData> out) throws Exception {
        ...
        // TODO_MA 注释： data 的 key
        RowData currentKey = ctx.getCurrentKey();

        // TODO_MA 注释： 获取 Accumulator， 判断是不是第一次处理，如果是，则需要初始化 Accumulator
        boolean firstRow;
        ...
        // TODO_MA 注释： 如果 counter 计数是 0
        else {
            // we retracted the last record for this key sent out a delete message
            if(!firstRow) {
