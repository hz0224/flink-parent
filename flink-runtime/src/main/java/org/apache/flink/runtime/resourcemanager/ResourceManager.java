...
public abstract class ResourceManager<WorkerType extends ResourceIDRetrievable> extends FencedRpcEndpoint<ResourceManagerId> implements ResourceManagerGateway, LeaderContender {
     ...
     *  注释： ResourceManager 借助 SlotManager 来管理 slot。
     *  SlotManager 维护了所有已经注册的 TaskExecutor 的所有 slot 的状态，它们的分配情况。
     *  SlotManager 还维护了所有处于等待状态的 slot 请求。
     *  每当有一个新的 slot 注册或者一个已经分配的 slot 被释放的时候，SlotManager 会试图去满足处于等待状态 slot request。
     *  如果是在 YARN 模式下运行，则 SlotManager 还会向 YARN 去申请更多的 container 来启动 TaskManager 获取更多的 slot
    ...
    public ResourceManager(RpcService rpcService, ResourceID resourceId, HighAvailabilityServices highAvailabilityServices, HeartbeatServices heartbeatServices,
            SlotManager slotManager, ResourceManagerPartitionTrackerFactory clusterPartitionTrackerFactory, JobLeaderIdService jobLeaderIdService,
            ClusterInformation clusterInformation, FatalErrorHandler fatalErrorHandler, ResourceManagerMetricGroup resourceManagerMetricGroup, Time rpcTimeout,
            Executor ioExecutor) {
        ...
        this.clusterPartitionTracker = checkNotNull(clusterPartitionTrackerFactory)
                .get((taskExecutorResourceId, dataSetIds) -> taskExecutors.get(taskExecutorResourceId).getTaskExecutorGateway()
    ...
    private void startResourceManagerServices() throws Exception {
        try {
            ...
            // TODO_MA 注释： DefaultLeaderElectionService
            leaderElectionService = highAvailabilityServices.getResourceManagerLeaderElectionService();

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 初始化，在 Standalone 模式下并没有做什么
             */
            initialize();

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建 ResourceManager 的时候，创建了两个基础服务： SlotManager  JobLeaderIDService
             */
    ...
    public CompletableFuture<RegistrationResponse> registerJobManager(final JobMasterId jobMasterId, final ResourceID jobManagerResourceId,
        ...
        // TODO_MA 注释： 注册到 jobLeaderIdService
        ...
        try {
            ...
            ResourceManagerException exception = new ResourceManagerException("Cannot obtain the " + "job leader id future to verify the correct job leader.",
                    e);
            onFatalError(exception);
        ...
        // TODO_MA 注释： 获取 JobMasterGateway
        ...
        CompletableFuture<RegistrationResponse> registrationResponseFuture = jobMasterGatewayFuture
                .thenCombineAsync(jobMasterIdFuture, (JobMasterGateway jobMasterGateway, JobMasterId leadingJobMasterId) -> {
                    if(Objects.equals(leadingJobMasterId, jobMasterId)) {
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 调用 jobMaster 的 注册 rpc 服务的内部实现
                         */
                        return registerJobMasterInternal(jobMasterGateway, jobId, jobManagerAddress, jobManagerResourceId);
                    } else {
                        final String declineMessage = String
                                .format("The leading JobMaster id %s did not match the received JobMaster id %s. " + "This indicates that a JobMaster leader change has happened.",
        ...
        // TODO_MA 注释： 如果注册有异常，则返回 RegistrationResponse.Decline 消息
    ...
    public CompletableFuture<RegistrationResponse> registerTaskExecutor(final TaskExecutorRegistration taskExecutorRegistration, final Time timeout) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 获取 TaskExecutorGateway，方便发送回去 注册响应
         */
    ...
    @Override
    public CompletableFuture<Acknowledge> sendSlotReport(ResourceID taskManagerResourceId, InstanceID taskManagerRegistrationId, SlotReport slotReport,
            Time timeout) {

        // TODO_MA 注释： 获取到 TaskExecutor 的注册对象： WorkerRegistration
        ...
        if(workerTypeWorkerRegistration.getInstanceID().equals(taskManagerRegistrationId)) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 注册一个 TaskManager 之上的所有的 Slot
             */
            ...
            return FutureUtils
                    .completedExceptionally(new ResourceManagerException(String.format("Unknown TaskManager registration id %s.", taskManagerRegistrationId)));
        }
    }
    ...
    public void heartbeatFromTaskManager(final ResourceID resourceID, final TaskExecutorHeartbeatPayload heartbeatPayload) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： ResourceManager 接收到 TaskExecutor 的心跳恢复
         *  心跳信息是： TaskExecutorHeartbeatPayload
         */
    ...
    public void heartbeatFromJobManager(final ResourceID resourceID) {
        ...
        // TODO_MA 注释： resourcemanager 执行心跳反馈处理
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： ResourceManager 接收到一个 申请 Slot 的 RPC 请求
     *  两件事
     *  1、要确保当前来申请 slot 的 jobamanager 必须已经注册过了
     *  2、调用 slotManager.registerSlotRequest(slotRequest); 执行请求的处理
     */
    @Override
    public CompletableFuture<Acknowledge> requestSlot(JobMasterId jobMasterId, SlotRequest slotRequest, final Time timeout) {

        // TODO_MA 注释： 从 SlotRequest 找到 JobID, 通过 jobID 找到 ResourceManager 中的 JobMaster 的注册对象
        JobID jobId = slotRequest.getJobId();
        JobManagerRegistration jobManagerRegistration = jobManagerRegistrations.get(jobId);

        // TODO_MA 注释： JobMaster 必须已经注册
        if(null != jobManagerRegistration) {
            if(Objects.equals(jobMasterId, jobManagerRegistration.getJobMasterId())) {
                ...
                try {
                     ...
                     *  在 ResourceManager 内部，是由 SlotManager 完成具体的 slot 申请工作
                     */
    ...
    public void notifySlotAvailable(final InstanceID instanceID, final SlotID slotId, final AllocationID allocationId) {
        ...
        if(registration != null) {
            ...
            log.debug("Could not find registration for resource id {}. Discarding the slot available" + "message {}.", resourceId.getStringWithMetadata(),
                    slotId);
        }
    }
    ...
    public CompletableFuture<Collection<TaskManagerInfo>> requestTaskManagerInfo(Time timeout) {
        ...
        for(Map.Entry<ResourceID, WorkerRegistration<WorkerType>> taskExecutorEntry : taskExecutors.entrySet()) {
            ...
            taskManagerInfos.add(new TaskManagerInfo(resourceId, taskExecutor.getTaskExecutorGateway().getAddress(), taskExecutor.getDataPort(),
                    ...
                    slotManager.getNumberRegisteredSlotsOf(taskExecutor.getInstanceID()), slotManager.getNumberFreeSlotsOf(taskExecutor.getInstanceID()),
                    slotManager.getRegisteredResourceOf(taskExecutor.getInstanceID()), slotManager.getFreeResourceOf(taskExecutor.getInstanceID()),
    ...
    public CompletableFuture<TaskManagerInfo> requestTaskManagerInfo(ResourceID resourceId, Time timeout) {
        ...
        if(taskExecutor == null) {
            ...
            final TaskManagerInfo taskManagerInfo = new TaskManagerInfo(resourceId, taskExecutor.getTaskExecutorGateway().getAddress(),
                    taskExecutor.getDataPort(), taskExecutor.getJmxPort(), taskManagerHeartbeatManager.getLastHeartbeatFrom(resourceId),
    ...
    public CompletableFuture<ResourceOverview> requestResourceOverview(Time timeout) {
        ...
        return CompletableFuture.completedFuture(new ResourceOverview(taskExecutors.size(), numberSlots, numberFreeSlots, totalResource, freeResource));
    }
    ...
    private RegistrationResponse registerJobMasterInternal(final JobMasterGateway jobMasterGateway, JobID jobId, String jobManagerAddress,
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 管理心跳
         *  当前过来注册 的 JobMaster 完成跟 ResourceManager的注册之后，
         *  会被 ResourceManager 当做一个 HeartbeatTarget 给管理起来
         *  所以下一次 那个发送心跳请求的for的目标对象就多了一个
         */
    ...
    private RegistrationResponse registerTaskExecutorInternal(TaskExecutorGateway taskExecutorGateway, TaskExecutorRegistration taskExecutorRegistration) {
        ...
        // TODO_MA 注释： TaskExecutor 的 ResourceID
        ResourceID taskExecutorResourceId = taskExecutorRegistration.getResourceId();

        // TODO_MA 注释： 获取 TaskExecutor 的注册对象，如果存在，则证明注册过，需要替换
        WorkerRegistration<WorkerType> oldRegistration = taskExecutors.remove(taskExecutorResourceId);

        // TODO_MA 注释： 存储 old 注册对想，则先注销，再注册
        if(oldRegistration != null) {
            ...
            slotManager.unregisterTaskManager(oldRegistration.getInstanceID(), new ResourceManagerException(
                    String.format("TaskExecutor %s re-connected to the ResourceManager.", taskExecutorResourceId.getStringWithMetadata())));
        }
        ...
        if(newWorker == null) {
             ...
             *  注释： 生成注册对象
             */
            WorkerRegistration<WorkerType> registration = new WorkerRegistration<>(taskExecutorGateway, newWorker, taskExecutorRegistration.getDataPort(),
                    taskExecutorRegistration.getJmxPort(), taskExecutorRegistration.getHardwareDescription(),
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 将 TaskExecutor 视为一个 HeartbeatTarget， 加入心跳目标对象集合
             */
            taskManagerHeartbeatManager.monitorTarget(taskExecutorResourceId, new HeartbeatTarget<Void>() {
                ...
                public void requestHeartbeat(ResourceID resourceID, Void payload) {
                    ...
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： ResourceManager 发送心跳 RPC 请求给 TaskExecutor
                     */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 返回注册成功消息 TaskExecutorRegistrationSuccess
             */
    ...
    protected void closeTaskManagerConnection(final ResourceID resourceID, final Exception cause) {
        ...
        if(workerRegistration != null) {
            ...
            // TODO_MA 注释： 告诉 TaskExecutor, 断开链接
            workerRegistration.getTaskExecutorGateway().disconnectResourceManager(cause);
        } else {
            log.debug("No open TaskExecutor connection {}. Ignoring close TaskExecutor connection. Closing reason was: {}", resourceID.getStringWithMetadata(),
                    cause.getMessage());
        }
    }
    ...
    public void grantLeadership(final UUID newLeaderSessionID) {
        final CompletableFuture<Boolean> acceptLeadershipFuture = clearStateFuture.thenComposeAsync(
    ...
    private CompletableFuture<Boolean> tryAcceptLeadership(final UUID newLeaderSessionID) {
        ...
        if(leaderElectionService.hasLeadership(newLeaderSessionID)) {
            ...
            // TODO_MA 注释： 生成一个 ResourceManagerId
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 成为 Leader 的 ResourceManager 才能执行这个方法： 启动服务
             *  1、启动心跳服务
             *  	启动两个心跳定时服务
             *  2、启动 SlotManager 服务
             *  	启动两个定时任务
    ...
    private void startServicesOnLeadership() {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动两个定时服务
         *  SlotManager 是存在于 ResourceManager 中用来管理所有的 TaskManager 汇报和注册的 Slot 的工作的
         *  1、检查 TaskExecutor 的死活状态的
         *  2、检查 Slot 申请的请求的状态的
         */
    ...
    private void startHeartbeatServices() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： TaskExecutor（TaskManager） 和 ResourceManager（JobManager） 维持心跳
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： JobManager（JobMaster） 和 ResourceManager（JobManager） 维持心跳
    ...
    private class ResourceActionsImpl implements ResourceActions {
        ...
        public void notifyAllocationFailure(JobID jobId, AllocationID allocationId, Exception cause) {
            ...
            if(jobManagerRegistration != null) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 通知 JobMaster ，针对该 allocationId 的 slot 申请失败
                 */
    ...
    private class TaskManagerHeartbeatListener implements HeartbeatListener<TaskExecutorHeartbeatPayload, Void> {
        ...
        public void notifyHeartbeatTimeout(final ResourceID resourceID) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 如果超时，则关闭和 TaskExecutor 之间的链接
             */
        ...
        public void reportPayload(final ResourceID resourceID, final TaskExecutorHeartbeatPayload payload) {
            ...
            // TODO_MA 注释： 获取 TaskExecutor 的注册信息
            ...
            if(workerRegistration == null) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 进行 TaskExecutor 之上的 Slot 状态汇报
                 */
