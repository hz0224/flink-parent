...
public class SlotReport implements Serializable, Iterable<SlotStatus> {
    ...
    public String toString() {
        final String lineSeparator = slotsStatus.size() == 1 || slotsStatus.size() > 10 ? "" : System.lineSeparator() + "\t";

        return slotsStatus.stream().map(SlotStatus::toString).collect(Collectors.joining(lineSeparator, "SlotReport{" + lineSeparator, "}"));
    }
}
