...
public class SourceCoordinator<SplitT extends SourceSplit, EnumChkT> implements OperatorCoordinator {
    ...
    public SourceCoordinator(String operatorName, ExecutorService coordinatorExecutor, Source<?, SplitT, EnumChkT> source,
    ...
    public void subtaskReset(int subtaskId, long checkpointId) {
        runInEventLoop(() -> {
            LOG.info("Recovering subtask {} to checkpoint {} for source {} to checkpoint.", subtaskId, checkpointId, operatorName);
    ...
    public void checkpointCoordinator(long checkpointId, CompletableFuture<byte[]> result) {
        runInEventLoop(() -> {
            ...
            try {
                ...
                result.completeExceptionally(new CompletionException(String.format("Failed to checkpoint SplitEnumerator for source %s", operatorName), e));
            }
    ...
    private void runInEventLoop(final ThrowingRunnable<Throwable> action, final String actionName, final Object... actionNameFormatParameters) {
        ...
        coordinatorExecutor.execute(() -> {
            try {
                ...
                LOG.error("Uncaught exception in the SplitEnumerator for Source {} while {}. Triggering job failover.", operatorName, actionString, t);
                context.failJob(t);
            }
        });
    }
    ...
    private byte[] toBytes(long checkpointId) throws Exception {
        return writeCheckpointBytes(checkpointId, enumerator.snapshotState(), context, enumCheckpointSerializer, splitSerializer);
    }

    static <SplitT extends SourceSplit, EnumChkT> byte[] writeCheckpointBytes(final long checkpointId, final EnumChkT enumeratorCheckpoint,
            final SourceCoordinatorContext<SplitT> coordinatorContext, final SimpleVersionedSerializer<EnumChkT> checkpointSerializer,
            final SimpleVersionedSerializer<SplitT> splitSerializer) throws Exception {

        try(ByteArrayOutputStream baos = new ByteArrayOutputStream(); DataOutputStream out = new DataOutputViewStreamWrapper(baos)) {
    ...
    private EnumChkT deserializeCheckpointAndRestoreContext(byte[] bytes) throws Exception {
        try(ByteArrayInputStream bais = new ByteArrayInputStream(bytes); DataInputStream in = new DataInputViewStreamWrapper(bais)) {
