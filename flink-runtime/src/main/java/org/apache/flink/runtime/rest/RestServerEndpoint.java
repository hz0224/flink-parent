...
public abstract class RestServerEndpoint implements AutoCloseableAsync {
    ...
    public final void start() throws Exception {
        synchronized(lock) {
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 注册 Handler
             *  每个 rest 请求，对应了一个 Handler
             *  拼装 URL 映射到 Handler ==> key：url，  value：handler
             */
            ...
            ChannelInitializer<SocketChannel> initializer = new ChannelInitializer<SocketChannel>() {
                ...
                protected void initChannel(SocketChannel ch) {
                    ...
                    ch.pipeline().addLast(new HttpServerCodec()).addLast(new FileUploadHandler(uploadDir))
                            .addLast(new FlinkHttpObjectAggregator(maxContentLength, responseHeaders))
                            .addLast(new ChunkedWriteHandler()).addLast(handler.getName(), handler)
            ...
            // TODO_MA 注释： 注意，端口是一个随机端口，不是一个固定端口
            ...
            // TODO_MA 注释： RUNNING ，到此时，我们的 RestServerEndpoint 已经启动好了
            state = State.RUNNING;
             ...
             *  注释： 启动
             *  内部两件事：
             *  1、开启选举，其实就是把 WebMonitorEndpoint 自己的地址信息，写到 ZK 中
             *  2、开启定时任务
             */
            startInternal();
        }
        ...
        synchronized(lock) {
            ...
            if(state == State.RUNNING) {
                final CompletableFuture<Void> shutDownFuture = FutureUtils
                        .composeAfterwards(closeHandlersAsync(), this::shutDownInternal);
    ...
    private FutureUtils.ConjunctFuture<Void> closeHandlersAsync() {
        return FutureUtils.waitForAll(
                handlers.stream().map(tuple -> tuple.f1).filter(handler -> handler instanceof AutoCloseableAsync)
    ...
    protected CompletableFuture<Void> shutDownInternal() {
        ...
        synchronized(lock) {
            ...
            channelFuture.thenRun(() -> {
                ...
                CompletableFuture<Void> combinedFuture = FutureUtils
                        .completeAll(Arrays.asList(groupFuture, childGroupFuture));
    ...
    private static void registerHandler(Router router,
            Tuple2<RestHandlerSpecification, ChannelInboundHandler> specificationHandler, Logger log) {
        ...
        for(final RestAPIVersion supportedVersion : specificationHandler.f0.getSupportedAPIVersions()) {
            ...
            log.debug("Register handler {} under {}@{}.", specificationHandler.f1,
                    specificationHandler.f0.getHttpMethod(), versionedHandlerURL);
            ...
            if(supportedVersion.isDefaultVersion()) {
                ...
                log.debug("Register handler {} under {}@{}.", specificationHandler.f1,
                        specificationHandler.f0.getHttpMethod(), handlerURL);
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 对应不同类的 Http 请求方式，来注册不同类型的 Handler
     */
    private static void registerHandler(Router router, String handlerURL, HttpMethodWrapper httpMethod,
    ...
    static void createUploadDir(final Path uploadDir, final Logger log,
        ...
        if(!Files.exists(uploadDir)) {
            if(initialCreation) {
                ...
                log.warn(
                        "Upload directory {} has been deleted externally. " + "Previously uploaded files are no longer available.",
                        uploadDir);
            }
    ...
    private static void checkAllEndpointsAndHandlersAreUnique(
        ...
        for(Tuple2<RestHandlerSpecification, ChannelInboundHandler> handler : handlers) {
            ...
            for(RestAPIVersion supportedAPIVersion : headers.getSupportedAPIVersions()) {
                final String parameterizedEndpoint = supportedAPIVersion.toString() + headers.getHttpMethod() + headers
    ...
    public static final class RestHandlerUrlComparator implements Comparator<Tuple2<RestHandlerSpecification, ChannelInboundHandler>>, Serializable {
        ...
        public int compare(Tuple2<RestHandlerSpecification, ChannelInboundHandler> o1,
            ...
            final int urlComparisonResult = CASE_INSENSITIVE_ORDER
                    .compare(o1.f0.getTargetRestEndpointURL(), o2.f0.getTargetRestEndpointURL());
