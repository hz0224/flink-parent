...
public class BatchTask<S extends Function, OT> extends AbstractInvokable implements TaskContext<S, OT> {
            ...
            try {
                ...
                // set the iterative status for inputs and broadcast inputs
                {
                    ...
                    for(int i = 0; i < numInputs; i++) {
                        final int numberOfEventsUntilInterrupt = getTaskConfig()
                                .getNumberOfEventsUntilInterruptInIterativeGate(i);
                ...
                throw new RuntimeException(
                        "Initializing the input processing failed" + (e.getMessage() == null ? "." : ": " + e
                                .getMessage()), e);
            }
    ...
    protected void initialize() throws Exception {
        ...
        try {
            ...
            throw new Exception("The driver setup for '" + this.getEnvironment().getTaskInfo()
                    .getTaskName() + "' , caused an error: " + t.getMessage(), t);
        }
        ...
        try {
            ...
            throw new RuntimeException("Initializing the UDF" + (e.getMessage() == null ? "." : ": " + e.getMessage()),
                    e);
        }
    }
    ...
    protected <X> void readAndSetBroadcastInput(int inputNum, String bcVarName, DistributedRuntimeUDFContext context,
        ...
        if(LOG.isDebugEnabled()) {
            LOG.debug(formatLogString(
                    "Setting broadcast variable '" + bcVarName + "'" + (superstep > 1 ? ", superstep " + superstep : "")));
        }
    ...
    protected void releaseBroadcastVariables(String bcVarName, int superstep, DistributedRuntimeUDFContext context) {
        if(LOG.isDebugEnabled()) {
            LOG.debug(formatLogString(
                    "Releasing broadcast variable '" + bcVarName + "'" + (superstep > 1 ? ", superstep " + superstep : "")));
        }
    ...
    protected void run() throws Exception {
        ...
        try {
            ...
            try {
                ...
                throw new Exception("The data preparation for task '" + this.getEnvironment().getTaskInfo()
                        .getTaskName() + "' , caused an error: " + t.getMessage(), t);
            }
    ...
    protected S initStub(Class<? super S> stubSuperClass) throws Exception {
        try {
            ...
            if(stubSuperClass != null && !stubSuperClass.isAssignableFrom(stub.getClass())) {
                throw new RuntimeException(
                        "The class '" + stub.getClass().getName() + "' is not a subclass of '" + stubSuperClass
    ...
    protected void initInputReaders() throws Exception {
        ...
        for(int i = 0; i < numInputs; i++) {
            ...
            if(groupSize == 1) {
                // non-union case
                inputReaders[i] = new MutableRecordReader<>(getEnvironment().getInputGate(currentReaderOffset),
                ...
                inputReaders[i] = new MutableRecordReader<>(new UnionInputGate(readers),
                        getEnvironment().getTaskManagerInfo().getTmpDirectories());
            } else {
    ...
    protected void initBroadcastInputReaders() throws Exception {
        ...
        for(int i = 0; i < this.config.getNumBroadcastInputs(); i++) {
            ...
            if(groupSize == 1) {
                // non-union case
                broadcastInputReaders[i] = new MutableRecordReader<>(getEnvironment().getInputGate(currentReaderOffset),
                        getEnvironment().getTaskManagerInfo().getTmpDirectories());
            } else if(groupSize > 1) {
                // union case
                IndexedInputGate[] readers = new IndexedInputGate[groupSize];
                ...
                broadcastInputReaders[i] = new MutableRecordReader<>(new UnionInputGate(readers),
                        getEnvironment().getTaskManagerInfo().getTmpDirectories());
            } else {
                throw new Exception("Illegal input group size in task configuration: " + groupSize);
            }
    ...
    protected void initInputsSerializersAndComparators(int numInputs, int numComparators) {
        ...
        for(int i = 0; i < numComparators; i++) {
            ...
            if(this.inputComparators != null) {
                final TypeComparatorFactory<?> comparatorFactory = this.config
                        .getDriverComparator(i, userCodeClassLoader);
    ...
    protected void initBroadcastInputsSerializers(int numBroadcastInputs) {
        ...
        for(int i = 0; i < numBroadcastInputs; i++) {
            ...
            final TypeSerializerFactory<?> serializerFactory = this.config
                    .getBroadcastInputSerializer(i, userCodeClassLoader);
        ...
        for(int i = 0; i < numInputs; i++) {
            ...
            if(async) {
                @SuppressWarnings({"unchecked", "rawtypes"}) TempBarrier<?> barrier = new TempBarrier(this, getInput(i),
                        this.inputSerializers[i], memMan, ioMan, memoryPages, emptyList());
                ...
                @SuppressWarnings({"unchecked", "rawtypes"}) SpillingResettableMutableObjectIterator<?> iter = new SpillingResettableMutableObjectIterator(
                        getInput(i), this.inputSerializers[i].getSerializer(), getMemoryManager(), getIOManager(),
                        memoryPages, this);
        ...
        for(int i = 0; i < this.inputs.length; i++) {
            if(this.excludeFromReset[i]) {
                ...
                if(this.inputIsCached[i]) {
                    ...
                    List<MemorySegment> allocated = tempBarriers[i] == null ? emptyList() : tempBarriers[i]
                            .closeAndGetLeftoverMemory();
                    tempBarriers[i] = null;
                    ...
                    try {
                        ...
                        if(this.inputIsAsyncMaterialized[i]) {
                            ...
                            Preconditions
                                    .checkState(allocated.size() <= pages); // pages shouldn't change, but some segments
                            // might have been consumed
                            @SuppressWarnings({"unchecked", "rawtypes"}) TempBarrier<?> barrier = new TempBarrier(this,
                                    getInput(i), this.inputSerializers[i], memMan, ioMan, pages, allocated);
                            barrier.startReading();
                            this.tempBarriers[i] = barrier;
                            this.inputs[i] = null;
                        } else {
        ...
        if(localStrategy != null) {
            switch(localStrategy) {
                case NONE:
                    ...
                    break;
                case SORT:
                    @SuppressWarnings({"rawtypes", "unchecked"}) Sorter<?> sorter = ExternalSorter
                            .newBuilder(getMemoryManager(), this, this.inputSerializers[inputNum].getSerializer(),
                ...
                case COMBININGSORT:
                    ...
                    try {
                        ...
                        throw new RuntimeException("Initializing the user code and the configuration failed" + (e
                    ...
                    @SuppressWarnings({"rawtypes", "unchecked"}) Sorter<?> cSorter = ExternalSorter
                            .newBuilder(getMemoryManager(), this, this.inputSerializers[inputNum].getSerializer(),
                                    getLocalStrategyComparator(inputNum))
                            .maxNumFileHandles(this.config.getFilehandlesInput(inputNum))
    ...
    protected MutableObjectIterator<?> createInputIterator(MutableReader<?> inputReader,
    ...
    protected void initOutputs() throws Exception {
        ...
        this.output = initOutputs(this, getEnvironment().getUserCodeClassLoader(), this.config, this.chainedTasks,
                this.eventualOutputs, this.getExecutionConfig(), this.accumulatorMap);
    }

    public DistributedRuntimeUDFContext createRuntimeContext(MetricGroup metrics) {
        Environment env = getEnvironment();

        return new DistributedRuntimeUDFContext(env.getTaskInfo(), env.getUserCodeClassLoader(), getExecutionConfig(),
                env.getDistributedCacheEntries(), this.accumulatorMap, metrics, env.getExternalResourceInfoProvider());
    }
        ...
        if(this.inputs[index] != null) {
            ...
            try {
                if(this.tempBarriers[index] != null) {
                    @SuppressWarnings("unchecked") MutableObjectIterator<X> iter = (MutableObjectIterator<X>) this.tempBarriers[index]
                            .getIterator();
                    in = iter;
    ...
    public static String constructLogString(String message, String taskName, AbstractInvokable parent) {
        return message + ":  " + taskName + " (" + (parent.getEnvironment().getTaskInfo()
                .getIndexOfThisSubtask() + 1) + '/' + parent.getEnvironment().getTaskInfo()
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> Collector<T> initOutputs(AbstractInvokable containingTask, UserCodeClassLoader cl,
            TaskConfig config, List<ChainedDriver<?, ?>> chainedTasksTarget, List<RecordWriter<?>> eventualOutputs,
            ExecutionConfig executionConfig, Map<String, Accumulator<?, ?>> accumulatorMap) throws Exception {
        ...
        if(numChained > 0) {
            ...
            for(int i = numChained - 1; i >= 0; --i) {
                ...
                if(i == numChained - 1) {
                    ...
                    previous = getOutputCollector(containingTask, chainedStubConf, cl.asClassLoader(), eventualOutputs, 0,
    ...
    public static void openUserCode(Function stub, Configuration parameters) throws Exception {
        try {
            ...
            throw new Exception("The user defined 'open(Configuration)' method in " + stub.getClass()
                    .toString() + " caused an exception: " + t.getMessage(), t);
        }
    }
    ...
    public static <T> T instantiateUserCode(TaskConfig config, ClassLoader cl, Class<? super T> superClass) {
        try {
            ...
            if(superClass != null && !superClass.isAssignableFrom(stub.getClass())) {
                throw new RuntimeException(
                        "The class '" + stub.getClass().getName() + "' is not a subclass of '" + superClass
                                .getName() + "' as is required.");
            }
            return stub;
        } catch(ClassCastException ccex) {
