...
class ChannelStateCheckpointWriter {
    ...
    ChannelStateCheckpointWriter(int subtaskIndex, CheckpointStartRequest startCheckpointItem,
            CheckpointStreamFactory streamFactory, ChannelStateSerializer serializer,
            RunnableWithException onComplete) throws Exception {
        this(subtaskIndex, startCheckpointItem.getCheckpointId(), startCheckpointItem.getTargetResult(),
                streamFactory.createCheckpointStateOutputStream(EXCLUSIVE), serializer, onComplete);
    }

    @VisibleForTesting
    ChannelStateCheckpointWriter(int subtaskIndex, long checkpointId, ChannelStateWriteResult result,
            CheckpointStateOutputStream stream, ChannelStateSerializer serializer,
            RunnableWithException onComplete) throws Exception {
    ...
    @VisibleForTesting
    ChannelStateCheckpointWriter(int subtaskIndex, long checkpointId, ChannelStateWriteResult result,
            ChannelStateSerializer serializer, RunnableWithException onComplete,
            CheckpointStateOutputStream checkpointStateOutputStream, DataOutputStream dataStream) throws Exception {
    ...
    private <K> void write(Map<K, StateContentMetaInfo> offsets, K key, Buffer buffer,
    ...
    private void finishWriteAndResult() throws IOException {
        ...
        complete(underlying, result.resultSubpartitionStateHandles, resultSubpartitionOffsets,
                HandleFactory.RESULT_SUBPARTITION);
    }

    private void doComplete(boolean precondition, RunnableWithException complete,
    ...
    private <I, H extends AbstractChannelStateHandle<I>> void complete(StreamStateHandle underlying,
            CompletableFuture<Collection<H>> future, Map<I, StateContentMetaInfo> offsets,
    ...
    private <I, H extends AbstractChannelStateHandle<I>> H createHandle(HandleFactory<I, H> handleFactory,
            StreamStateHandle underlying, I channelInfo, StateContentMetaInfo contentMetaInfo) throws IOException {
        ...
        if(bytes.isPresent()) {
            ...
            return handleFactory.create(subtaskIndex, channelInfo, extracted, singletonList(serializer.getHeaderLength()),
                    extracted.getStateSize());
        } else {
            return handleFactory.create(subtaskIndex, channelInfo, underlying, contentMetaInfo.getOffsets(),
