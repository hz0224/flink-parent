...
public class MiniDispatcher extends Dispatcher {
    ...
    public MiniDispatcher(RpcService rpcService, DispatcherId fencingToken, DispatcherServices dispatcherServices,
            JobGraph jobGraph, DispatcherBootstrapFactory dispatcherBootstrapFactory,
    ...
    public CompletableFuture<Acknowledge> submitJob(JobGraph jobGraph, Time timeout) {
        ...
        acknowledgeCompletableFuture.whenComplete((Acknowledge ignored, Throwable throwable) -> {
            if(throwable != null) {
                onFatalError(
                        new FlinkException("Failed to submit job " + jobGraph.getJobID() + " in job mode.", throwable));
            }
        });
    ...
    public CompletableFuture<JobResult> requestJobResult(JobID jobId, Time timeout) {
        ...
        if(executionMode == ClusterEntrypoint.ExecutionMode.NORMAL) {
            ...
            jobResultFuture.thenAccept((JobResult result) -> {
                ApplicationStatus status = result.getSerializedThrowable()
    ...
    protected CleanupJobState jobReachedGloballyTerminalState(ArchivedExecutionGraph archivedExecutionGraph) {
        ...
        if(jobCancelled || executionMode == ClusterEntrypoint.ExecutionMode.DETACHED) {
            ...
            log.info("Shutting down cluster with state {}, jobCancelled: {}, executionMode: {}",
                    archivedExecutionGraph.getState(), jobCancelled, executionMode);
