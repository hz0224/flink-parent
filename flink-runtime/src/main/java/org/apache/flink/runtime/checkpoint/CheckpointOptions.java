...
public class CheckpointOptions implements Serializable {
    ...
    public static CheckpointOptions create(CheckpointType checkpointType,
            CheckpointStorageLocationReference locationReference, boolean isExactlyOnceMode,
            boolean unalignedCheckpointsEnabled, long alignmentTimeout) {

        boolean canBeUnaligned = checkpointType == CheckpointType.CHECKPOINT && unalignedCheckpointsEnabled;

        return new CheckpointOptions(checkpointType, locationReference, isExactlyOnceMode,
                canBeUnaligned && alignmentTimeout == 0, canBeUnaligned ? alignmentTimeout : NO_ALIGNMENT_TIME_OUT);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(targetLocation, checkpointType, isExactlyOnceMode, isUnalignedCheckpoint, alignmentTimeout);
    }
    ...
    public boolean equals(Object obj) {
        if(this == obj) {
            ...
            return this.checkpointType == that.checkpointType && this.targetLocation
                    .equals(that.targetLocation) && this.isExactlyOnceMode == that.isExactlyOnceMode && this.isUnalignedCheckpoint == that.isUnalignedCheckpoint && this.alignmentTimeout == that.alignmentTimeout;
        } else {
            return false;
        }
    }
    ...
    public String toString() {
        return "CheckpointOptions {" + "checkpointType = " + checkpointType + ", targetLocation = " + targetLocation + ", isExactlyOnceMode = " + isExactlyOnceMode + ", isUnalignedCheckpoint = " + isUnalignedCheckpoint + ", alignmentTimeout = " + alignmentTimeout + "}";
    }
    ...
    private static final CheckpointOptions CHECKPOINT_AT_DEFAULT_LOCATION = new CheckpointOptions(
    ...
    public static CheckpointOptions forCheckpointWithDefaultLocation(boolean isExactlyOnceMode,
            boolean isUnalignedCheckpoint, long alignmentTimeout) {
        return new CheckpointOptions(CheckpointType.CHECKPOINT, CheckpointStorageLocationReference.getDefault(),
                isExactlyOnceMode, isUnalignedCheckpoint, alignmentTimeout);
    }
}
