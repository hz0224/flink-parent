...
public class TaskExecutorPartitionTrackerImpl extends AbstractPartitionTracker<JobID, TaskExecutorPartitionInfo> implements TaskExecutorPartitionTracker {
    ...
    public void stopTrackingAndReleaseJobPartitionsFor(JobID producingJobId) {
        Collection<ResultPartitionID> partitionsForJob = CollectionUtil
                .project(stopTrackingPartitionsFor(producingJobId), PartitionTrackerEntry::getResultPartitionId);
    ...
    public void promoteJobPartitions(Collection<ResultPartitionID> partitionsToPromote) {
        ...
        for(PartitionTrackerEntry<JobID, TaskExecutorPartitionInfo> partitionTrackerEntry : partitionTrackerEntries) {
            ...
            final DataSetEntry dataSetEntry = clusterPartitions
                    .computeIfAbsent(dataSetMetaInfo.getIntermediateDataSetId(), ignored -> new DataSetEntry(dataSetMetaInfo.getNumberOfPartitions()));
    ...
    public ClusterPartitionReport createClusterPartitionReport() {
        List<ClusterPartitionReport.ClusterPartitionReportEntry> reportEntries = clusterPartitions.entrySet().stream()
                .map(entry -> new ClusterPartitionReport.ClusterPartitionReportEntry(entry.getKey(), entry.getValue().getPartitionIds(),
                        entry.getValue().getTotalNumberOfPartitions())).collect(Collectors.toList());
