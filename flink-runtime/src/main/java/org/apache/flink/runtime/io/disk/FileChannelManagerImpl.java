...
public class FileChannelManagerImpl implements FileChannelManager {
    ...
    public FileChannelManagerImpl(String[] tempDirs, String prefix) {
        ...
        shutdownHook = ShutdownHookUtil
                .addShutdownHook(this, String.format("%s-%s", getClass().getSimpleName(), prefix), LOG);
    ...
    public void close() throws Exception {
        ...
        IOUtils.closeAll(Arrays.stream(paths).filter(File::exists).map(FileChannelManagerImpl::getFileCloser)
                .collect(Collectors.toList()));

        ShutdownHookUtil
                .removeShutdownHook(shutdownHook, String.format("%s-%s", getClass().getSimpleName(), prefix), LOG);
    }
        ...
        return () -> {
            try {
                ...
                String errorMessage = String
                        .format("FileChannelManager failed to properly clean up temp file directory: %s", path);
