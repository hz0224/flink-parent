...
public class DefaultLeaderElectionService implements LeaderElectionService, LeaderElectionEventHandler {
    ...
    public final void start(LeaderContender contender) throws Exception {
        ...
        synchronized(lock) {
             ...
             *  注释： 注意这个 LeaderContender = DispatcherRestEndpoint
             *  -
             *  1、此时这个 contender = leaderContender = WebMonitorEndpoint
             *  2、此时这个 contender = leaderContender = ResourceManager
             *  3、此时这个 contender = leaderContender = DispatcherRunner
             *  -
             *  4、此时这个 contender = leaderContender = JobManagerRunnerImpl
             */
            leaderContender = contender;

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 当前这个 leaderElectionDriver 就是负责给 leaderContender 执行选举的
             *  如果选举成功，则回调： leaderElectionDriver 的 isLeader 方法
             *  -
             *  LeaderElectionDriver flink-1.12 版本新封装的东西
             *  在Flink-1.11 版本以前是没有的。
             */
            leaderElectionDriver = leaderElectionDriverFactory
                    .createLeaderElectionDriver(this, new LeaderElectionFatalErrorHandler(), leaderContender.getDescription());
        ...
        synchronized(lock) {
            ...
            if(hasLeadership(leaderSessionID)) {
                ...
                if(!leaderSessionID.equals(this.issuedLeaderSessionID)) {
                    if(LOG.isDebugEnabled()) {
                        LOG.debug("Receive an old confirmation call of leader session ID {}, " + "current issued session ID is {}", leaderSessionID,
    ...
    private void confirmLeaderInformation(UUID leaderSessionID, String leaderAddress) {
        // TODO_MA 注释： 写入信息到 ZK 的内容包括： sessionID 和 address(host+port)
         ...
         *  注释：
         */
        leaderElectionDriver.writeLeaderInformation(LeaderInformation.known(confirmedLeaderSessionID, confirmedLeaderAddress));
    }
    ...
    public void onRevokeLeadership() {
        synchronized(lock) {
            if(running) {
                ...
                if(LOG.isDebugEnabled()) {
                    LOG.debug("Ignoring the revoke leadership notification since the {} " + "has already been closed.", leaderElectionDriver);
                }
            }
        }
    }
    ...
    public void onLeaderInformationChange(LeaderInformation leaderInformation) {
        synchronized(lock) {
            if(running) {
                if(LOG.isDebugEnabled()) {
                    LOG.debug("Leader node changed while {} is the leader with session ID {}.", leaderContender.getDescription(), confirmedLeaderSessionID);
                }
                if(confirmedLeaderSessionID != null) {

                    final LeaderInformation confirmedLeaderInfo = LeaderInformation.known(confirmedLeaderSessionID, confirmedLeaderAddress);

                    if(leaderInformation.isEmpty()) {
                        if(LOG.isDebugEnabled()) {
                            LOG.debug("Writing leader information by {} since the external storage is empty.", leaderContender.getDescription());
                        }
