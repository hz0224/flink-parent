...
/**
 * // TODO_MA 注释： 为注册的工作检索当前工作领导者ID（负责该工作的工作经理的领导者ID）的服务。
 * Service which retrieves for a registered job the current job leader id (the leader id of the job manager responsible
 * for the job).
 *
 * // TODO_MA 注释： 领导者ID将来会通过 {@link＃getLeaderId（JobID）} 公开。
 * The leader id will be exposed as a future via the {@link#getLeaderId(JobID)}.
 *
 * // TODO_MA 注释： 将来只有在停止服务的情况下才能完成。
