...
public class CheckpointStatsHistory implements Serializable {
    ...
    CheckpointStatsHistory(int maxSize) {
        this(false, maxSize, new AbstractCheckpointStats[0], Collections.emptyList(), Collections.emptyMap(), null, null,
                null);
    }
     ...
     */
    private CheckpointStatsHistory(boolean readOnly, int maxSize, AbstractCheckpointStats[] checkpointArray,
            List<AbstractCheckpointStats> checkpointsHistory, Map<Long, AbstractCheckpointStats> checkpointsById,
            @Nullable CompletedCheckpointStats latestCompletedCheckpoint,
            @Nullable FailedCheckpointStats latestFailedCheckpoint, @Nullable CompletedCheckpointStats latestSavepoint) {
    ...
    CheckpointStatsHistory createSnapshot() {
        ...
        return new CheckpointStatsHistory(true, maxSize, null, checkpointsHistory, checkpointsById,
                latestCompletedCheckpoint, latestFailedCheckpoint, latestSavepoint);
    }
    ...
    boolean replacePendingCheckpointById(AbstractCheckpointStats completedOrFailed) {
        checkArgument(!completedOrFailed.getStatus().isInProgress(),
        ...
        if(completedOrFailed.getStatus().isCompleted()) {
            ...
            if(completed.getProperties().isSavepoint() && (latestSavepoint == null || completed
                    .getCheckpointId() > latestSavepoint.getCheckpointId())) {

                latestSavepoint = completed;
            } else if(latestCompletedCheckpoint == null || completed.getCheckpointId() > latestCompletedCheckpoint
                    .getCheckpointId()) {
            ...
            if(latestFailedCheckpoint == null || failed.getCheckpointId() > latestFailedCheckpoint.getCheckpointId()) {
