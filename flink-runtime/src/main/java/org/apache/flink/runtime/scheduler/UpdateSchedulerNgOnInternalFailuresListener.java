...
class UpdateSchedulerNgOnInternalFailuresListener implements InternalFailuresListener {
    ...
    public UpdateSchedulerNgOnInternalFailuresListener(final SchedulerNG schedulerNg, final JobID jobId) {
        this.schedulerNg = checkNotNull(schedulerNg);
        this.jobId = checkNotNull(jobId);
    }

    @Override
    public void notifyTaskFailure(final ExecutionAttemptID attemptId, final Throwable t, final boolean cancelTask,
            final boolean releasePartitions) {
        final TaskExecutionState state = new TaskExecutionState(jobId, attemptId, ExecutionState.FAILED, t);
