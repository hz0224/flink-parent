...
public class SingleInputGate extends IndexedInputGate {
    ...
    /**
     * // TODO_MA 注释： ResultSubPartition 的 Index
    ...
    /**
     * // TODO_MA 注释： 该 InputGate 的总 InputChannel 数量
     ...
     *  注释： 一个 InputGate 的内部，由多个 InputChannel 组成
     */
    ...
    public void setup() throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 为 InputChannel 申请 Buffer
         */
        setupChannels();

        BufferPool bufferPool = bufferPoolFactory.get();

        // TODO_MA 注释： 设置 BufferPool
        ...
        for(Map.Entry<IntermediateResultPartitionID, InputChannel> entry : inputChannels.entrySet()) {
            ...
            if(inputChannel instanceof RecoveredInputChannel) {
                try {
                    ...
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 如果是 RecoveredInputChannel 则进行类型转换
                     */
    ...
    private void internalRequestPartitions() {
        ...
        for(InputChannel inputChannel : inputChannels.values()) {
            try {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： InputChannel 订阅 ResultSubPartition
                 *  内部，其实是通过 NettyClient 发起链接请求到上游 Task 所在的 TaskExecutor 中的 NettyServer
    ...
    public void setupChannels() throws IOException {
        synchronized(requestLock) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 启动 InputChannel，其实就是为每个 InputChannel 去申请相应数量的 Buffer
             */
            for(InputChannel inputChannel : inputChannels.values()) {

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 每个 InputChannel 的 setup 其实就是申请得到两个 MemorySegment 然后封装成 NetoworkBuffer
                 */
                inputChannel.setup();
            }
        }
    ...
    public void setInputChannels(InputChannel... channels) {
        ...
        synchronized(requestLock) {
            ...
            // TODO_MA 注释： 遍历每个 InputChannel 执行和 分区的映射处理
            for(InputChannel inputChannel : channels) {
                ...
                // TODO_MA 注释： 注意： InputChannel 和 PartitionID 对应
        ...
        synchronized(requestLock) {
            ...
            if(current instanceof UnknownInputChannel) {
                ...
                if(isLocal) {
                    ...
                    RemoteInputChannel remoteInputChannel = unknownChannel
                            .toRemoteInputChannel(shuffleDescriptor.getConnectionId());
                ...
                try {
                    ...
                    for(InputChannel inputChannel : inputChannels.values()) {
                        try {
                            ...
                            LOG.warn("{}: Error during release of channel resources: {}.", owningTaskName, e.getMessage(),
                                    e);
                        }
                    }
    ...
    private Optional<BufferOrEvent> getNextBufferOrEvent(boolean blocking) throws IOException, InterruptedException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将读取到的数据，转换成 Buffer 或者 Event 返回
         */
        return Optional.of(transformToBufferOrEvent(inputWithData.data.buffer(), inputWithData.moreAvailable,
                inputWithData.input, inputWithData.morePriorityEvents));
    }
        ...
        while(true) {
            synchronized(inputChannelsWithData) {
                ...
                if(bufferAndAvailability.hasPriority()) {
                    lastPrioritySequenceNumber[inputChannel.getChannelIndex()] = bufferAndAvailability
                            .getSequenceNumber();
                ...
                // TODO_MA 注释： 返回一条数据： InputWithData
                return Optional
                        .of(new InputWithData<>(inputChannel, bufferAndAvailability, !inputChannelsWithData.isEmpty(),
                                morePriorityEvents));
            }
        }
    }
    ...
    private BufferOrEvent transformToBufferOrEvent(Buffer buffer, boolean moreAvailable, InputChannel currentChannel,
    ...
    private BufferOrEvent transformEvent(Buffer buffer, boolean moreAvailable, InputChannel currentChannel,
            boolean morePriorityEvents) throws IOException, InterruptedException {
        ...
        return new BufferOrEvent(event, buffer.getDataType().hasPriority(), currentChannel.getChannelInfo(),
                moreAvailable, buffer.getSize(), morePriorityEvents);
    }
    ...
    void triggerPartitionStateCheck(ResultPartitionID partitionId) {
        partitionProducerStateProvider.requestPartitionProducerState(consumedResultId, partitionId,
     ...
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释：
     *  1、接收到数据，编程buffer，加入到队列：receivedBuffers
     *  2、对应到的某个 inputChannel 变成可用的，所以加入到可用 inputchannel 队列
     */
    private void queueChannel(InputChannel channel, @Nullable Integer prioritySequenceNumber) {
        ...
        try(GateNotificationHelper notification = new GateNotificationHelper(this, inputChannelsWithData)) {
            synchronized(inputChannelsWithData) {
                ...
                if(priority && isOutdated(prioritySequenceNumber,
    ...
    private boolean isOutdated(int sequenceNumber, int lastSequenceNumber) {
        if((lastSequenceNumber < 0) != (sequenceNumber < 0) && Math
                .max(lastSequenceNumber, sequenceNumber) > Integer.MAX_VALUE / 2) {
    ...
    private boolean queueChannelUnsafe(InputChannel channel, boolean priority) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 有数据可读的 inputchannel 增加一个
         *  某一个 InputChannel 中已经接收到了一条数据！
         *  如果某个channel 接收到数据，也就意味着，这个channel 是可用，则加入到这个 队列
         */
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 一定会有一个地方在消费这个队列！
         *  从 OneInputStreamTask 的 processInput（） 方法中找！
         *  SourceStreamTask 去
         */
    ...
    private Optional<InputChannel> getChannel(boolean blocking) throws InterruptedException {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 阻塞
         *  1、OneInputStreamTask 一启动起来，就会执行 processInput() 进入到等待获取数据的 流程中，但是阻塞了
         *  2、如果 inputChannelsWithData 中，被加入了一个有可用可消费数据的 InputChannel 的时候。
         *  那么该方法： getChannel 可返回！
         */
