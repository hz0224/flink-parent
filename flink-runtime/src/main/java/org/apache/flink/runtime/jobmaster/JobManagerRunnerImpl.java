...
public class JobManagerRunnerImpl implements LeaderContender, OnCompletionActions, JobManagerRunner {
     ...
     */
    public JobManagerRunnerImpl(final JobGraph jobGraph, final JobMasterServiceFactory jobMasterFactory, final HighAvailabilityServices haServices,
            final LibraryCacheManager.ClassLoaderLease classLoaderLease, final Executor executor, final FatalErrorHandler fatalErrorHandler,
        ...
        // TODO_MA 注释： ZooKeeperRunningJobsRegistry
        // high availability services next
        this.runningJobsRegistry = haServices.getRunningJobsRegistry();

        // TODO_MA 注释： 返回 ZooKeeperLeaderElectionService = DefaultLeaderElectionService
        ...
        synchronized(lock) {
             ...
             *  注释： 启动 JobManagerRunnerImpl 的选举功能
             *  JobManagerRunnerImpl LeaderContender 选举者角色
             *  当选举成功，则会回调： this.grantLeadership() 方法
             */
            try {
    ...
    public CompletableFuture<Void> closeAsync() {
        synchronized(lock) {
            if(!shutdown) {
                ...
                jobManagerTerminationFuture.whenComplete((Void ignored, Throwable throwable) -> {
                    try {
                    ...
                    if(throwable != null) {
                        terminationFuture.completeExceptionally(new FlinkException("Could not properly shut down the JobManagerRunner", throwable));
                    } else {
    ...
    private void unregisterJobFromHighAvailability() {
        try {
            ...
            log.error("Could not un-register from high-availability services job {} ({})." + "Other JobManager's may attempt to recover it and re-execute it.",
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 当 JobManagerRunnerImpl 选举成功，就会回调 grantLeadership 方法
     */
    @Override
    public void grantLeadership(final UUID leaderSessionID) {
        jobMasterCreationFuture.whenComplete((ignore, throwable) -> {
            if(throwable != null) {
            ...
            synchronized(lock) {
                ...
                leadershipOperation = leadershipOperation.thenCompose((ignored) -> {
                    synchronized(lock) {
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 确认状态，并且启动 JobMaster
                         *  1、verifyJobSchedulingStatus 校验 job 的状态
                         *  2、StartJobManager 启动 JobManager（jobMaster）
                         */
    ...
    private CompletionStage<Void> startJobMaster(UUID leaderSessionId) {
        log.info("JobManager runner for job {} ({}) was granted leadership with session id {} at {}.", jobGraph.getName(), jobGraph.getJobID(), leaderSessionId,
                jobMasterService.getAddress());

        // TODO_MA 注释： 注册： job is running
        try {
            runningJobsRegistry.setJobRunning(jobGraph.getJobID());
        } catch(IOException e) {
            return FutureUtils.completedExceptionally(
                    new FlinkException(String.format("Failed to set the job %s to running in the running jobs registry.", jobGraph.getJobID()), e));
        }
        ...
        return startFuture.thenAcceptAsync(
                (Acknowledge ack) -> confirmLeaderSessionIdIfStillLeader(leaderSessionId, jobMasterService.getAddress(), currentLeaderGatewayFuture), executor);
    }
    ...
    private CompletableFuture<JobSchedulingStatus> getJobSchedulingStatus() {
        try {
            ...
            return FutureUtils.completedExceptionally(
                    new FlinkException(String.format("Could not retrieve the job scheduling status for job %s.", jobGraph.getJobID()), e));
        }
    }

    private void confirmLeaderSessionIdIfStillLeader(UUID leaderSessionId, String leaderAddress,
    ...
    private CompletableFuture<Void> revokeJobMasterLeadership() {
        log.info("JobManager for job {} ({}) at {} was revoked leadership.", jobGraph.getName(), jobGraph.getJobID(), jobMasterService.getAddress());

        setNewLeaderGatewayFuture();

        return jobMasterService.suspend(new FlinkException("JobManager is no longer the leader.")).thenApply(FunctionUtils.nullFn());
    }
    ...
    private void setNewLeaderGatewayFuture() {
        ...
        if(!oldLeaderGatewayFuture.isDone()) {
            leaderGatewayFuture.whenComplete((JobMasterGateway jobMasterGateway, Throwable throwable) -> {
                if(throwable != null) {
