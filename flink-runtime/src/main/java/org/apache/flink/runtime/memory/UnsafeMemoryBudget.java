...
class UnsafeMemoryBudget {
    ...
    void reserveMemory(long size, int maxSleeps) throws MemoryReservationException {
        ...
        try {
            ...
            throw new MemoryReservationException(String.format(
                    "Could not allocate %d bytes, only %d bytes are remaining. This usually indicates " + "that you are requesting more memory than you have reserved. " + "However, when running an old JVM version it can also be caused by slow garbage collection. " + "Try to upgrade to Java 8u72 or higher if running on an old Java version.",
    ...
    void releaseMemory(@Nonnegative long size) {
        ...
        while(!released && totalMemorySize >= (currentAvailableMemorySize = availableMemorySize.get()) + size) {
            released = availableMemorySize.compareAndSet(currentAvailableMemorySize, currentAvailableMemorySize + size);
        }
        if(!released) {
