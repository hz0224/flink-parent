...
class CreditBasedPartitionRequestClientHandler extends ChannelInboundHandlerAdapter implements NetworkClientHandler {
    ...
    // TODO_MA 注释： 这个 Message 就是从 NettyServer 中接收到的一条数据
    @Override
    ...
    private void decodeMsg(Object msg) throws Throwable {
        ...
        if(msgClazz == NettyMessage.BufferResponse.class) {
            ...
            // TODO_MA 注释： 接收到的数据
            // TODO_MA 注释： Buffer 正常业务数据， Event：CheckpointBarrier
            NettyMessage.BufferResponse bufferOrEvent = (NettyMessage.BufferResponse) msg;

            // TODO_MA 注释： 数据对应的 InputChannel
            ...
            if(error.isFatalError()) {
                notifyAllChannelsOfErrorAndClose(
                        new RemoteTransportException("Fatal error at remote task manager '" + remoteAddr + "'.",
                ...
                if(inputChannel != null) {
                    if(error.cause.getClass() == PartitionNotFoundException.class) {
                        ...
                        inputChannel.onError(
                                new RemoteTransportException("Error at remote task manager '" + remoteAddr + "'.",
                                        remoteAddr, error.cause));
                    }
                }
            }
