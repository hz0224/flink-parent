...
public class BufferFileChannelReader {
    ...
    public boolean readBufferFromFileChannel(Buffer buffer) throws IOException {
        ...
        if(size > buffer.getMaxCapacity()) {
            throw new IllegalStateException("Buffer is too small for data: " + buffer
                    .getMaxCapacity() + " bytes available, but " + size + " needed. This is most likely due to an serialized event, which is larger than the buffer size.");
        }
