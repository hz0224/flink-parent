...
public class StateBackendLoader {
    ...
    public static StateBackend loadStateBackendFromConfig(ReadableConfig config, ClassLoader classLoader,
         ...
         *  注释： 获取配置的 StateBackend
         *  backendName 这个值有三个情况：
         *  jobmanager  MemoryStateBackend
         *  filesystem  FsStateBackend
         *  rocksdb     RocksDBStateBackend
         *  -
         *  ke = CheckpointingOptions.STATE_BACKEND = state.backend
         *  value = backendName = jobmanager、filesystem、rocksdb
         */
        ...
        switch(backendName.toLowerCase()) {
            ...
            // TODO_MA 注释： 如果配置是 jobManager， 则 MemoryStateBackend
            case MEMORY_STATE_BACKEND_NAME:
                ...
                // TODO_MA 注释： 如果配置是 filesystem， 则 FsStateBackend
            case FS_STATE_BACKEND_NAME:
                FsStateBackend fsBackend = new FsStateBackendFactory().createFromConfig(config, classLoader);
                if(logger != null) {
                    logger.info("State backend is set to heap memory (checkpoints to filesystem \"{}\")", fsBackend.getCheckpointPath());
                }
                return fsBackend;

                // TODO_MA 注释： rocksdb ， 则 RocksDBStateBackend
            ...
            default:
                ...
                try {
                    @SuppressWarnings("rawtypes") Class<? extends StateBackendFactory> clazz = Class.forName(factoryClassName, false, classLoader)
                    ...
                    throw new DynamicCodeLoadingException("The class configured under '" + CheckpointingOptions.STATE_BACKEND
                            .key() + "' is not a valid state backend factory (" + backendName + ')', e);
                }

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 根据 RocksDBStateBackendFactory 创建得到 RocksDBStateBackend
                 */
     ...
     *                                       backend
     */
    public static StateBackend fromApplicationOrConfigOrDefault(@Nullable StateBackend fromApplication, Configuration config, ClassLoader classLoader,
            @Nullable Logger logger) throws IllegalConfigurationException, DynamicCodeLoadingException, IOException {
        ...
        // TODO_MA 注释： 如果 Application 已经定义了。获取就行
        ...
        if(fromApplication != null) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第二种情况，如果 Application 没有定义，则从配置中，获取
             */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 如果配置中，也没有配置，从使用默认的 StateBackend
             */
            else {
                // (3) use the default
    ...
    public static boolean stateBackendFromApplicationOrConfigOrDefaultUseManagedMemory(Configuration config,
            Optional<Boolean> stateBackendFromApplicationUsesManagedMemory, ClassLoader classLoader) {
