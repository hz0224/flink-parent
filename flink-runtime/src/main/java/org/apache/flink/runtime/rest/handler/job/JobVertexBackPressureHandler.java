...
public class JobVertexBackPressureHandler extends AbstractRestHandler<RestfulGateway, EmptyRequestBody, JobVertexBackPressureInfo, JobVertexMessageParameters> {
    ...
    public JobVertexBackPressureHandler(GatewayRetriever<? extends RestfulGateway> leaderRetriever, Time timeout,
    ...
    protected CompletableFuture<JobVertexBackPressureInfo> handleRequest(
         ...
         *  注释：
         */
        return gateway.requestOperatorBackPressureStats(jobId, jobVertexId).thenApply(
                operatorBackPressureStats -> operatorBackPressureStats.getOperatorBackPressureStats()
    ...
    private static JobVertexBackPressureInfo createJobVertexBackPressureInfo(
        ...
        return new JobVertexBackPressureInfo(JobVertexBackPressureInfo.VertexBackPressureStatus.OK,
                getBackPressureLevel(operatorBackPressureStats.getMaxBackPressureRatio()),
                operatorBackPressureStats.getEndTimestamp(),
                IntStream.range(0, operatorBackPressureStats.getNumberOfSubTasks()).mapToObj(subtask -> {
                    final double backPressureRatio = operatorBackPressureStats.getBackPressureRatio(subtask);
                    return new JobVertexBackPressureInfo.SubtaskBackPressureInfo(subtask,
                            getBackPressureLevel(backPressureRatio), backPressureRatio);
