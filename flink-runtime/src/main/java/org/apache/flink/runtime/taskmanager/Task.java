...
public class Task implements Runnable, TaskSlotPayload, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
    ...
    public Task(JobInformation jobInformation, TaskInformation taskInformation, ExecutionAttemptID executionAttemptID, AllocationID slotAllocationId,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： ShuffleIOOwnerContext
         *  在次数，注意关注 NettyShuffleEnvironment 的初始化。
         *  在 TaskManagerServices 对象初始化的时候创建的 NettyShuffleEnvironment
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建 Task 实例的时候，就已经把 ResultPartitionWriter 构建好了
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： ConsumableNotifyingResultPartitionWriter
         *  把每一个 ResultPartitioin = PipelinedResultPartition 包装成 ConsumableNotifyingResultPartitionWriter
         */
    ...
    private void doRun() {
        ...
        // all resource acquisitions and registrations from here on need to be undone in the end
        ...
        try {
            ...
            // TODO_MA 注释： 准备 ClassLoader， 因为将来会通过 反射的方式来启动 Task 执行
            userCodeClassLoader = createUserCodeClassloader();

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第二步：准备 ExecutionConfig
             */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第三步： 注册 ResultPartition 输出组件 和 InputGate　等输入组件
             */
            setupPartitionsAndGates(consumableNotifyingPartitionWriters, inputGates);

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第四步： 注册 ResultPartition 到 taskEventDispatcher
             */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第五步： 从分布式缓存中，拷贝下来一些运行 Task 所需要的资源文件
             */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第六步： 初始 环境对象 RuntimeEnvironment, 包装在 Task 执行过程中需要的各种组件
             */
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第七步： 通过反射 获取启动类实例
             *  在这句代码中，会涉及到 SourceStreamTask 或者 StreamTask 的初始化。
             *  取决于当前这个 ExecutionVertex 是属于哪一个 Operator 的
             *  -
             *  当前这个 Task 必定属于某一个 ExecutionVertex, 都有一个启动类的 成员变量
             *  当来这个 Task 启动的到底是那种具体的 Task？
             *  invokable 的类别： SourceStreamTask， OneInputStreamTask， ....
             *  -
             *  要去找： SourceStreamTask 和 OneInputStreamTask 的 带 RuntimeEnvironment 参数的构造方法！
             *  -
             *  相同点：都是 StreamTask 的子类： 内部会初始化 输出 和 输入 组件
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第十步： Task 切换进入 RUNNING 状态， 并告知 JobMaster
             */
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第十二步
             *  ResultPartitionWriter 完成所有 还未 flush 的数据的 flush 动作
             */
            ...
            // counted as finished when this happens errors here will only be logged
            try {
                metrics.close();
            } catch(Throwable t) {
    ...
    public static void setupPartitionsAndGates(ResultPartitionWriter[] producedPartitions, InputGate[] inputGates) throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 给所有的 Task 进行注册，让 ResultPartitionManager 进行管理
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 InputGate，为 InputGate 中的 InputChannel 申请 Buffer
         */
    ...
    public void triggerCheckpointBarrier(final long checkpointID, final long checkpointTimestamp, final CheckpointOptions checkpointOptions,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 如果 Task 的 状态是 RUNNING 并且 invokable 不为空，证明 Task 是正常执行的状态
         */
        if(executionState == ExecutionState.RUNNING && invokable != null) {
            try {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 触发 invokable 的 checkpoint
                 *  此处的 invokable 需要注意： SourceStreamTask
                 *  这个 Task 只可能是 SourceStreamTask
                 */
    ...
    private static AbstractInvokable loadAndInstantiateInvokable(ClassLoader classLoader, String className, Environment environment) throws Throwable {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：
         *  1、Task 是一个通用的抽象： flink 系统中的任何 具体 Task 都是由这个来封装启动
         *  2、具体的Task： AbstractInvokable
         *
         */

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 获取 invokableClass
         *  关于 invokableClass 有典型的两种：
         *  1、第一种： 普通的一般情况下的 StreamTask
         *  2、第二种： SourceStreamTask
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 获取 AbstractInvokable 的 Constructor 实例
         */
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建实例
         *  此时代码跳转到： SourceStreamTask 的 带 environment 的构造方法
         *                 OneInputStreamTask
         */
