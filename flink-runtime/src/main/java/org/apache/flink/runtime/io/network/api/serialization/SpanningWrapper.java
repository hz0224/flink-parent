...
final class SpanningWrapper {
    ...
    private CloseableIterator<Buffer> createSpilledDataIterator() throws IOException {
        ...
        return CloseableIterator.flatten(toSingleBufferIterator(wrapInt(recordLength)),
                new FileBasedBufferIterator(spillFile, min(accumulatedRecordBytes, recordLength), fileBufferSize),
                leftOverData == null ? empty() : toSingleBufferIterator(
    ...
    private FileChannel createSpillingChannel() throws IOException {
        ...
        throw new IOException("Could not find a unique file channel name in '" + Arrays
                .toString(tempDirs) + "' for spilling large records during deserialization.");
    }
    ...
    private static CloseableIterator<Buffer> toSingleBufferIterator(MemorySegment segment) {
        NetworkBuffer buffer = new NetworkBuffer(segment, FreeingBufferRecycler.INSTANCE, Buffer.DataType.DATA_BUFFER,
                segment.size());
