...
public class DefaultSchedulerComponents {
    ...
    private DefaultSchedulerComponents(final SchedulingStrategyFactory schedulingStrategyFactory,
     ...
     *  注释： 创建调度相关组件
     */
    static DefaultSchedulerComponents createSchedulerComponents(final ScheduleMode scheduleMode,
            final boolean isApproximateLocalRecoveryEnabled, final Configuration jobMasterConfiguration,
            final SlotPool slotPool, final Time slotRequestTimeout) {

        // TODO_MA 注释： jobmanager.scheduler.scheduling-strategy = region
        ...
        switch(schedulingStrategy) {
            ...
            case PIPELINED_REGION_SCHEDULING:
                checkArgument(!isApproximateLocalRecoveryEnabled,
                        "Approximate local recovery can not be used together with PipelinedRegionScheduler for now! " + "Please set %s to legacy.",
                 ...
                 *  注释： 创建各种组件
                 */
                return createPipelinedRegionSchedulerComponents(scheduleMode, jobMasterConfiguration, slotPool,
                        slotRequestTimeout);
            ...
            case LEGACY_SCHEDULING:
                ...
                return createLegacySchedulerComponents(scheduleMode, jobMasterConfiguration, slotPool,
                        slotRequestTimeout);
            default:
    ...
    private static DefaultSchedulerComponents createLegacySchedulerComponents(final ScheduleMode scheduleMode,
            final Configuration jobMasterConfiguration, final SlotPool slotPool, final Time slotRequestTimeout) {
        ...
        final SlotProviderStrategy slotProviderStrategy = SlotProviderStrategy
                .from(scheduleMode, scheduler, slotRequestTimeout);
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：
         */
        return new DefaultSchedulerComponents(createLegacySchedulingStrategyFactory(scheduleMode), scheduler::start,
    ...
    private static DefaultSchedulerComponents createPipelinedRegionSchedulerComponents(final ScheduleMode scheduleMode,
        ...
        // TODO_MA 注释： SlotSharingExecutionSlotAllocatorFactory
        final ExecutionSlotAllocatorFactory allocatorFactory = new SlotSharingExecutionSlotAllocatorFactory(
                physicalSlotProvider, scheduleMode != ScheduleMode.LAZY_FROM_SOURCES_WITH_BATCH_SLOT_REQUEST, bulkChecker,
                slotRequestTimeout);

        // TODO_MA 注释： PipelinedRegionSchedulingStrategy
        return new DefaultSchedulerComponents(new PipelinedRegionSchedulingStrategy.Factory(), bulkChecker::start,
                allocatorFactory);
    }
    ...
    private static SlotSelectionStrategy selectSlotSelectionStrategy(final Configuration configuration) {
        ...
        locationPreferenceSlotSelectionStrategy = evenlySpreadOutSlots ? LocationPreferenceSlotSelectionStrategy
                .createEvenlySpreadOut() : LocationPreferenceSlotSelectionStrategy.createDefault();

        return configuration.getBoolean(CheckpointingOptions.LOCAL_RECOVERY) ? PreviousAllocationSlotSelectionStrategy
                .create(locationPreferenceSlotSelectionStrategy) : locationPreferenceSlotSelectionStrategy;
    }
}
