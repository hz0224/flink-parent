...
public class PipelinedSubpartition extends ResultSubpartition implements CheckpointedResultSubpartition, ChannelStateHolder {
    ...
    private boolean addBuffer(BufferConsumer bufferConsumer, int partialRecordLength) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： add BufferConsumerWithPartialRecordLength
         */
    ...
    private boolean processPriorityBuffer(BufferConsumer bufferConsumer, int partialRecordLength) {
        ...
        if(barrier != null) {
            ...
            if(!inflightBuffers.isEmpty()) {
                 ...
                 */
                channelStateWriter
                        .addOutputData(barrier.getId(), subpartitionInfo, ChannelStateWriter.SEQUENCE_NUMBER_UNKNOWN,
    ...
    BufferAndBacklog pollBuffer() {
        synchronized(buffers) {
            ...
            // TODO_MA 注释： 此处为 false， 不执行 return
            if(isBlocked) {
                return null;
            }
            ...
            // queue
            // will be 2 or more.
            return new BufferAndBacklog(buffer, getBuffersInBacklog(),
    ...
    public PipelinedSubpartitionView createReadView(BufferAvailabilityListener availabilityListener) {
        synchronized(buffers) {
            ...
            checkState(readView == null,
                    "Subpartition %s of is being (or already has been) consumed, " + "but pipelined subpartitions can only be consumed once.",
                    getSubPartitionIndex(), parent.getPartitionId());

            LOG.debug("{}: Creating read view for subpartition {} of partition {}.", parent.getOwningTaskName(),
                    getSubPartitionIndex(), parent.getPartitionId());
    ...
    public String toString() {
        ...
        return String
                .format("%s#%d [number of buffers: %d (%d bytes), number of buffers in backlog: %d, finished? %s, read view? %s]",
                        this.getClass().getSimpleName(), getSubPartitionIndex(), numBuffers, numBytes,
                        getBuffersInBacklog(), finished, hasReadView);
    }
    ...
    public void flush() {
        ...
        synchronized(buffers) {
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： buffers.size() == 1
             *  如果刚才的数据加入到 buffer 中的去的时候，那么里面只有 1 条数据，就说明，
             *  上一次加入 buffer 的数据，已经 flush 了
             *  按照类似的逻辑， 只要buffer中有数据，就要 调用 notifyDataAvailable
             */
            // if there is more then 1 buffer, we already notified the reader
            // (at the latest when adding the second buffer)
            notifyDataAvailable = !isBlocked && buffers.size() == 1 && buffers.peek().getBufferConsumer()
                    .isDataAvailable();
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 通知有数据可用
         *  1、假设现在有数据可用，我先给下游Task发送一个消息，告诉task有数据了额
         *  -
         *  2、事实上，就是把数据发送出去
         */
        if(notifyDataAvailable) {
    ...
    private void notifyDataAvailable() {
        ...
        // TODO_MA 注释： PipelinedSubpartitionView 存在于 Task 中，用来给对应的某个 ResultSubPartition 执行数据消费
        // TODO_MA 注释： 这个消费的概念： 是指，逻辑处理完成之后的 StrewamRecord 被序列化封装成一个 ByteBuffer 加入到
        // TODO_MA 注释： Buffers 队列中。 然后由： PipelinedSubpartitionView 去消费（执行真正的写出到下游）
        final PipelinedSubpartitionView readView = this.readView;
        if(readView != null) {

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 通过 PipelinedSubpartitionView 告知数据可用
             */
