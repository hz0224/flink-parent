...
class AkkaRpcActor<T extends RpcEndpoint & RpcGateway> extends AbstractActor {
    ...
    AkkaRpcActor(final T rpcEndpoint, final CompletableFuture<Boolean> terminationFuture, final int version,
        ...
        this.rpcEndpointTerminationResult = RpcEndpointTerminationResult.failure(new AkkaRpcException(
                String.format("RpcEndpoint %s has not been properly stopped.", rpcEndpoint.getEndpointId())));

        // TODO_MA 注释： state = StoppedState
    ...
    public void postStop() throws Exception {
        ...
        if(rpcEndpointTerminationResult.isSuccess()) {
            ...
            log.info("The RpcEndpoint {} failed.", rpcEndpoint.getEndpointId(),
    ...
    // TODO_MA 注释： 如果该 Actor 对应的 RpcEndpint 接收到 START 消息，则这个方法会被调用
    // TODO_MA 注释： 根据方法的的逻辑： 因为 START 就是 ControlMessages， 所有调用 handleControlMessage() 执行消息处理
    @Override
    public Receive createReceive() {
        return ReceiveBuilder.create()

                // TODO_MA 注释： 处理 RemoteHandshakeMessage 消息
                .match(RemoteHandshakeMessage.class, this::handleHandshakeMessage)

                // TODO_MA 注释： 处理 ControlMessages 消息
    ...
    private void handleMessage(final Object message) {
        if(state.isRunning()) {
            ...
            log.info("The rpc endpoint {} has not been started yet. Discarding message {} until processing is started.",
                    rpcEndpoint.getClass().getName(), message.getClass().getName());
    ...
    private void handleUnknownControlMessage(ControlMessages controlMessage) {
        final String message = String
                .format("Received unknown control message %s. Dropping this message!", controlMessage);
        log.warn(message);
    ...
    private void handleHandshakeMessage(RemoteHandshakeMessage handshakeMessage) {
        if(!isCompatibleVersion(handshakeMessage.getVersion())) {
            ...
            sendErrorIfSender(new AkkaHandshakeException(
                    String.format("The rpc endpoint does not support the gateway %s.",
    ...
    private void handleRpcInvocation(RpcInvocation rpcInvocation) {
        ...
        try {
            ...
            RpcConnectionException rpcException = new RpcConnectionException(
                    "Could not deserialize rpc invocation message.", e);
            getSender().tell(new Status.Failure(rpcException), getSelf());
    ...
    private void sendSyncResponse(Object response, String methodName) {
        if(isRemoteSender(getSender())) {
            Either<SerializedValue<?>, AkkaRpcException> serializedResult = serializeRemoteResultAndVerifySize(response,
                    methodName);
        ...
        FutureUtils.assertNoException(asyncResponse.handle((value, throwable) -> {
            if(throwable != null) {
                ...
                if(isRemoteSender(sender)) {
                    Either<SerializedValue<?>, AkkaRpcException> serializedResult = serializeRemoteResultAndVerifySize(
                            value, methodName);
    ...
    private Either<SerializedValue<?>, AkkaRpcException> serializeRemoteResultAndVerifySize(Object result,
            String methodName) {
        try {
            ...
            if(resultSize > maximumFramesize) {
                return Either.Right(new AkkaRpcException(
                        "The method " + methodName + "'s result size " + resultSize + " exceeds the maximum size " + maximumFramesize + " ."));
            } else {
                return Either.Left(serializedResult);
            }
        } catch(IOException e) {
            return Either
                    .Right(new AkkaRpcException("Failed to serialize the result for RPC call : " + methodName + '.', e));
        }
    }
    ...
    private void handleRunAsync(RunAsync runAsync) {
        ...
        if(timeToRun == 0 || (delayNanos = timeToRun - System.nanoTime()) <= 0) {
            ...
            getContext().system().scheduler()
                    .scheduleOnce(delay, getSelf(), envelopedSelfMessage, getContext().dispatcher(), ActorRef.noSender());
        }
    }
    ...
    enum StartedState implements State {
        ...
        public State terminate(AkkaRpcActor<?> akkaRpcActor) {
            ...
            try {
                ...
                terminationFuture = FutureUtils.completedExceptionally(new AkkaRpcException(
            ...
            terminationFuture
                    .whenComplete((ignored, throwable) -> akkaRpcActor.stop(RpcEndpointTerminationResult.of(throwable)));
    ...
    enum StoppedState implements State {
        ...
        public State start(AkkaRpcActor<?> akkaRpcActor) {
            ...
            try {
                ...
                akkaRpcActor.stop(RpcEndpointTerminationResult.failure(new AkkaRpcException(
