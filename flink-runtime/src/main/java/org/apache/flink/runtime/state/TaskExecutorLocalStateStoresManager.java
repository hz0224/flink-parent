...
public class TaskExecutorLocalStateStoresManager {
    ...
    public TaskExecutorLocalStateStoresManager(boolean localRecoveryEnabled, @Nonnull File[] localStateRootDirectories,
        ...
        for(File localStateRecoveryRootDir : localStateRootDirectories) {
            ...
            if(!localStateRecoveryRootDir.exists() && !localStateRecoveryRootDir.mkdirs()
                ...
                throw new IOException(
                        "Could not create root directory for local recovery: " + localStateRecoveryRootDir);
            }
        }
    ...
    @Nonnull
    public TaskLocalStateStore localStateStoreForSubtask(@Nonnull JobID jobId, @Nonnull AllocationID allocationID,
            @Nonnull JobVertexID jobVertexID, @Nonnegative int subtaskIndex) {
        ...
        synchronized(lock) {
            ...
            Map<JobVertexSubtaskKey, OwnedTaskLocalStateStore> taskStateManagers = this.taskStateStoresByAllocationID
                    .get(allocationID);
            ...
            if(taskLocalStateStore == null) {
                ...
                LocalRecoveryConfig localRecoveryConfig = new LocalRecoveryConfig(localRecoveryEnabled,
                        directoryProvider);
                ...
                LOG.debug(
                        "Registered new local state store with configuration {} for {} - {} - {} under allocation " + "id {}.",
                        localRecoveryConfig, jobId, jobVertexID, subtaskIndex, allocationID);

            } else {
                LOG.debug("Found existing local state store for {} - {} - {} under allocation id {}: {}", jobId,
                        jobVertexID, subtaskIndex, allocationID, taskLocalStateStore);
            }
