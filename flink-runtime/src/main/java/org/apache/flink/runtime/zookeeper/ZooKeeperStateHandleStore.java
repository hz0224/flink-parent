...
public class ZooKeeperStateHandleStore<T extends Serializable> implements StateHandleStore<T, IntegerResourceVersion> {
     ...
     *
     * // TODO_MA 注释： 重要：这将不将实际状态存储在ZooKeeper中，但创建一个状态句柄并将其存储在ZooKeeper中。
     * // TODO_MA 注释： 这种间接级别可确保 ZooKeeper 中的数据很小。
    ...
    public RetrievableStateHandle<T> addAndLock(String pathInZooKeeper, T state) throws Exception {
         ...
         *  注释： RetrievableStreamStateHandle
         */
        ...
        try {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： RetrievableStateHandle 句柄对象，进行序列化
             */
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 将 serializedStoreHandle 对象存储在 zk 上
             */
            ...
            client.inTransaction().create().withMode(CreateMode.PERSISTENT).forPath(path, serializedStoreHandle).and()
                    .create().withMode(CreateMode.EPHEMERAL).forPath(getLockPath(path)).and().commit();
    ...
    public void replace(String pathInZooKeeper, IntegerResourceVersion expectedVersion, T state) throws Exception {
        ...
        try {
            ...
            client.setData().withVersion(expectedVersion.getValue()).forPath(path, serializedStateHandle);
            success = true;
            ...
            if(stat == null) {
                ...
                for(String path : children) {
                    ...
                    try {
                        ...
                        LOG.warn(
                                "Could not get all ZooKeeper children. Node {} contained " + "corrupted data. Ignoring this node.",
                                path, ioException);
                    }
                }
    ...
    private RetrievableStateHandle<T> get(String pathInZooKeeper, boolean lock) throws Exception {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 获取数据，构建 RetrievableStateHandle
         */
        try {
            byte[] data = client.getData().forPath(path);
            RetrievableStateHandle<T> retrievableStateHandle = InstantiationUtil
