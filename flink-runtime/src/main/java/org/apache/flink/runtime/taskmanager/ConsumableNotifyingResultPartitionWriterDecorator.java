...
public class ConsumableNotifyingResultPartitionWriterDecorator {
    ...
    public static ResultPartitionWriter[] decorate(Collection<ResultPartitionDeploymentDescriptor> descs,
        ...
        for(ResultPartitionDeploymentDescriptor desc : descs) {
            ...
            // TODO_MA 注释： 如果是流式 Task
            if(desc.sendScheduleOrUpdateConsumersMessage() && desc.getPartitionType().isPipelined()) {

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 将 PiplinedSubPartitioin 包装成 ConsumableNotifyingResultPartitionWriter
                 */
                consumableNotifyingPartitionWriters[counter] = new ConsumableNotifyingResultPartitionWriter(taskActions,
                        jobId, partitionWriters[counter], notifier);
            } else {
    ...
    private static final class ConsumableNotifyingResultPartitionWriter implements ResultPartitionWriter, CheckpointedResultPartition {
        ...
        public ConsumableNotifyingResultPartitionWriter(TaskActions taskActions, JobID jobId,
                ResultPartitionWriter partitionWriter, ResultPartitionConsumableNotifier partitionConsumableNotifier) {
        ...
        private void notifyPipelinedConsumers() {
            if(!hasNotifiedPipelinedConsumers && !partitionWriter.isReleased()) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： RpcResultPartitionConsumableNotifier
                 */
                partitionConsumableNotifier
                        .notifyPartitionConsumable(jobId, partitionWriter.getPartitionId(), taskActions);
