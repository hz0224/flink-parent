...
public class SlotManagerConfiguration {
    ...
    public SlotManagerConfiguration(Time taskManagerRequestTimeout, Time slotRequestTimeout, Time taskManagerTimeout, boolean waitResultConsumedBeforeRelease,
            SlotMatchingStrategy slotMatchingStrategy, WorkerResourceSpec defaultWorkerResourceSpec, int numSlotsPerWorker, int maxSlotNum,
    ...
    public static SlotManagerConfiguration fromConfiguration(Configuration configuration,
        ...
        // TODO_MA 注释： Slot 申请超时时间：5 * 60 * 1000 ms
        final Time slotRequestTimeout = getSlotRequestTimeout(configuration);

        // TODO_MA 注释： TaskManager 超时时间：30 s
        ...
        // TODO_MA 注释： 默认的 SlotMatchingStrategy = false = AnyMatchingSlotMatchingStrategy
        boolean evenlySpreadOutSlots = configuration.getBoolean(ClusterOptions.EVENLY_SPREAD_OUT_SLOTS_STRATEGY);
        final SlotMatchingStrategy slotMatchingStrategy = evenlySpreadOutSlots ? LeastUtilizationSlotMatchingStrategy.INSTANCE : AnyMatchingSlotMatchingStrategy.INSTANCE;

        // TODO_MA 注释： 每个节点的 slot 个数： 默认是 1， 可配置
        int numSlotsPerWorker = configuration.getInteger(TaskManagerOptions.NUM_TASK_SLOTS);

        // TODO_MA 注释： 每个节点最大的 slot 个数： Integer.MAX_VALUE
        int maxSlotNum = configuration.getInteger(ResourceManagerOptions.MAX_SLOT_NUM);

        // TODO_MA 注释： 多余的 TaskManager 个数： 默认是 0
        int redundantTaskManagerNum = configuration.getInteger(ResourceManagerOptions.REDUNDANT_TASK_MANAGER_NUM);

        return new SlotManagerConfiguration(rpcTimeout, slotRequestTimeout, taskManagerTimeout, waitResultConsumedBeforeRelease, slotMatchingStrategy,
                defaultWorkerResourceSpec, numSlotsPerWorker, maxSlotNum, redundantTaskManagerNum);
    }
