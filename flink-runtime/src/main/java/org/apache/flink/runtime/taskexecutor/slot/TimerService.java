...
public class TimerService<K> {
    ...
    public void registerTimeout(final K key, final long delay, final TimeUnit unit) {
        Preconditions.checkState(timeoutListener != null, "The " + getClass().getSimpleName() + " has not been started.");

        // TODO_MA 注释： 如果已经存在，则选注销
    ...
    private static final class Timeout<K> implements Runnable {
        ...
        Timeout(final TimeoutListener<K> timeoutListener, final K key, final long delay, final TimeUnit unit,
