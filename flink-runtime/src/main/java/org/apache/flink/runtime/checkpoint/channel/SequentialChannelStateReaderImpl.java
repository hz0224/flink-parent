...
public class SequentialChannelStateReaderImpl implements SequentialChannelStateReader {
    ...
    public void readOutputData(ResultPartitionWriter[] writers,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 ResultSubpartitionRecoveredStateHandler
         */
    ...
    private <Info, Context, Handle extends AbstractChannelStateHandle<Info>> void readSequentially(
            StreamStateHandle streamStateHandle, List<Handle> channelStateHandles,
    ...
    private static <Info, Handle extends AbstractChannelStateHandle<Info>> Map<StreamStateHandle, List<Handle>> groupByDelegate(
        ...
        return states.map(stateHandleExtractor).flatMap(Collection::stream).peek(validate())
    ...
    private static <Info, Handle extends AbstractChannelStateHandle<Info>> List<Tuple2<Long, Info>> extractOffsetsSorted(
            List<Handle> channelStateHandles) {
        return channelStateHandles.stream().flatMap(SequentialChannelStateReaderImpl::extractOffsets)
                .sorted(comparingLong(offsetAndInfo -> offsetAndInfo.f0)).collect(toList());
    }

    private static <Info, Handle extends AbstractChannelStateHandle<Info>> Stream<Tuple2<Long, Info>> extractOffsets(
            Handle handle) {
...
class ChannelStateChunkReader {
    ...
    <Info, Context, Handle extends AbstractChannelStateHandle<Info>> void readChunk(FSDataInputStream source,
            long sourceOffset, RecoveredChannelStateHandler<Info, Context> stateHandler,
        ...
        while(length > 0) {
            ...
            // TODO_MA 注释： 获取 bufferWithContext
            RecoveredChannelStateHandler.BufferWithContext<Context> bufferWithContext = stateHandler
