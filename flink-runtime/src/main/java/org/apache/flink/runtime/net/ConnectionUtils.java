...
public class ConnectionUtils {
    ...
    public static InetAddress findConnectingAddress(InetSocketAddress targetAddress, long maxWaitMillis, long startLoggingAfter) throws IOException {
        ...
        final List<AddressDetectionState> strategies = Collections.unmodifiableList(
                Arrays.asList(AddressDetectionState.LOCAL_HOST, AddressDetectionState.ADDRESS, AddressDetectionState.FAST_CONNECT,
    ...
    public static class LeaderConnectingAddressListener implements LeaderRetrievalListener {
        ...
        private enum LeaderRetrievalState {
            NOT_RETRIEVED, RETRIEVED, NEWLY_RETRIEVED
        }
        ...
        public InetAddress findConnectingAddress(Duration timeout, Duration startLoggingAfter) throws LeaderRetrievalException {
            ...
            try {
                while(elapsedTimeMillis < timeout.toMillis()) {
                    ...
                    long timeToWait = Math.min(Math.max(timeout.toMillis() - elapsedTimeMillis, 0), currentSleepTime);
                ...
                throw new LeaderRetrievalException("Could not retrieve the connecting address to the " + "current leader with the akka URL " + akkaURL + ".",
                        e);
            }
        }
