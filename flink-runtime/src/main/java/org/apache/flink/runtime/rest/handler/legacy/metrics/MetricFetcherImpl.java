...
public class MetricFetcherImpl<T extends RestfulGateway> implements MetricFetcher {
    ...
    public MetricFetcherImpl(GatewayRetriever<T> retriever, MetricQueryServiceRetriever queryServiceRetriever, Executor executor, Time timeout,
            long updateInterval) {
    ...
    private void fetchMetrics() {
        ...
        try {
            ...
            if(optionalLeaderGateway.isPresent()) {
                ...
                jobDetailsFuture.whenCompleteAsync((MultipleJobsDetails jobDetails, Throwable throwable) -> {
                    if(throwable != null) {
                ...
                queryServiceAddressesFuture.whenCompleteAsync((Collection<String> queryServiceAddresses, Throwable throwable) -> {
                    if(throwable != null) {
                ...
                taskManagerQueryServiceGatewaysFuture.whenCompleteAsync((Collection<Tuple2<ResourceID, String>> queryServiceGateways, Throwable throwable) -> {
                    if(throwable != null) {
                        LOG.debug("Requesting TaskManager's path for query services failed.", throwable);
                    } else {
                        List<String> taskManagersToRetain = queryServiceGateways.stream().map((Tuple2<ResourceID, String> tuple) -> {
                            queryServiceRetriever.retrieveService(tuple.f1).thenAcceptAsync(this::queryMetrics, executor);
    ...
    private void retrieveAndQueryMetrics(String queryServiceAddress) {
        ...
        queryServiceGatewayFuture.whenCompleteAsync((MetricQueryServiceGateway queryServiceGateway, Throwable t) -> {
            if(t != null) {
    ...
    private void queryMetrics(final MetricQueryServiceGateway queryServiceGateway) {
        ...
        queryServiceGateway.queryMetrics(timeout).whenCompleteAsync((MetricDumpSerialization.MetricSerializationResult result, Throwable t) -> {
            if(t != null) {
    ...
    @Nonnull
    public static <T extends RestfulGateway> MetricFetcherImpl<T> fromConfiguration(final Configuration configuration,
         ...
         *  注释：
         */
        return new MetricFetcherImpl<>(dispatcherGatewayRetriever, metricQueryServiceGatewayRetriever, executor, timeout, updateInterval);
    }
}
