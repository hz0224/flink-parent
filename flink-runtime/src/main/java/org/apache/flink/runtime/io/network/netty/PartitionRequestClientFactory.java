...
class PartitionRequestClientFactory {
     ...
     */
    NettyPartitionRequestClient createPartitionRequestClient(
        ...
        while(true) {
            ...
            final CompletableFuture<NettyPartitionRequestClient> clientFuture = clients
                    .putIfAbsent(connectionId, newClientFuture);
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 重试次数，可以通过： taskmanager.network.retries 配置
     */
    ...
    private NettyPartitionRequestClient connect(
            ConnectionID connectionId) throws RemoteTransportException, InterruptedException {
        try {

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： NettyClient 链接 Netty 服务端
             *  这个 Channel 就是 Netty 中的 Channel 的概念
             *  和 Flnk 中的 InputChannel 注意区分
             */
            Channel channel = nettyClient.connect(connectionId.getAddress()).await().channel();

            // TODO_MA 注释： NetworkClientHandler
            // TODO_MA 注释： 是一个基于信用评分带流控的一个 NetworkClientHandler = CreditBasedPartitionRequestClientHandler
            NetworkClientHandler clientHandler = channel.pipeline().get(NetworkClientHandler.class);

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 构建一个 NettyPartitionRequestClient 返回
             */
            return new NettyPartitionRequestClient(channel, clientHandler, connectionId, this);
        } catch(InterruptedException e) {
            throw e;
        } catch(Exception e) {
            throw new RemoteTransportException("Connecting to remote task manager '" + connectionId
