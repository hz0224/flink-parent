...
public class NettyShuffleEnvironment implements ShuffleEnvironment<ResultPartition, SingleInputGate> {
    ...
    NettyShuffleEnvironment(ResourceID taskExecutorResourceId, NettyShuffleEnvironmentConfiguration config,
            NetworkBufferPool networkBufferPool, ConnectionManager connectionManager,
            ResultPartitionManager resultPartitionManager, FileChannelManager fileChannelManager,
            ResultPartitionFactory resultPartitionFactory, SingleInputGateFactory singleInputGateFactory,
            Executor ioExecutor) {
    ...
    @Override
    public ShuffleIOOwnerContext createShuffleIOOwnerContext(String ownerName, ExecutionAttemptID executionAttemptID,
    ...
    public List<ResultPartition> createResultPartitionWriters(ShuffleIOOwnerContext ownerContext,
        ...
        synchronized(lock) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 根据下游来决定 ResultPartition 的个数，一般来说，都是一个
             *  一个 ResultPartition 负责一个 Task 的所有输出的管理
             */
            ResultPartition[] resultPartitions = new ResultPartition[resultPartitionDeploymentDescriptors.size()];

            // TODO_MA 注释： 根据输出分区个数，遍历创建 ResultPartition
            for(int partitionIndex = 0; partitionIndex < resultPartitions.length; partitionIndex++) {
                ...
                resultPartitions[partitionIndex] = resultPartitionFactory
                        .create(ownerContext.getOwnerName(), partitionIndex,
    ...
    public List<SingleInputGate> createInputGates(ShuffleIOOwnerContext ownerContext,
        ...
        synchronized(lock) {
            ...
            @SuppressWarnings("deprecation") InputChannelMetrics inputChannelMetrics = new InputChannelMetrics(
                    networkInputGroup, ownerContext.getParentGroup());

            // TODO_MA 注释： 初始化一个 InputGate 数组
            ...
            for(int gateIndex = 0; gateIndex < inputGates.length; gateIndex++) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 创建 InputGate = SingleInputGate
                 */
                SingleInputGate inputGate = singleInputGateFactory
                        .create(ownerContext.getOwnerName(), gateIndex, igdd, partitionProducerStateProvider,
                                inputChannelMetrics);
    ...
    @Deprecated
    public void registerLegacyNetworkMetrics(MetricGroup metricGroup, ResultPartitionWriter[] producedPartitions,
            InputGate[] inputGates) {
        NettyShuffleMetricFactory
                .registerLegacyNetworkMetrics(config.isNetworkDetailedMetrics(), metricGroup, producedPartitions,
                        inputGates);
    }
