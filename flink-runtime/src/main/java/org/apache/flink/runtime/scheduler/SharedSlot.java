...
class SharedSlot implements SlotOwner, PhysicalSlot.Payload {
    ...
    SharedSlot(SlotRequestId physicalSlotRequestId, ResourceProfile physicalSlotResourceProfile,
            ExecutionSlotSharingGroup executionSlotSharingGroup, CompletableFuture<PhysicalSlot> slotContextFuture,
            boolean slotWillBeOccupiedIndefinitely, Consumer<ExecutionSlotSharingGroup> externalReleaseCallback) {
    ...
    private CompletableFuture<SingleLogicalSlot> allocateNonExistentLogicalSlot(ExecutionVertexID executionVertexId) {
        ...
        // TODO_MA 注释： 实现从 physicalSlot 到 LogicalSlot 的映射
    ...
    private SingleLogicalSlot createLogicalSlot(PhysicalSlot physicalSlot, SlotRequestId logicalSlotRequestId) {
         ...
         *  注释：
         */
        return new SingleLogicalSlot(logicalSlotRequestId, physicalSlot, null, Locality.UNKNOWN, this,
    ...
    void cancelLogicalSlotRequest(ExecutionVertexID executionVertexID, @Nullable Throwable cause) {
        Preconditions.checkState(state == State.ALLOCATED, "SharedSlot (physical request %s) has been released",
        ...
        if(logicalSlotFuture != null) {
            ...
            LOG.debug("No SlotExecutionVertexAssignment for logical {} from physical {}}", logicalSlotRequestId,
                    physicalSlotRequestId);
        }
    }
    ...
    public void release(Throwable cause) {
        ...
        Map<ExecutionVertexID, CompletableFuture<SingleLogicalSlot>> logicalSlotFutures = requestedLogicalSlots.keySetA()
                .stream()
