...
public class HighAvailabilityServicesUtils {
    ...
    public static HighAvailabilityServices createHighAvailabilityServices(Configuration configuration, Executor executor,
         ...
         *  注释： 在 fink-conf.yaml 配置文件中，我们会去配置： high-availability = zookeeper
         */
        ...
        switch(highAvailabilityMode) {
            case NONE:
                ...
                final String dispatcherRpcUrl = AkkaRpcServiceUtils
                        .getRpcUrl(hostnamePort.f0, hostnamePort.f1, AkkaRpcServiceUtils.createWildcardName(Dispatcher.DISPATCHER_NAME), addressResolution,
                                configuration);
                final String webMonitorAddress = getWebMonitorAddress(configuration, addressResolution);

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 如果不是 HA 集群，则使用 StandaloneHaServices
                 */
            ...
            case ZOOKEEPER:
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 得到一个 ZooKeeperHaServices ，内部封装了一个 ZK 客户端
    ...
    public static Path getClusterHighAvailableStoragePath(Configuration configuration) {
        ...
        // TODO_MA 注释： 获取 high-availability.storageDir
    ...
    private static ClientHighAvailabilityServices createCustomClientHAServices(Configuration config) throws FlinkException {
        ...
        try {
            ...
            throw new FlinkException(String.format("Could not create the client ha services from the instantiated HighAvailabilityServicesFactory %s.",
                    highAvailabilityServicesFactory.getClass().getName()), e);
        }
    }
