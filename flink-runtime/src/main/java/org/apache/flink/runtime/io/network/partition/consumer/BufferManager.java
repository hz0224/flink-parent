...
public class BufferManager implements BufferListener, BufferRecycler {
    ...
    void requestExclusiveBuffers(int numExclusiveBuffers) throws IOException {
        ...
        synchronized(bufferQueue) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 将申请到的独占 buffer 加入到 bufferQueue 中进行管理
             *  根据此处代码也可得知，其实一个 NetworkBuffer 就是包装了一个 MemorySegment
             *  MemorySegment 默认大小： 32kb
             */
    ...
    static final class AvailableBufferQueue {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 添加一个独占的buffer，如果当前可用的 buffer 总量超出了要求的数量，则向本地缓冲池归还一个流动的buffer
         *  返回值是新增的 buffer 数量
         */
        int addExclusiveBuffer(Buffer buffer, int numRequiredBuffers) {

            // TODO_MA 注释： 独占加一个 buffer
            exclusiveBuffers.add(buffer);

            // TODO_MA 注释： 加一个，归还一个浮动，相当于没加
