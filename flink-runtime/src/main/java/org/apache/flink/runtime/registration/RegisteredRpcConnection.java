...
public abstract class RegisteredRpcConnection<F extends Serializable, G extends RpcGateway, S extends RegistrationResponse.Success> {
    ...
    private static final AtomicReferenceFieldUpdater<RegisteredRpcConnection, RetryingRegistration> REGISTRATION_UPDATER = AtomicReferenceFieldUpdater
            .newUpdater(RegisteredRpcConnection.class, RetryingRegistration.class, "pendingRegistration");
    ...
    public void start() {
         ...
         *  注释： 创建注册对象： ResourceManagerRegistration
         *  1、生成一个注册对象
         *  2、内部有关于回调的处理
         *  -
         *  1、当从节点 TaskExecutor 启动好了之后，的第一件事也是向 ResourceManager 注册
         *  2、同样， JobMaster 也是要干这个事儿
         *  3、他们两干这个 注册 事情的逻辑是一样的
         */
    ...
    private RetryingRegistration<F, G, S> createNewRegistration() {
         ...
         *  注释： 注册成功或者失败的回调
         */
        future.whenCompleteAsync((Tuple2<G, S> result, Throwable failure) -> {
            if(failure != null) {
