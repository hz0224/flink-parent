...
public class TaskManagerServices {
    ...
    TaskManagerServices(UnresolvedTaskManagerLocation unresolvedTaskManagerLocation, long managedMemorySize,
    ...
    public static TaskManagerServices fromConfiguration(TaskManagerServicesConfiguration taskManagerServicesConfiguration,
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 初始化 IOManager
         *  内部启动一定数量的 ReaderThread 和 WriterThread 来负责读写
         */
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第一个非常重要的组件
         *  启动的 ShuffleEnvironment 专门负责该 TaskManager 上的 数据传输相关
         *  在创建 ShuffleEnvironment 的时候，内部会创建，NettyConnectionManager
         *  在创建 NettyConnectionManager 的时候，内部会创建： NettyClient 和 NettyServer
         *  负责该节点上的 Task 的数据 Shuffle 传输
         *  -
         *  负责上游 Task 节点和 下游节点之间的 数据的传输！
         */
        ...
        final UnresolvedTaskManagerLocation unresolvedTaskManagerLocation = new UnresolvedTaskManagerLocation(
                ...
                taskManagerServicesConfiguration.getExternalDataPort() > 0 ? taskManagerServicesConfiguration
                        .getExternalDataPort() : listeningDataPort);
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第二个非常重要的组件
         *  TaskExecutor 的内部，最重要的一个成员变量就是： TaskSlotTable = TaskSlot_Table
         *  -
         *  补充一个关于 slot 管理的知识：
         *  1、主节点 ResourceManager 中有一个组件负责 Slot 的管理： SlotManager 具体实现是 SlotManagerImpl
         *  2、从节点 TaskExecutor 中有一个组件负责 Slot 管理： TaskSlotTable
         *  3、主控程序 JobMaster 中也有一个组件专门负责 Slot 管理：  SlotPool（申请slot: requestSlot()）
         *  -
         *  当一个 Job 启动，首选启动 JobMaster， 然后 JobMaster 向 ResourceManager 申请资源。
         *  然后 ResourceManager 做逻辑规划，然后转交分配请求给 TaskExecutor。 TaskExecutor 完成真正的 Slot 的分配
         *  然后告知 ResourceManager 和 JobMaster
         */
        ...
        return new TaskManagerServices(unresolvedTaskManagerLocation,
                taskManagerServicesConfiguration.getManagedMemorySize().getBytes(), ioManager, shuffleEnvironment,
                kvStateService, broadcastVariableManager, taskSlotTable, jobTable, jobLeaderService, taskStateManager,
    ...
    private static TaskSlotTable<Task> createTaskSlotTable(final int numberOfSlots,
            final TaskExecutorResourceSpec taskExecutorResourceSpec, final long timerServiceShutdownTimeout,
            final int pageSize, final Executor memoryVerificationExecutor) {

        // TODO_MA 注释： timerService 初始化
        final TimerService<AllocationID> timerService = new TimerService<>(new ScheduledThreadPoolExecutor(1),
                timerServiceShutdownTimeout);

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 TaskSlotTable 实例，具体实现是： TaskSlotTableImpl
         */
    ...
    private static ShuffleEnvironment<?, ?> createShuffleEnvironment(
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构造得到一个用于创建 NettyShuffleEnviroment 的上下文对象
         */
        final ShuffleEnvironmentContext shuffleEnvironmentContext = new ShuffleEnvironmentContext(
                taskManagerServicesConfiguration.getConfiguration(), taskManagerServicesConfiguration.getResourceID(),
                taskManagerServicesConfiguration.getNetworkMemorySize(),
                taskManagerServicesConfiguration.isLocalCommunicationOnly(),
                taskManagerServicesConfiguration.getBindAddress(), taskEventDispatcher, taskManagerMetricGroup,
                ioExecutor);

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建得到一个 NettyShuffleEnviroment 实例
         */
    ...
    /**
     * // TODO_MA 注释： 验证由字符串表示的所有目录是否确实存在或可以*创建，为正确的目录（而非文件）并且可写。
    ...
    private static void checkTempDirs(String[] tmpDirs) throws IOException {
        for(String dir : tmpDirs) {
            if(dir != null && !dir.equals("")) {
                ...
                if(!file.exists()) {
                    if(!file.mkdirs()) {
                        throw new IOException("Temporary file directory " + file
                                .getAbsolutePath() + " does not exist and could not be created.");
                    }
                }
                if(!file.isDirectory()) {
