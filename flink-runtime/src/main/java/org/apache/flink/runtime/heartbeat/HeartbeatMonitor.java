...
/**
 * // TODO_MA 注释： 心跳监视器，用于管理关联心跳目标的心跳状态。 *监视器在*指定的心跳超时间隔中未看到心跳信号时，
 * // TODO_MA 注释： 会通知{@link HeartbeatListener}。每个心跳信号都会重置此计时器。
 * Heartbeat monitor which manages the heartbeat state of the associated heartbeat target.
 *
 * The monitor notifies the {@link HeartbeatListener} whenever it has not seen a heartbeat signal in the
 * specified heartbeat timeout interval.
 *
...
public interface HeartbeatMonitor<O> {
    ...
    interface Factory<O> {
         ...
         */
        HeartbeatMonitor<O> createHeartbeatMonitor(ResourceID resourceID, HeartbeatTarget<O> heartbeatTarget, ScheduledExecutor mainThreadExecutor,
