...
public class RestClient implements AutoCloseableAsync {
     ...
     *  注释： 构造方法的内部，主要是启动一个 Netty Client　引导程序
     */
    ...
    public <M extends MessageHeaders<R, P, U>, U extends MessageParameters, R extends RequestBody, P extends ResponseBody> CompletableFuture<P> sendRequest(
            String targetAddress, int targetPort, M messageHeaders, U messageParameters, R request, Collection<FileUpload> fileUploads) throws IOException {
    ...
    public <M extends MessageHeaders<R, P, U>, U extends MessageParameters, R extends RequestBody, P extends ResponseBody> CompletableFuture<P> sendRequest(
            String targetAddress, int targetPort, M messageHeaders, U messageParameters, R request, Collection<FileUpload> fileUploads,
        ...
        if(!messageHeaders.getSupportedAPIVersions().contains(apiVersion)) {
            throw new IllegalArgumentException(
                    String.format("The requested version %s is not supported by the request (method=%s URL=%s). Supported versions are: %s.", apiVersion,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 处理得到 URL，然后好决定到底使用 WebMonitorEndpoint 中的哪个 Handler 来执行处理
         *  这个地方，其实是在拼接 url，根据URL 就能决定到底调用哪个 Hnadler 来执行处理
         *  提交 Job:  JobSubmitHandler 执行处理
         *  domain/v1/jobs/run
         *  domain/v1/jobs/list
         *  -
         *  之前讲述 WebMonitorEndpoint 的时候，已经讲过： 初始化一个 Router 对象，然后初始化一大堆 Hnalder
         *  然后注册到 Router 中， 到底是如何完成注册的呢？
         *  其实key就是 url, value 就是 handler
         *  -
         *  提交 job 的请求，在 服务端是由  JobSubmitHandler 来处理的
         */
        String versionedHandlerURL = "/" + apiVersion.getURLVersionPrefix() + messageHeaders.getTargetRestEndpointURL();
        // TODO_MA 注释： 处理 url 和 参数，进行拼接得到完整的一个 url
        // TODO_MA 注释： http://host:port/url?a=1&b=2
        ...
        // TODO_MA 注释： 生成一个 请求对象： http Request
        Request httpRequest = createRequest(targetAddress + ':' + targetPort, targetUrl, messageHeaders.getHttpMethod().getNettyHttpMethod(), payload,
                fileUploads);
    ...
    private <P extends ResponseBody> CompletableFuture<P> submitRequest(String targetAddress, int targetPort, Request httpRequest, JavaType responseType) {
         ...
         *  bootstrap 是  netty 客户端的 引导程序！
         *  主节点启动的时候，启动了 WebMonitorEndnpint 这个组件，这个组件在启动的时候，启动了 Netty 的服务端
         *  然后客户端提交 Job 的时候，其实是通过 RestClient 提交的。 在初始化 RestClient 的时候就初始化了  Netty
         *  的客户端。 如果调用： submitRequest(...) 方法，就会执行 请求的提交：
         *  netty客户端链接 netty 服务端，发送请求！ 其实就是 将  Request 请求对象的数据，写入到服务端
         */
        ...
        return channelFuture.thenComposeAsync(channel -> {
            ...
            try {
                if(handler == null) {
                    ...
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 客户端到此为止，已经把数据发送给服务端了 。
                     *  -
                     *  在服务端，WebMonitorEndpoint 中的 netty 服务端中的 router 会根据 url 找到对应的 Hnalder
                     *  调用该 Handler 的 handleRequest 方法来完成处理
                     */
                }
            } catch(IOException e) {
            ...
            return future;
        }, executor).thenComposeAsync((JsonResponse rawResponse) ->

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 解析提交 job 请求的响应
                 */
                parseResponse(rawResponse, responseType), executor);
    }
        ...
        protected void channelRead0(ChannelHandlerContext ctx, Object msg) {
            if(msg instanceof HttpResponse && ((HttpResponse) msg).status().equals(REQUEST_ENTITY_TOO_LARGE)) {
                jsonFuture.completeExceptionally(
                        new RestClientException(String.format(REQUEST_ENTITY_TOO_LARGE + ". Try to raise [%s]", RestOptions.CLIENT_MAX_CONTENT_LENGTH.key()),
