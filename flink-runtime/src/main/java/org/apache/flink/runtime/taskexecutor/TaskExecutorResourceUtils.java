...
public class TaskExecutorResourceUtils {
    ...
    static final List<ConfigOption<?>> CONFIG_OPTIONS = Arrays
            .asList(TaskManagerOptions.CPU_CORES, TaskManagerOptions.TASK_HEAP_MEMORY,
                    TaskManagerOptions.TASK_OFF_HEAP_MEMORY, TaskManagerOptions.NETWORK_MEMORY_MIN,
                    TaskManagerOptions.NETWORK_MEMORY_MAX, TaskManagerOptions.MANAGED_MEMORY_SIZE);

    private static final List<ConfigOption<?>> UNUSED_CONFIG_OPTIONS = Arrays
            .asList(TaskManagerOptions.TOTAL_PROCESS_MEMORY, TaskManagerOptions.TOTAL_FLINK_MEMORY,
                    TaskManagerOptions.FRAMEWORK_HEAP_MEMORY, TaskManagerOptions.FRAMEWORK_OFF_HEAP_MEMORY,
                    TaskManagerOptions.JVM_METASPACE, TaskManagerOptions.JVM_OVERHEAD_MIN,
    ...
    static TaskExecutorResourceSpec resourceSpecFromConfig(Configuration config) {
        ...
        return new TaskExecutorResourceSpec(new CPUResource(config.getDouble(TaskManagerOptions.CPU_CORES)),
                config.get(TaskManagerOptions.TASK_HEAP_MEMORY), config.get(TaskManagerOptions.TASK_OFF_HEAP_MEMORY),
                config.get(TaskManagerOptions.NETWORK_MEMORY_MIN), config.get(TaskManagerOptions.MANAGED_MEMORY_SIZE));
    }
    ...
    static ResourceProfile generateDefaultSlotResourceProfile(TaskExecutorResourceSpec taskExecutorResourceSpec,
            int numberOfSlots) {
         ...
         *  注释：
         */
        return ResourceProfile.newBuilder().setCpuCores(taskExecutorResourceSpec.getCpuCores().divide(numberOfSlots))
                ...
                .setNetworkMemory(taskExecutorResourceSpec.getNetworkMemSize().divide(numberOfSlots)).build();
    }
    ...
    static ResourceProfile generateTotalAvailableResourceProfile(TaskExecutorResourceSpec taskExecutorResourceSpec) {
         ...
         *  注释：
         */
        return ResourceProfile.newBuilder().setCpuCores(taskExecutorResourceSpec.getCpuCores())
                ...
                .setNetworkMemory(taskExecutorResourceSpec.getNetworkMemSize()).build();
    }
    ...
    private static void adjustNetworkMemoryForLocalExecution(Configuration config) {
        if(!config.contains(TaskManagerOptions.NETWORK_MEMORY_MIN) && config
                .contains(TaskManagerOptions.NETWORK_MEMORY_MAX)) {
            config.set(TaskManagerOptions.NETWORK_MEMORY_MIN, config.get(TaskManagerOptions.NETWORK_MEMORY_MAX));
        }
        if(!config.contains(TaskManagerOptions.NETWORK_MEMORY_MAX) && config
                .contains(TaskManagerOptions.NETWORK_MEMORY_MIN)) {
    ...
    private static <T> void setConfigOptionToDefaultIfNotSet(Configuration config, ConfigOption<T> option,
            T defaultValue) {
