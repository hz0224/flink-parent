...
/**
 * // TODO_MA 注释： RPC端点的基类。提供远程过程调用的分布式组件必须扩展 RPCEndpoint 基类。 RPC端点由{@link RpcService}支持
 ...
 *
 * // TODO_MA 注释： 同一endpoint上的所有RPC调用均由同一线程（称为端点的<i>主线程</ i>）调用。
 * // TODO_MA 注释： 因此，通过在主线程中执行所有状态更改操作，我们不必以 Erlang 或 Akka 的 Actor 模型中的相同方式推理并发访问。
 ...
 * Erlang or Akka.
 *
 * // TODO_MA 注释： RPC端点提供{@link #runAsync（Runnable）}，{@ link #callAsync（Callable，Time）}和
 * // TODO_MA 注释： {@link #getMainThreadExecutor（）}，以在RPC端点的主线程中执行代码。
 ...
 *
 * <ul>
 *     // TODO_MA 注释： RPC endpoint是在非运行状态下创建的，不为任何RPC请求提供服务。
 *     <li>The RPC endpoint is created in a non-running state and does not serve any RPC requests.
 *
 *     // TODO_MA 注释： 要点 要点 要点
 *     // TODO_MA 注释： 调用{@link #start（）}方法将触发RPC endpoint的启动，并将可重写的{@link #onStart（）}方法调用调度到主线程。
 ...
 *
 *     // TODO_MA 注释： 当开始操作结束时，RPC endpoint 将移至运行状态，并开始服务并完成RPC请求。
 ...
 *
 *     // TODO_MA 注释： 调用{@link #closeAsync（）}方法会触发RPC终结点的终止，
 *     // TODO_MA 注释： 并且将可覆盖的{@link #onStop（）}方法调用调度到主线程。
 ...
 *
 *     // TODO_MA 注释： 调用{@link #onStop（）}方法时，它将触发异步停止操作。
 *     // TODO_MA 注释： RPC端点不再处于运行状态，但它将继续为RPC请求提供服务。
 ...
 *
 *     // TODO_MA 注释： 当异步停止操作结束时，RPC端点将完全终止，并且不再为RPC请求提供服务。
 ...
 * </ul>
 *
 * // TODO_MA 注释： 可以通过调用{@link #isRunning（）}方法在RPC方法处理程序或主线程中查询运行状态。
