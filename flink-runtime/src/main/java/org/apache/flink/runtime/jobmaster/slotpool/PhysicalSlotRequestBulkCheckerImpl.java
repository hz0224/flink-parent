...
public class PhysicalSlotRequestBulkCheckerImpl implements PhysicalSlotRequestBulkChecker {
    ...
    private ComponentMainThreadExecutor componentMainThreadExecutor = new DummyComponentMainThreadExecutor(
            "PhysicalSlotRequestBulkCheckerImpl is not initialized with proper main thread executor, " + "call to PhysicalSlotRequestBulkCheckerImpl#start is required");
    ...
    private void schedulePendingRequestBulkWithTimestampCheck(final PhysicalSlotRequestBulkWithTimestamp bulk,
            final Time timeout) {
     ...
     */
    @VisibleForTesting
    TimeoutCheckResult checkPhysicalSlotRequestBulkTimeout(final PhysicalSlotRequestBulkWithTimestamp slotRequestBulk,
     ...
     */
    @VisibleForTesting
    static boolean isSlotRequestBulkFulfillable(final PhysicalSlotRequestBulk slotRequestBulk,
    ...
    private static Set<SlotInfo> getReusableSlots(final Supplier<Set<SlotInfo>> slotsRetriever,
            final Set<AllocationID> slotsToExclude) {

        return slotsRetriever.get().stream().filter(slotInfo -> !slotInfo.willBeOccupiedIndefinitely())
                .filter(slotInfo -> !slotsToExclude.contains(slotInfo.getAllocationId())).collect(Collectors.toSet());
    }
     ...
     * number of all slots).
     */
    private static boolean areRequestsFulfillableWithSlots(final Collection<ResourceProfile> requestResourceProfiles,
    ...
    private static Optional<SlotInfo> findMatchingSlotForRequest(final ResourceProfile requestResourceProfile,
    ...
    private static Set<SlotInfo> getAllSlotInfos(SlotPool slotPool) {
        return Stream.concat(slotPool.getAvailableSlotsInformation().stream(),
                slotPool.getAllocatedSlotsInformation().stream()).collect(Collectors.toSet());
    }
