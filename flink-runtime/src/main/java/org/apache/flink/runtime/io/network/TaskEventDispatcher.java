...
/**
 * // TODO_MA 注释： 任务事件分配器将事件从消耗任务向后分配到产生消耗结果的任务。
 * The task event dispatcher dispatches events flowing backwards from a consuming task to the task producing the
 * consumed result.
 *
 * // TODO_MA 注释： 向后事件仅适用于产生流水线结果的任务，其中和消费任务同时运行。
...
public class TaskEventDispatcher implements TaskEventPublisher {
     ...
     */
    public void subscribeToEvent(ResultPartitionID partitionId, EventListener<TaskEvent> eventListener,
