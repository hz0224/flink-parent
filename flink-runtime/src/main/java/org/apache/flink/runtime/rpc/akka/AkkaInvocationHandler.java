...
class AkkaInvocationHandler implements InvocationHandler, AkkaBasedEndpoint, RpcServer {
    ...
    public void start() {
        ...
        // TODO_MA 注释： 发送一个启动消息 给自己
        // TODO_MA 注释： 然后 AkkaRpcActor 组件中的 Receive 会接收到这个消息，然后执行消息匹配，调用对应的方法执行处理
        // TODO_MA 注释： 详细情况见： AkkaRpcActor.createReceive() 方法
        rpcEndpoint.tell(ControlMessages.START, ActorRef.noSender());

        // TODO_MA 注释： rpcEndpoint 内部包装了一个 AkkaRpcActor
        // TODO_MA 注释： 如果这个 rpcEndpoint 接收到 START 控制消息
        // TODO_MA 注释： 则这个 rpcEndpoint 的 onStart() 方法
    }
