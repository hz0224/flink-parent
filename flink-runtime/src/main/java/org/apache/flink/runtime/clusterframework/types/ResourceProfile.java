...
/**
 * // TODO_MA 注释： 在需要或提供 slot 时，描述 slot 的不可变资源配置文件。
...
public class ResourceProfile implements Serializable {
    ...
    public static final ResourceProfile ANY = newBuilder()
            ...
            // TODO_MA 注释： task heap memory 大小
            .setTaskHeapMemory(MemorySize.MAX_VALUE)
            // TODO_MA 注释： task off-heap memory大小
            .setTaskOffHeapMemory(MemorySize.MAX_VALUE)
            // TODO_MA 注释： managed memory大小
            .setManagedMemory(MemorySize.MAX_VALUE)
            // TODO_MA 注释： network memory大小
            .setNetworkMemory(MemorySize.MAX_VALUE).build();
     ...
     */
    private ResourceProfile(final Resource cpuCores, final MemorySize taskHeapMemory, final MemorySize taskOffHeapMemory,
            final MemorySize managedMemory, final MemorySize networkMemory,
     ...
     * // TODO_MA 注释： 在你的要的 slot 和 集群中有的 slot 中做对比。
     * // TODO_MA 注释： 必须集群有一个 slot 的各项配置，都大于 你要申请的slot 的配置
     *
    ...
    public boolean isMatching(final ResourceProfile required) {
         ...
         *  注释： 检查需要申请的资源，是否都是大于该 slot 能提供的，如果是，则满足要求
         */
        if(cpuCores.getValue().compareTo(required.cpuCores.getValue()) >= 0
            ...
            for(Map.Entry<String, Resource> resource : required.extendedResources.entrySet()) {
                if(!extendedResources.containsKey(resource.getKey()) || extendedResources.get(resource.getKey())
                        .getValue().compareTo(resource.getValue().getValue()) < 0) {
                    return false;
                }
            }
            return true;
    ...
    public boolean equals(Object obj) {
        if(obj == this) {
            ...
            return Objects.equals(this.cpuCores, that.cpuCores) && Objects
                    .equals(taskHeapMemory, that.taskHeapMemory) && Objects
                    .equals(taskOffHeapMemory, that.taskOffHeapMemory) && Objects
                    .equals(managedMemory, that.managedMemory) && Objects
                    .equals(networkMemory, that.networkMemory) && Objects
                    .equals(extendedResources, that.extendedResources);
        }
        return false;
    }
    ...
    public ResourceProfile merge(final ResourceProfile other) {
        ...
        return new ResourceProfile(cpuCores.merge(other.cpuCores), taskHeapMemory.add(other.taskHeapMemory),
                taskOffHeapMemory.add(other.taskOffHeapMemory), managedMemory.add(other.managedMemory),
                networkMemory.add(other.networkMemory), resultExtendedResource);
    }
    ...
    public ResourceProfile subtract(final ResourceProfile other) {
        ...
        return new ResourceProfile(cpuCores.subtract(other.cpuCores), taskHeapMemory.subtract(other.taskHeapMemory),
                taskOffHeapMemory.subtract(other.taskOffHeapMemory), managedMemory.subtract(other.managedMemory),
                networkMemory.subtract(other.networkMemory), resultExtendedResource);
    }
    ...
    public ResourceProfile multiply(final int multiplier) {
        ...
        return new ResourceProfile(cpuCores.multiply(multiplier), taskHeapMemory.multiply(multiplier),
                taskOffHeapMemory.multiply(multiplier), managedMemory.multiply(multiplier),
                networkMemory.multiply(multiplier), resultExtendedResource);
    }
    ...
    private String getResourceString() {
        String resourceStr = cpuCores == null || cpuCores.getValue()
                .compareTo(MAX_CPU_CORE_NUMBER_TO_LOG) > 0 ? "" : "cpuCores=" + cpuCores.getValue();
    ...
    private static String addMemorySizeString(String resourceStr, String name, MemorySize size) {
        ...
        String memorySizeStr = size == null || size.compareTo(MAX_MEMORY_SIZE_TO_LOG) > 0 ? "" : comma + name + '=' + size
    ...
    public static ResourceProfile fromResourceSpec(ResourceSpec resourceSpec, MemorySize networkMemory) {
        ...
        return newBuilder().setCpuCores(resourceSpec.getCpuCores()).setTaskHeapMemory(resourceSpec.getTaskHeapMemory())
                .setTaskOffHeapMemory(resourceSpec.getTaskOffHeapMemory())
                .setManagedMemory(resourceSpec.getManagedMemory()).setNetworkMemory(networkMemory)
                .addExtendedResources(resourceSpec.getExtendedResources()).build();
    }
    ...
    public static class Builder {
        ...
        public ResourceProfile build() {
             ...
             *  注释：
             */
            return new ResourceProfile(cpuCores, taskHeapMemory, taskOffHeapMemory, managedMemory, networkMemory,
                    extendedResources);
        }
    }
}
