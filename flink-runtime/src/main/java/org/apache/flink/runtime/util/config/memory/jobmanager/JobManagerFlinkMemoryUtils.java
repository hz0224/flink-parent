...
public class JobManagerFlinkMemoryUtils implements FlinkMemoryUtils<JobManagerFlinkMemory> {
    ...
    public JobManagerFlinkMemory deriveFromRequiredFineGrainedOptions(Configuration config) {
        MemorySize jvmHeapMemorySize = ProcessMemoryUtils
                .getMemorySizeFromConfig(config, JobManagerOptions.JVM_HEAP_MEMORY);
        MemorySize offHeapMemorySize = ProcessMemoryUtils
                .getMemorySizeFromConfig(config, JobManagerOptions.OFF_HEAP_MEMORY);
        ...
        if(config.contains(JobManagerOptions.TOTAL_FLINK_MEMORY)) {
            ...
            MemorySize totalFlinkMemorySize = ProcessMemoryUtils
                    .getMemorySizeFromConfig(config, JobManagerOptions.TOTAL_FLINK_MEMORY);
    ...
    private static void sanityCheckTotalFlinkMemory(MemorySize totalFlinkMemorySize, MemorySize jvmHeapMemorySize,
        ...
        if(derivedTotalFlinkMemorySize.getBytes() != totalFlinkMemorySize.getBytes()) {
            throw new IllegalConfigurationException(String.format(
                    "Sum of the configured JVM Heap Memory (%s) and the configured Off-heap Memory (%s) " + "does not match the configured Total Flink Memory (%s). Please, make the configuration consistent " + "or configure only one option: either JVM Heap or Total Flink Memory.",
                    jvmHeapMemorySize.toHumanReadableString(), offHeapMemorySize.toHumanReadableString(),
                    totalFlinkMemorySize.toHumanReadableString()));
        }
    }

    private static MemorySize deriveOffHeapMemory(MemorySize jvmHeapMemorySize, MemorySize totalFlinkMemorySize,
            MemorySize defaultOffHeapMemorySize) {
        if(totalFlinkMemorySize.getBytes() < jvmHeapMemorySize.getBytes()) {
            throw new IllegalConfigurationException(String.format(
                    "The configured JVM Heap Memory (%s) exceeds the configured Total Flink Memory (%s). " + "Please, make the configuration consistent or configure only one option: either JVM Heap " + "or Total Flink Memory.",
                    jvmHeapMemorySize.toHumanReadableString(), totalFlinkMemorySize.toHumanReadableString()));
        }
        ...
        if(offHeapMemorySize.getBytes() != defaultOffHeapMemorySize.getBytes()) {
            LOG.info(
                    "The Off-Heap Memory size ({}) is derived the configured Total Flink Memory size ({}) minus " + "the configured JVM Heap Memory size ({}). The default Off-Heap Memory size ({}) is ignored.",
                    offHeapMemorySize.toHumanReadableString(), totalFlinkMemorySize.toHumanReadableString(),
                    jvmHeapMemorySize.toHumanReadableString(), defaultOffHeapMemorySize.toHumanReadableString());
        }
    ...
    public JobManagerFlinkMemory deriveFromTotalFlinkMemory(Configuration config, MemorySize totalFlinkMemorySize) {
        ...
        if(totalFlinkMemorySize.compareTo(offHeapMemorySize) < 1) {
            throw new IllegalConfigurationException(
                    ...
                    totalFlinkMemorySize.toHumanReadableString(), offHeapMemorySize.toHumanReadableString());
        }
    ...
    private static void verifyJvmHeapSize(MemorySize jvmHeapSize) {
        if(jvmHeapSize.compareTo(JobManagerOptions.MIN_JVM_HEAP_SIZE) < 0) {
            LOG.warn(
                    ...
                    jvmHeapSize.toHumanReadableString(), JobManagerOptions.MIN_JVM_HEAP_SIZE.toHumanReadableString());
        }
    }
}
