...
public final class DispatcherJob implements AutoCloseableAsync {
    ...
    private enum DispatcherJobStatus {
        ...
        INITIALIZING(JobStatus.INITIALIZING), // JobManagerRunner is initialized
        JOB_MANAGER_RUNNER_INITIALIZED(
                null), // waiting for cancellation. We stay in this status until the job result future completed,
    ...
    static DispatcherJob createFor(CompletableFuture<JobManagerRunner> jobManagerRunnerFuture, JobID jobId,
            String jobName, long initializationTimestamp) {
    ...
    private DispatcherJob(CompletableFuture<JobManagerRunner> jobManagerRunnerFuture, JobID jobId, String jobName,
                ...
                if(throwable == null) { // initialization succeeded
                    ...
                    jobManagerRunner.getResultFuture().whenComplete((archivedExecutionGraph, resultThrowable) -> {
                        if(archivedExecutionGraph != null) {
                            ...
                            jobResultFuture
                                    .completeExceptionally(ExceptionUtils.stripCompletionException(resultThrowable));
                        }
                    });
                } else { // failure during initialization
                    final Throwable strippedThrowable = ExceptionUtils.stripCompletionException(throwable);
                    ArchivedExecutionGraph archivedExecutionGraph = ArchivedExecutionGraph
                            .createFromInitializingJob(jobId, jobName, JobStatus.FAILED, strippedThrowable,
                                    initializationTimestamp);
                    jobResultFuture.complete(
                            DispatcherJobResult.forInitializationFailure(archivedExecutionGraph, strippedThrowable));
                }
            }
            return null;
        }));
        ...
        synchronized(lock) {
            if(isInitialized()) {
                ...
                Preconditions.checkState(
                        this.jobStatus == DispatcherJobStatus.INITIALIZING || jobStatus == DispatcherJobStatus.CANCELLING);
                return CompletableFuture.completedFuture(ArchivedExecutionGraph
                        .createFromInitializingJob(jobId, jobName, jobStatus.asJobStatus(), null,
                                initializationTimestamp));
            }
        }
    }
    ...
    public CompletableFuture<Void> closeAsync() {
        FutureUtils.assertNoException(jobManagerRunnerFuture.handle((runner, throwable) -> {
            if(throwable == null) {
                ...
                CompletableFuture<Void> jobManagerRunnerClose = jobManagerRunnerFuture
