...
public class TaskStateManagerImpl implements TaskStateManager {
    ...
    public TaskStateManagerImpl(@Nonnull JobID jobId, @Nonnull ExecutionAttemptID executionAttemptID,
            @Nonnull TaskLocalStateStore localStateStore, @Nullable JobManagerTaskRestore jobManagerTaskRestore,
            @Nonnull CheckpointResponder checkpointResponder) {

        // TODO_MA 注释： 创建一个 TaskStateManagerImpl
        this(jobId, executionAttemptID, localStateStore, jobManagerTaskRestore, checkpointResponder,

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： SequentialChannelStateReade 的具体实现是： SequentialChannelStateReaderImpl
                 */
                new SequentialChannelStateReaderImpl(
                        jobManagerTaskRestore == null ? new TaskStateSnapshot() : jobManagerTaskRestore
    ...
    public void reportTaskStateSnapshots(@Nonnull CheckpointMetaData checkpointMetaData,
         ...
         */
        checkpointResponder
                .acknowledgeCheckpoint(jobId, executionAttemptID, checkpointId, checkpointMetrics, acknowledgedState);
    }
    ...
    public PrioritizedOperatorSubtaskState prioritizedOperatorState(OperatorID operatorID) {
        ...
        LOG.debug(
                "Operator {} has remote state {} from job manager and local state alternatives {} from local " + "state store {}.",
