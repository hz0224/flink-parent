...
public abstract class NettyMessage {
     ...
     * NettyMessageEncoder}
     */
    private static ByteBuf allocateBuffer(ByteBufAllocator allocator, byte id, int messageHeaderLength,
            int contentLength, boolean allocateForContent) {
        ...
        buffer.writeInt(
                FRAME_HEADER_LENGTH + messageHeaderLength + contentLength); // may be updated later, e.g. if contentLength == -1
    ...
    static class BufferResponse extends NettyMessage {
        ...
        private BufferResponse(@Nullable Buffer buffer, Buffer.DataType dataType, boolean isCompressed,
                int sequenceNumber, InputChannelID receiverId, int backlog, int bufferSize) {
            this.buffer = buffer;
    ...
    static class PartitionRequest extends NettyMessage {
        ...
        // TODO_MA 注释： ResultPartitionID
        final ResultPartitionID partitionId;

        // TODO_MA 注释：
        final int queueIndex;

        // TODO_MA 注释： InputChannel ID
        final InputChannelID receiverId;
    ...
    void writeToChannel(ChannelOutboundInvoker out, ChannelPromise promise, ByteBufAllocator allocator,
            Consumer<ByteBuf> consumer, byte id, int length) throws IOException {
