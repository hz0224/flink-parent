...
public class TaskDeploymentDescriptorFactory {
    ...
    private TaskDeploymentDescriptorFactory(ExecutionAttemptID executionId, int attemptNumber,
            MaybeOffloaded<JobInformation> serializedJobInformation, MaybeOffloaded<TaskInformation> taskInfo,
            JobID jobID, boolean allowUnknownPartitions, int subtaskIndex, ExecutionEdge[][] inputEdges) {
    ...
    public TaskDeploymentDescriptor createDeploymentDescriptor(AllocationID allocationID, int targetSlotNumber,
         ...
         *  注释：
         */
        return new TaskDeploymentDescriptor(jobID, serializedJobInformation, taskInfo, executionId, allocationID,
                subtaskIndex, attemptNumber, targetSlotNumber, taskRestore, new ArrayList<>(producedPartitions),

                // TODO_MA 注释： InputGateDeploymentDescriptor
    ...
    private List<InputGateDeploymentDescriptor> createInputGateDeploymentDescriptors() {
        ...
        for(ExecutionEdge[] edges : inputEdges) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 每一个 ExecutionEdge 将来都会变成: InputChannel
             */
            inputGates.add(new InputGateDeploymentDescriptor(resultId, partitionType, queueToRequest,
    ...
    private ShuffleDescriptor[] getConsumedPartitionShuffleDescriptors(ExecutionEdge[] edges) {
        ...
        for(int i = 0; i < edges.length; i++) {
            shuffleDescriptors[i] = getConsumedPartitionShuffleDescriptor(edges[i], allowUnknownPartitions);
        }
    ...
    public static TaskDeploymentDescriptorFactory fromExecutionVertex(ExecutionVertex executionVertex,
        ...
        // TODO_MA 注释： 获取 ExecutionGraph
        ExecutionGraph executionGraph = executionVertex.getExecutionGraph();

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： TaskDeploymentDescriptorFactory
         */
        return new TaskDeploymentDescriptorFactory(executionVertex.getCurrentExecutionAttempt().getAttemptId(),
                attemptNumber, getSerializedJobInformation(executionGraph),
                getSerializedTaskInformation(executionVertex.getJobVertex().getTaskInformationOrBlobKey()),
                executionGraph.getJobID(), executionGraph.getScheduleMode().allowLazyDeployment(),
                executionVertex.getParallelSubtaskIndex(), executionVertex.getAllInputEdges());
    }

    private static MaybeOffloaded<JobInformation> getSerializedJobInformation(ExecutionGraph executionGraph) {
        Either<SerializedValue<JobInformation>, PermanentBlobKey> jobInformationOrBlobKey = executionGraph
    ...
    private static MaybeOffloaded<TaskInformation> getSerializedTaskInformation(
        ...
        return taskInfo.isLeft() ? new TaskDeploymentDescriptor.NonOffloaded<>(
                taskInfo.left()) : new TaskDeploymentDescriptor.Offloaded<>(taskInfo.right());
    }
    ...
    public static ShuffleDescriptor getConsumedPartitionShuffleDescriptor(ExecutionEdge edge,
        ...
        Optional<ResultPartitionDeploymentDescriptor> consumedPartitionDescriptor = producer
                .getResultPartitionDeploymentDescriptor(consumedPartition.getPartitionId());
         ...
         *  注释：
         */
        return getConsumedPartitionShuffleDescriptor(consumedPartitionId, consumedPartition.getResultType(),
                consumedPartition.isConsumable(), producerState, allowUnknownPartitions,
    ...
    static ShuffleDescriptor getConsumedPartitionShuffleDescriptor(ResultPartitionID consumedPartitionId,
            ResultPartitionType resultPartitionType, boolean isConsumable, ExecutionState producerState,
            boolean allowUnknownPartitions, @Nullable ResultPartitionDeploymentDescriptor consumedPartitionDescriptor) {
        // The producing task needs to be RUNNING or already FINISHED
        if((resultPartitionType
                .isPipelined() || isConsumable) && consumedPartitionDescriptor != null && isProducerAvailable(
                producerState)) {
            ...
            throw handleConsumedPartitionShuffleDescriptorErrors(consumedPartitionId, resultPartitionType, isConsumable,
                    producerState);
        }
    }
    ...
    private static RuntimeException handleConsumedPartitionShuffleDescriptorErrors(ResultPartitionID consumedPartitionId,
        ...
        if(isProducerFailedOrCanceled(producerState)) {
            msg = "Trying to consume an input partition whose producer has been canceled or failed. " + "The producer is in state " + producerState + ".";
        } else {
            msg = String
                    .format("Trying to consume an input partition whose producer " + "is not ready (result type: %s, partition consumable: %s, producer state: %s, partition id: %s).",
    ...
    private static boolean isProducerAvailable(ExecutionState producerState) {
        return producerState == ExecutionState.RUNNING || producerState == ExecutionState.FINISHED || producerState == ExecutionState.SCHEDULED || producerState == ExecutionState.DEPLOYING;
    }
