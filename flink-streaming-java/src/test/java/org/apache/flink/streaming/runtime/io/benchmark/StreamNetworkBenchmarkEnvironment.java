...
public class StreamNetworkBenchmarkEnvironment<T extends IOReadableWritable> {
    ...
    public void setUp(int writers, int channels, boolean localMode, int senderBufferPoolSize,
            int receiverBufferPoolSize) throws Exception {
        setUp(writers, channels, localMode, senderBufferPoolSize, receiverBufferPoolSize, new Configuration());
    }
     ...
     */
    public void setUp(int writers, int channels, boolean localMode, int senderBufferPoolSize,
            int receiverBufferPoolSize, Configuration config) throws Exception {
        ...
        gateFactory = new SingleInputGateBenchmarkFactory(location, receiverEnv.getConfiguration(),
                receiverEnv.getConnectionManager(), receiverEnv.getResultPartitionManager(), new TaskEventDispatcher(),
    ...
    public SerializingLongReceiver createReceiver() throws Exception {
        TaskManagerLocation senderLocation = new TaskManagerLocation(ResourceID.generate(), LOCAL_ADDRESS, dataPort);
    ...
    public ResultPartitionWriter createResultPartitionWriter(int partitionIndex) throws Exception {
        ...
        ResultPartitionWriter resultPartitionWriter = new ResultPartitionBuilder()
                ...
                .setResultPartitionType(ResultPartitionType.PIPELINED_BOUNDED).setNumberOfSubpartitions(channels)
                .setResultPartitionManager(senderEnv.getResultPartitionManager())
                .setupBufferPoolFactoryFromNettyShuffleEnvironment(senderEnv).build();
    ...
    private NettyShuffleEnvironment createShuffleEnvironment(@SuppressWarnings("SameParameterValue") int bufferPoolSize,
            Configuration config) throws Exception {

        final NettyConfig nettyConfig = new NettyConfig(LOCAL_ADDRESS, 0, ConfigurationParserUtils.getPageSize(config),
                // please note that the number of slots directly influences the number of
                // netty threads!
                ConfigurationParserUtils.getSlot(config), config);
        return new NettyShuffleEnvironmentBuilder().setNumNetworkBuffers(bufferPoolSize).setNettyConfig(nettyConfig)
                .build();
    }
    ...
    private InputGate createInputGate(TaskManagerLocation senderLocation) throws Exception {
        ...
        for(int gateIndex = 0; gateIndex < gates.length; ++gateIndex) {
            final InputGateDeploymentDescriptor gateDescriptor = createInputGateDeploymentDescriptor(senderLocation,
                    gateIndex, location);
    ...
    private InputGateDeploymentDescriptor createInputGateDeploymentDescriptor(TaskManagerLocation senderLocation,
            int gateIndex, ResourceID localLocation) {

        final ShuffleDescriptor[] channelDescriptors = new ShuffleDescriptor[channels];
        for(int channelIndex = 0; channelIndex < channels; ++channelIndex) {
            channelDescriptors[channelIndex] = createShuffleDescriptor(localMode, partitionIds[gateIndex],
                    localLocation, senderLocation, channelIndex);
        }

        return new InputGateDeploymentDescriptor(dataSetID, ResultPartitionType.PIPELINED_BOUNDED,
                ...
                0, channelDescriptors);
    }

    private IndexedInputGate createInputGateWithMetrics(SingleInputGateFactory gateFactory,
            InputGateDeploymentDescriptor gateDescriptor, int gateIndex) {

        final SingleInputGate singleGate = gateFactory
                .create("receiving task[" + gateIndex + "]", gateIndex, gateDescriptor,
    ...
    private static ShuffleDescriptor createShuffleDescriptor(boolean localMode, ResultPartitionID resultPartitionID,
            ResourceID location, TaskManagerLocation senderLocation, int connectionIndex) {
        final NettyShuffleDescriptorBuilder builder = NettyShuffleDescriptorBuilder.newBuilder()
                .setId(resultPartitionID).setProducerInfoFromTaskManagerLocation(senderLocation)
