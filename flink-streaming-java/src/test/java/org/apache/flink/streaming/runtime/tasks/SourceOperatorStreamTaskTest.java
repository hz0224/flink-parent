...
public class SourceOperatorStreamTaskTest {
    ...
    public void testExternallyInducedSource() throws Exception {
        ...
        TestingExternallyInducedSourceReader testingReader = new TestingExternallyInducedSourceReader(
                numEventsBeforeCheckpoint, totalNumEvents);
        try(StreamTaskMailboxTestHarness<Integer> testHarness = createTestHarness(
                new TestingExternallyInducedSource(testingReader), 0, null)) {
            TestingExternallyInducedSourceReader runtimeTestingReader = (TestingExternallyInducedSourceReader) ((SourceOperator) testHarness
                    .getStreamTask().mainOperator).getSourceReader();
    ...
    private TaskStateSnapshot executeAndWaitForCheckpoint(long checkpointId, TaskStateSnapshot initialSnapshot,
    ...
    private void triggerCheckpointWaitForFinish(StreamTaskMailboxTestHarness<Integer> testHarness, long checkpointId,
        ...
        Future<Boolean> checkpointFuture = testHarness.getStreamTask()
    ...
    private StreamTaskMailboxTestHarness<Integer> createTestHarness(long checkpointId,
    ...
    private StreamTaskMailboxTestHarness<Integer> createTestHarness(MockSource source, long checkpointId,
            TaskStateSnapshot snapshot) throws Exception {
        // get a source operator.
        SourceOperatorFactory<Integer> sourceOperatorFactory = new SourceOperatorFactory<>(source,
                WatermarkStrategy.noWatermarks());

        // build a test harness.
        StreamTaskMailboxTestHarnessBuilder<Integer> builder = new StreamTaskMailboxTestHarnessBuilder<>(
        ...
        return builder.setupOutputForSingletonOperatorChain(sourceOperatorFactory, OPERATOR_ID).build();
    }
    ...
    private static class TestingExternallyInducedSourceReader implements ExternallyInducedSourceReader<Integer, MockSourceSplit>, Serializable {
