...
public class OneInputStreamTask<IN, OUT> extends StreamTask<OUT, OneInputStreamOperator<IN, OUT>> {
    ...
    public void init() throws Exception {
        ...
        if(numberOfInputs > 0) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 存在于 OneInputStreawmTask 真正完成逻辑处理的一个组件
             */
    ...
    private StreamTaskInput<IN> wrapWithSorted(StreamTaskInput<IN> input) {
        ...
        return new SortingDataInput<>(input,
                ...
                getEnvironment().getMemoryManager(), getEnvironment().getIOManager(),
                getExecutionConfig().isObjectReuseEnabled(), configuration
                .getManagedMemoryFractionOperatorUseCaseOfSlot(ManagedMemoryUseCase.BATCH_OP, getTaskConfiguration(),
                        userCodeClassLoader), getJobConfiguration(), this);
    }
    ...
    private CheckpointedInputGate createCheckpointedInputGate() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建一个 具备 checkpoint 功能的 CheckpointedInputGate
         */
        return InputProcessorUtil.createCheckpointedInputGate(this, configuration, getCheckpointCoordinator(), inputGates,
                getEnvironment().getMetricGroup().getIOMetricGroup(), getTaskNameWithSubtaskAndId(), mainMailboxExecutor);
    }
    ...
    private DataOutput<IN> createDataOutput(Counter numRecordsIn) {
         ...
         *  注释： 初始化输出
         */
        return new StreamTaskNetworkOutput<>(mainOperator, getStreamStatusMaintainer(), inputWatermarkGauge,
                numRecordsIn);
    }
    ...
    private StreamTaskInput<IN> createTaskInput(CheckpointedInputGate inputGate) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建得到： StreamTaskNetworkInput
         */
        return new StreamTaskNetworkInput<>(inputGate, inSerializer, getEnvironment().getIOManager(),
    ...
    private static class StreamTaskNetworkOutput<IN> extends AbstractDataOutput<IN> {
        ...
        private StreamTaskNetworkOutput(OneInputStreamOperator<IN, ?> operator,
                StreamStatusMaintainer streamStatusMaintainer, WatermarkGauge watermarkGauge, Counter numRecordsIn) {
