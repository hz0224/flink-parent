...
public class StreamGraph implements Pipeline {
    ...
    public <IN, OUT> void addSource(Integer vertexID, @Nullable String slotSharingGroup, @Nullable String coLocationGroup,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 往 StreamGraph 中加入一个 StreamOperator
         *  注意方法的最后一个 参数： SourceOperatorStreamTask， 它是 SourceTask 的启动类，启动入口
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将生成的 StreamNode 的 ID 加入到 sources 集合中
         */
    ...
    public <IN, OUT> void addOperator(Integer vertexID, @Nullable String slotSharingGroup, @Nullable String coLocationGroup,
        ...
        // TODO_MA 注释： 注意 invokableClass = OneInputStreamTask/SourceStreamTask
    ...
    private <IN, OUT> void addOperator(Integer vertexID, @Nullable String slotSharingGroup, @Nullable String coLocationGroup,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 一个 StreamOperator 对应一个 StreamNode
         *  所以 addOperator 方法的内部，对应着调用一次 addNode() 方法
         */
    ...
    public <IN1, IN2, OUT> void addCoOperator(Integer vertexID, String slotSharingGroup, @Nullable String coLocationGroup,
            StreamOperatorFactory<OUT> taskOperatorFactory, TypeInformation<IN1> in1TypeInfo, TypeInformation<IN2> in2TypeInfo,
            TypeInformation<OUT> outTypeInfo, String operatorName) {

        // TODO_MA 注释： 启动类： TwoInputStreamTask
    ...
    protected StreamNode addNode(Integer vertexID, @Nullable String slotSharingGroup, @Nullable String coLocationGroup,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 一个 StreamOperator 生成一个 StreamNode 节点
         */
        StreamNode vertex = new StreamNode(vertexID, slotSharingGroup, coLocationGroup, operatorFactory, operatorName, vertexClass);

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将该 StreamNode 加入到 StreamGraph 中
         *  编写程序写算子调用 ==> UserFunction ===> StreamOperator ===> Treansformation ===> StreamNode
         *  构建 StreamNode 的时候，会多做一件事： 指定 InvokableClass
         *  1、StreamSource:  source算子　　SourceStrewamTask
    ...
    public void addEdge(Integer upStreamVertexID, Integer downStreamVertexID, int typeNumber) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 拿到了上下游两个顶点的 ID，然后构建成 StreamEdge
         */
    ...
    private void addEdgeInternal(Integer upStreamVertexID, Integer downStreamVertexID, int typeNumber, StreamPartitioner<?> partitioner,
        ...
        // TODO_MA 注释： 先判断是不是虚拟节点上的边，如果是，并不直接构建 StreamEdge，而是找到虚拟节点上游对应的物理节点构建 StreamEdge
        // TODO_MA 注释： 在两个物理节点之间添加边，并把对应的 StreamPartitioner,或者 OutputTag 等补充信息添加到 StreamEdge 中
        if(virtualSideOutputNodes.containsKey(upStreamVertexID)) {
            ...
            // TODO_MA 注释： 找到 该 StreamNode 的上游顶点
            StreamNode upstreamNode = getStreamNode(upStreamVertexID);
            // TODO_MA 注释： 找到 该 StreamNode 的下游顶点
             ...
             */
            StreamEdge edge = new StreamEdge(upstreamNode, downstreamNode, typeNumber, partitioner, outputTag, shuffleMode);
             ...
             *  -
             *  1、先生成了一个 StreamEdge
             *  2、把当前这个 边 设置为 上有顶点的出边
             *  2、把当前这个 边 设置为 下游顶点的入边
             */
    ...
    public void setManagedMemoryUseCaseWeights(int vertexID, Map<ManagedMemoryUseCase, Integer> operatorScopeUseCaseWeights,
    ...
    /**
     * // TODO_MA 注释： 其实， StrewamGraph 自己就提供了 转换成 JObGraph 的方法
    ...
    public JobGraph getJobGraph(@Nullable JobID jobID) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： StreamGraph 自己拥有变成 JobGraph 的方法
         */
