...
public class StreamSourceContexts {
    ...
    public static <OUT> SourceFunction.SourceContext<OUT> getSourceContext(TimeCharacteristic timeCharacteristic,
            ProcessingTimeService processingTimeService, Object checkpointLock,
            StreamStatusMaintainer streamStatusMaintainer, Output<StreamRecord<OUT>> output, long watermarkInterval,
            long idleTimeout) {
         ...
         *  注释： 三种时间语义对应的 Context
         */
        switch(timeCharacteristic) {
            case EventTime:
                ctx = new ManualWatermarkContext<>(output, processingTimeService, checkpointLock, streamStatusMaintainer,
                        idleTimeout);
    ...
    private static class NonTimestampContext<T> implements SourceFunction.SourceContext<T> {
        ...
        // TODO_MA 注释：  就是接收到的一条数据。此时还在 mainOperator
        // TODO_MA 注释：  env.socketText().flatMap().map().keyby();
        @Override
        public void collect(T element) {
            synchronized(lock) {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释：
                 *  1、output = ChaingingOutput 输送到下一个 Operator， 还是在同一个 OperatorChain
                 *  2、output = RecordWriterOutput 输送到下一个 Task
                 *  -
                 *  reuse.replace(element)
                 *  内存复用！流式处理： 来一条算一条， 事实上，在计算过程中那个，只需要一个对象的内存就够了
                 */
    ...
    private static class AutomaticWatermarkContext<T> extends WatermarkContext<T> {
        ...
        private AutomaticWatermarkContext(final Output<StreamRecord<T>> output, final long watermarkInterval,
                ...
                final StreamStatusMaintainer streamStatusMaintainer, final long idleTimeout) {
        ...
        protected void processAndCollect(T element) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 设置了 时间戳 lastRecordTime
             *  element 就是读取到的要处理的数据
             *  reuse 就是一个可重复使用的一个对象
             */
            output.collect(reuse.replace(element, lastRecordTime));

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 在合适的时刻，发送 Watermark
             *  其实，就是每个 watermarkInterval 就发送一次 Watermark
             */
            // this is to avoid lock contention in the lockingObject by
            // sending the watermark before the firing of the watermark emission task.
        ...
        private class WatermarkEmittingTask implements ProcessingTimeCallback {
            ...
            public void onProcessingTime(long timestamp) {
                ...
                nextWatermarkTimer = this.timeService
                        .registerTimer(nextWatermark, new WatermarkEmittingTask(this.timeService, lock, output));
            }
        }
    }
    ...
    private static class ManualWatermarkContext<T> extends WatermarkContext<T> {
        ...
        private ManualWatermarkContext(final Output<StreamRecord<T>> output, final ProcessingTimeService timeService,
                final Object checkpointLock, final StreamStatusMaintainer streamStatusMaintainer,
    ...
    private abstract static class WatermarkContext<T> implements SourceFunction.SourceContext<T> {
         ...
         */
        public WatermarkContext(final ProcessingTimeService timeService, final Object checkpointLock,
                final StreamStatusMaintainer streamStatusMaintainer, final long idleTimeout) {
            ...
            this.streamStatusMaintainer = Preconditions
                    .checkNotNull(streamStatusMaintainer, "Stream Status Maintainer cannot be null.");
