...
public class StreamOperatorWrapper<OUT, OP extends StreamOperator<OUT>> {
    ...
    StreamOperatorWrapper(OP wrapped, Optional<ProcessingTimeService> processingTimeService,
            MailboxExecutor mailboxExecutor, boolean isHead) {
    ...
    private void quiesceTimeServiceAndCloseOperator(
            StreamTaskActionExecutor actionExecutor) throws InterruptedException, ExecutionException {
    ...
    private CompletableFuture<Void> quiesceProcessingTimeService() {
        return processingTimeService.map(ProcessingTimeService::quiesce).orElse(CompletableFuture.completedFuture(null));
    }

    private CompletableFuture<Void> sendClosedMail() {
        final CompletableFuture<Void> future = new CompletableFuture<>();

        mailboxExecutor.execute(() -> future.complete(null), "StreamOperatorWrapper#sendClosedMail for " + wrapped);
        return future;
    }
