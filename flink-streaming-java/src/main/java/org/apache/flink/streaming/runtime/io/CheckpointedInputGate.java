...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent>, Closeable {
    ...
    public CheckpointedInputGate(InputGate inputGate, CheckpointBarrierHandler barrierHandler,
            MailboxExecutor mailboxExecutor, UpstreamRecoveryTracker upstreamRecoveryTracker) {
    ...
    public Optional<BufferOrEvent> pollNext() throws IOException, InterruptedException {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 处理事件
         *  其中包括： CheckpointBarrier
         */
        if(bufferOrEvent.isEvent()) {
            return handleEvent(bufferOrEvent);
        }

        // TODO_MA 注释： 如果是 Buffer，正常待处理的数据
    ...
    private Optional<BufferOrEvent> handleEvent(BufferOrEvent bufferOrEvent) throws IOException, InterruptedException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blCheckpointBarrierog.csdn.net/zhongqi2513
         *  注释： 如果是 CheckpointBarrier
         */
        ...
        // TODO_MA 注释： 如果是 EventAnnouncement
            ...
            checkState(announcedEvent instanceof CheckpointBarrier,
                    "Only CheckpointBarrier announcement are currently supported, but found [%s]", announcedEvent);
            CheckpointBarrier announcedBarrier = (CheckpointBarrier) announcedEvent;
            barrierHandler.processBarrierAnnouncement(announcedBarrier, eventAnnouncement.getSequenceNumber(),
                    bufferOrEvent.getChannelInfo());
        }

        // TODO_MA 注释： EndOfChannelStateEvent
