...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable implements AsyncExceptionHandler {
     ...
     * {@code System.currentTimeMillis()}) and register timers for tasks to be executed in the future.
     */
    ...
    protected StreamTask(Environment environment, @Nullable TimerService timerService,
         ...
         *  11111
         */
        this(environment, timerService, uncaughtExceptionHandler, actionExecutor,
    ...
    protected StreamTask(Environment environment, @Nullable TimerService timerService,
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 RecordWriter
         *  当 一个 Task 真正运行的时候，其实，输入的真正工作的完成，是由 RecordReader 完成(批处理)
         *  输出的真正的工作是 RecordWriter 完成
         *  -
         *  如果你现在执行的这个 Task 是一个 OperatorChain, 必然内部有多个算子
         *  最后一个算子的输出就是： RecordWriterOutput
         *  前面的算子的输出就是： ChainingOutput
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建 MailboxProcessor， 这是真正用来处理 Mail 的一个 邮件处理器
         *  其实，真正处理 数据邮件的 处理，就是 StreamTask 的 processInput 方法
         *  第一个参数，很重要，用来处理输入
         *  第二个参数，具体实现是： TaskMailboxImpl
         *  -
         *  当前这个 Task 接收的要执行的任何的任务，都被封装成了一个 邮件 Mail
         *  然后被放置于 MailBox 中
         *  必然会有一个组件去轮询这个 MailBox 获取 Mail 来执行处理
         *  所有 Task 的输入的处理，都是这儿！
         *  进来的时候，先分类： 一条数据的完整处理： env.socketTextStrewam(host, port).map().key().sum();
         *  SourceStrewamTask  OneInputStreamTask
         *  -
         *  Flink 从 Flink-1.10 版本开始开始，进行了改革: 数据处理方式： Mailbox 邮件模型
         *  MailBox（treadpool, processor）
         *      processMail(） 处理一封邮件： 处理一条数据
         */
        ...
        this.asyncOperationsThreadPool = Executors
                .newCachedThreadPool(new ExecutorThreadFactory("AsyncOperations", uncaughtExceptionHandler));
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 初始化得到 SubtaskCheckpointCoordinatorImpl
         *  Flink checkpoint 机制
         */
        this.subtaskCheckpointCoordinator = new SubtaskCheckpointCoordinatorImpl(
                stateBackend.createCheckpointStorage(getEnvironment().getJobID()), getName(), actionExecutor,
                getCancelables(), getAsyncOperationsThreadPool(), getEnvironment(), this,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将 ChannelStateWriter 注入到 InputChannel 和 ResultSubPartition 中
         *  其实 ChannelStateWriter 类似于 Partitioner
         */
     ...
     *
     * // TODO_MA 注释： 这个方法是所有的 StrewamTask 都会执行的。是当前这个 StreamTask 获取数据的入口
     * // TODO_MA 注释： CheckpointBarrier 是嵌入到数据流中那个，和数据一起流动的。
     * // TODO_MA 注释： 所以当前合格 processInput 方法也会处理 CheckpointBarrier
     */
    protected void processInput(MailboxDefaultAction.Controller controller) throws Exception {

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 普通 StreamTask 通过内部的 StreamInputProcessor 来处理该 Task 的输入
         */
    ...
    private void setSynchronousSavepointId(long checkpointId) {
        Preconditions
                .checkState(syncSavepointId == null, "at most one stop-with-savepoint checkpoint at a time is allowed");
    ...
    protected void beforeInvoke() throws Exception {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 主要初始化在 OperatorChain 中进行连接输出的各种 Output 组件
         *  1、一个 Operatorchain 其实就是由多个 Operator 组成的
         *  当不是最后一个 Operator 输出的时候，使用的 output 组件就是： ChainingOutput
         *  如果是最后一个 Operator 输出，也就是说是当前这个 Task 要输出了，这个输出组件就是 RecordWriterOutput
         */
        operatorChain = new OperatorChain<>(this, recordWriter);
        mainOperator = operatorChain.getMainOperator();

        // TODO_MA 注释： 初始化 StreamOneInputProcessor, DataOutput, DataInput, CheckpointedIntputGate
        // task specific initialization
        init();
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 关于这个 init() 方法：
         *  1、SourceStreamTask 比 OneINputStreamTask 多出来一个东西： TriggerHook (checkpoint)
         */

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 注意两个细节：
         *  1、在 SourceStreamTask 的构造方法中，创建一个 LegacySourceFunctionThread 线程，用来对接数据源，接收数据
         *  这个事儿，就可以认为是 SourceStreamTask 的主业
         *  2、如果说是 OneInputStreamTask，也需要一个组件来处理数据，那么这个组件叫做：StreamOneInputProcessor
         *  就是用来完成  OneInputStreamTask 的主业： 数据的逻辑处理
         *  -
         *  不同点：一个在构造方法中， 一个在 init 方法
         */
        ...
        actionExecutor.runThrowing(() -> {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 获取 SequentialChannelStateReader = SequentialChannelStateReaderImpl
             */
            SequentialChannelStateReader reader = getEnvironment().getTaskStateManager()
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： requestPartitions
             *  在 beforeInvoke() 方法中，这个地方，的代码是实现了一个 Task 与上游 Task 建立连接的地方
             *  -
             *  上游Task 的 ResultPartition（ResultSubPartition） 和 下游Task 的 InputGate（InputChannel） 连接起来
             */
            for(InputGate inputGate : getEnvironment().getAllInputGates()) {
                inputGate.getStateConsumedFuture().thenRun(

                        // TODO_MA 注释： 注意这儿
                        () -> mainMailboxExecutor.execute(inputGate::requestPartitions, "Input gate request partitions"));

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： nputGate::requestPartitions
                 *  完成 InputChannel 到 ResultSubPartition 的注册
                 */
            }
        });
    ...
    public final void invoke() throws Exception {
        try {
             ...
             *  如果是 OneInputStreamTask， 则需要去对接上游 Task 的 ResultPartition
             */
            beforeInvoke();
    ...
    public void runMailboxLoop() throws Exception {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 调用 MailBox Processor 来处理邮箱中的邮件
         */
    ...
    public Future<Boolean> triggerCheckpointAsync(CheckpointMetaData checkpointMetaData,
        ...
        mainMailboxExecutor.execute(() -> {
            latestAsyncCheckpointStartDelayNanos = 1_000_000 * Math
                    .max(0, System.currentTimeMillis() - checkpointMetaData.getTimestamp());
    ...
    private boolean triggerCheckpoint(CheckpointMetaData checkpointMetaData, CheckpointOptions checkpointOptions,
        ...
        try {
             ...
             *  注释： 主要是确保数据发送给了下游了
             *  -
             *  当前是 SourceStreamTask, 只要接收到 checkpint 的 rpc 请求
             *  事实上不需要做任何等待，只需要确保当前 Task 的数据输出到了下游，就可以执行 checkpint 了
             */
            ...
            if(isRunning) {
                throw new Exception("Could not perform checkpoint " + checkpointMetaData
                        .getCheckpointId() + " for operator " + getName() + '.', e);
            } else {
                LOG.debug(
                        "Could not perform checkpoint {} for operator {} while the " + "invokable was not in state running.",
    ...
    public void triggerCheckpointOnBarrier(CheckpointMetaData checkpointMetaData, CheckpointOptions checkpointOptions,
        ...
        try {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 从此 StreamTask 执行 checkpoint 了
             *  此处讲的这个 StreamTask 是指非 SourceStreamTask
             */
            ...
            LOG.info("Operator {} was cancelled while performing checkpoint {}.", getName(),
                    checkpointMetaData.getCheckpointId());
            throw e;
        } catch(Exception e) {
            throw new IOException("Could not perform checkpoint " + checkpointMetaData
                    .getCheckpointId() + " for operator " + getName() + '.', e);
        }
    }
    ...
    private boolean performCheckpoint(CheckpointMetaData checkpointMetaData, CheckpointOptions checkpointOptions,
            CheckpointMetricsBuilder checkpointMetrics, boolean advanceToEndOfTime) throws Exception {

        LOG.debug("Starting checkpoint ({}) {} on task {}", checkpointMetaData.getCheckpointId(),
        ...
        if(isRunning) {
            actionExecutor.runThrowing(() -> {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释：
                 *  CheckpintCoordinator 和 subtaskCheckpointCoordinator 组成了一个 一主多从的结构
                 */
                subtaskCheckpointCoordinator
                        .checkpointState(checkpointMetaData, checkpointOptions, checkpointMetrics, operatorChain,
                                this::isCanceled);
            });
    ...
    protected void declineCheckpoint(long checkpointId) {
        getEnvironment().declineCheckpoint(checkpointId, new CheckpointException("Task Name" + getName(),
    ...
    public void dispatchOperatorEvent(OperatorID operator, SerializedValue<OperatorEvent> event) throws FlinkException {
        try {
            mainMailboxExecutor.execute(
                    ...
                    // TODO_MA 注释：
                    () -> operatorChain.dispatchOperatorEvent(operator, event), "dispatch operator event");
    ...
    private static <OUT> List<RecordWriter<SerializationDelegate<StreamRecord<OUT>>>> createRecordWriters(
        ...
        // TODO_MA 注释： StreamGraph 中的 StreamEdge 一般都是一个
        List<StreamEdge> outEdgesInOrder = configuration
                .getOutEdgesInOrder(environment.getUserCodeClassLoader().asClassLoader());

        // TODO_MA 注释： 遍历这个 Task 的每个出边，都创建一个 RecordWriter
        for(int i = 0; i < outEdgesInOrder.size(); i++) {
            StreamEdge edge = outEdgesInOrder.get(i);

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建一个 RecordWriter
             */
    ...
    private static <OUT> RecordWriter<SerializationDelegate<StreamRecord<OUT>>> createRecordWriter(StreamEdge edge,
            int outputIndex, Environment environment, String taskName, long bufferTimeout) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 RecordWriter = ChannelSelectorRecordWriter
         */
    ...
    ProcessingTimeCallback deferCallbackToMailbox(MailboxExecutor mailboxExecutor, ProcessingTimeCallback callback) {
        return timestamp -> {
            mailboxExecutor.execute(() -> invokeProcessingTimeCallback(callback, timestamp), "Timer callback for %s @ %d",
                    callback, timestamp);
        };
    }
