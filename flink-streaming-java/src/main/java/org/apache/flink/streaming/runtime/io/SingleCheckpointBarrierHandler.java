...
public class SingleCheckpointBarrierHandler extends CheckpointBarrierHandler {
    ...
    static SingleCheckpointBarrierHandler createUnalignedCheckpointBarrierHandler(
            SubtaskCheckpointCoordinator checkpointCoordinator, String taskName, AbstractInvokable toNotifyOnCheckpoint,
            CheckpointableInput... inputs) {
        return new SingleCheckpointBarrierHandler(taskName, toNotifyOnCheckpoint,
    ...
    SingleCheckpointBarrierHandler(String taskName, AbstractInvokable toNotifyOnCheckpoint, int numOpenChannels,
        ...
        if(currentCheckpointId < barrierId) {
            ...
            try {
                if(controller.preProcessFirstBarrier(channelInfo, barrier)) {
                    LOG.debug("{}: Triggering checkpoint {} on the first barrier at {}.", taskName, barrier.getId(),
        ...
        if(currentCheckpointId == barrierId) {
            if(++numBarriersReceived == numOpenChannels) {
                ...
                if(controller.postProcessLastBarrier(channelInfo, barrier)) {
                    LOG.debug("{}: Triggering checkpoint {} on the last barrier at {}.", taskName, barrier.getId(),
                            barrier.getTimestamp());
                    notifyCheckpoint(barrier);
                }
    ...
    @Override
    public void processBarrierAnnouncement(CheckpointBarrier announcedBarrier, int sequenceNumber,
    ...
    private void cancelSubsumedCheckpoint(long barrierId) throws IOException {
        ...
        LOG.warn(
                ...
                taskName, barrierId, currentCheckpointId);
    ...
    public String toString() {
        return String.format("%s: current checkpoint: %d, current barriers: %d, open channels: %d", taskName,
                currentCheckpointId, numBarriersReceived, numOpenChannels);
    }
}
