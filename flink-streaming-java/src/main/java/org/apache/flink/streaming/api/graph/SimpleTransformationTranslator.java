...
public abstract class SimpleTransformationTranslator<OUT, T extends Transformation<OUT>> implements TransformationTranslator<OUT, T> {
    ...
    public final Collection<Integer> translateForStreaming(final T transformation, final Context context) {
         ...
         *  注释： 这个地方注意：
         *  有可能是： Source， 里面咩有构建边的逻辑
         *  有可能是： OneInput
         *  有可能是： TwoInput  union  join
         */
    ...
    private void configure(final T transformation, final Context context) {
        ...
        if(streamNode != null && streamNode.getManagedMemoryOperatorScopeUseCaseWeights().isEmpty() && streamNode.getManagedMemorySlotScopeUseCases()
                .isEmpty()) {
