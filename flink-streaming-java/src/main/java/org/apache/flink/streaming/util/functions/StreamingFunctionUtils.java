...
public final class StreamingFunctionUtils {
    ...
    public static <T> void setOutputType(Function userFunction, TypeInformation<T> outTypeInfo,
    ...
    @SuppressWarnings("unchecked")
    private static <T> boolean trySetOutputType(Function userFunction, TypeInformation<T> outTypeInfo,
            ExecutionConfig executionConfig) {
    ...
    public static void snapshotFunctionState(StateSnapshotContext context, OperatorStateBackend backend,
    ...
    private static boolean trySnapshotFunctionState(StateSnapshotContext context, OperatorStateBackend backend,
            Function userFunction) throws Exception {
        ...
        if(userFunction instanceof ListCheckpointed) {
            ...
            @SuppressWarnings("unchecked") List<Serializable> partitionableState = ((ListCheckpointed<Serializable>) userFunction)
                    .snapshotState(context.getCheckpointId(), context.getCheckpointTimestamp());

            // We are using JavaSerializer from the flink-runtime module here. This is very naughty
            // and we shouldn't be doing it because ideally nothing in the API modules/connector depends
            ...
            ListStateDescriptor<Serializable> listStateDescriptor = new ListStateDescriptor<>(
                    DefaultOperatorStateBackend.DEFAULT_OPERATOR_STATE_NAME, new JavaSerializer<>());
    ...
    private static boolean tryRestoreFunction(StateInitializationContext context,
            Function userFunction) throws Exception {
