...
public class StreamSource<OUT, SRC extends SourceFunction<OUT>> extends AbstractUdfStreamOperator<OUT, SRC> {
    ...
    public void run(final Object lockingObject, final StreamStatusMaintainer streamStatusMaintainer, final OperatorChain<?, ?> operatorChain) throws Exception {
    ...
    public void run(final Object lockingObject, final StreamStatusMaintainer streamStatusMaintainer, final Output<StreamRecord<OUT>> collector,
            final OperatorChain<?, ?> operatorChain) throws Exception {

        final TimeCharacteristic timeCharacteristic = getOperatorConfig().getTimeCharacteristic();

        final Configuration configuration = this.getContainingTask().getEnvironment().getTaskManagerInfo().getConfiguration();
        final long latencyTrackingInterval = getExecutionConfig().isLatencyTrackingConfigured() ? getExecutionConfig()
                .getLatencyTrackingInterval() : configuration.getLong(MetricOptions.LATENCY_INTERVAL);

        LatencyMarksEmitter<OUT> latencyEmitter = null;
        if(latencyTrackingInterval > 0) {
            latencyEmitter = new LatencyMarksEmitter<>(getProcessingTimeService(), collector, latencyTrackingInterval, this.getOperatorID(),
        ...
        // TODO_MA 注释： 根据时间语义创建上下文对象
        this.ctx = StreamSourceContexts
                .getSourceContext(timeCharacteristic, getProcessingTimeService(), lockingObject, streamStatusMaintainer, collector, watermarkInterval, -1);
        ...
        try {
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 调用用户定义的数据处理函数来执行
             *  最开始在讲 StreamGraph 构建的时候，注意一个转化关系：
             *  Function  ===> StreamOperator =====> Transformation  ====> StreamNode
             *  -
             *  userFunction 两种最典型的实现：
             *  1、socket 网络流
             *  2、Kafka
             *  -
             *  从 Operator 进来的。 Operator 其实封装了 UserFunction
             *  dataStream.textFile("/").flatMap(new FlatMapFunction(....))
             *  flatMap = StreamFlatMap
             *  FlatMapFunction = userFunction
             */
    ...
    private static class LatencyMarksEmitter<OUT> {
        ...
        public LatencyMarksEmitter(final ProcessingTimeService processingTimeService, final Output<StreamRecord<OUT>> output, long latencyTrackingInterval,
                final OperatorID operatorId, final int subtaskIndex) {
