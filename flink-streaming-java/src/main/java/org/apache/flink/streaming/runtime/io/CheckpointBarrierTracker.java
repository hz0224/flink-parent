...
public class CheckpointBarrierTracker extends CheckpointBarrierHandler {
    ...
    public void processBarrier(CheckpointBarrier receivedBarrier, InputChannelInfo channelInfo) throws IOException {
        ...
        // TODO_MA 注释： barrierId
        final long barrierId = receivedBarrier.getId();

        // TODO_MA 注释： 如果只有一个 inputchannel，那么接收到这个 barrier 就可以直接执行 checkpoint 了
        ...
        if(barrierCount != null) {
            ...
            // TODO_MA 注释： 接收到的 barrier 计数器 + 1
            // add one to the count to that barrier and check for completion
            int numBarriersNew = barrierCount.incrementBarrierCount();

            // TODO_MA 注释： 如果从所有 inputchannel 处都收到了 Barrier
            if(numBarriersNew == totalNumberOfInputChannels) {

                // TODO_MA 注释： 可以触发 checkpoint 了
    ...
    private static final class CheckpointBarrierCount {
        ...
        public String toString() {
            return isAborted() ? String.format("checkpointID=%d - ABORTED", checkpointId) : String
                    .format("checkpointID=%d, count=%d", checkpointId, barrierCount);
        }
    }
}
