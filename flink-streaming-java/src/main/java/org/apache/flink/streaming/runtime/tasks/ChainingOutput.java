...
class ChainingOutput<T> implements WatermarkGaugeExposingOutput<StreamRecord<T>> {
    ...
    public ChainingOutput(OneInputStreamOperator<T, ?> operator, StreamStatusProvider streamStatusProvider,
        ...
        this(operator, (OperatorMetricGroup) operator.getMetricGroup(), streamStatusProvider, outputTag, operator::close);
    }

    public ChainingOutput(Input<T> input, OperatorMetricGroup operatorMetricGroup,
            StreamStatusProvider streamStatusProvider, @Nullable OutputTag<T> outputTag,
    ...
    // TODO_MA 注释： （OperatorChain  中有 o1 和 o2）
    @Override
