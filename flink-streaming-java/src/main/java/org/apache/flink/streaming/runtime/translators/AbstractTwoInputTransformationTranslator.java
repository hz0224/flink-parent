...
public abstract class AbstractTwoInputTransformationTranslator<IN1, IN2, OUT, OP extends Transformation<OUT>> extends SimpleTransformationTranslator<OUT, OP> {
    ...
    protected Collection<Integer> translateInternal(final Transformation<OUT> transformation,
            final Transformation<IN1> firstInputTransformation, final Transformation<IN2> secondInputTransformation,
            final StreamOperatorFactory<OUT> operatorFactory, @Nullable final TypeInformation<?> keyTypeInfo,
            @Nullable final KeySelector<IN1, ?> firstKeySelector, @Nullable final KeySelector<IN2, ?> secondKeySelector,
         ...
         *  注释： 顶点 StreamNode 处理
         */
        streamGraph.addCoOperator(transformationId, slotSharingGroup, transformation.getCoLocationGroupKey(),
                operatorFactory, firstInputTransformation.getOutputType(), secondInputTransformation.getOutputType(),
        ...
        final int parallelism = transformation.getParallelism() != ExecutionConfig.PARALLELISM_DEFAULT ? transformation
                .getParallelism() : executionConfig.getParallelism();
        streamGraph.setParallelism(transformationId, parallelism);
        streamGraph.setMaxParallelism(transformationId, transformation.getMaxParallelism());

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第一个边 StreamEdge 处理
         */
