...
public class SourceOperatorFactory<OUT> extends AbstractStreamOperatorFactory<OUT> implements CoordinatedOperatorFactory<OUT>, ProcessingTimeServiceAware {
    ...
    public SourceOperatorFactory(Source<OUT, ?, ?> source, WatermarkStrategy<OUT> watermarkStrategy,
            boolean emitProgressiveWatermarks, int numCoordinatorWorkerThread) {
    ...
    public <T extends StreamOperator<OUT>> T createStreamOperator(StreamOperatorParameters<OUT> parameters) {
        ...
        final SourceOperator<OUT, ?> sourceOperator = instantiateSourceOperator(source::createReader, gateway,
                source.getSplitSerializer(), watermarkStrategy, parameters.getProcessingTimeService(),
    ...
    private static <T, SplitT extends SourceSplit> SourceOperator<T, SplitT> instantiateSourceOperator(
            ...
            OperatorEventGateway eventGateway, SimpleVersionedSerializer<?> splitSerializer,
            WatermarkStrategy<T> watermarkStrategy, ProcessingTimeService timeService, Configuration config,
            String localHostName, boolean emitProgressiveWatermarks) {
        ...
        return new SourceOperator<>(typedReaderFactory, eventGateway, typedSplitSerializer, watermarkStrategy,
                timeService, config, localHostName, emitProgressiveWatermarks);
    }
}
