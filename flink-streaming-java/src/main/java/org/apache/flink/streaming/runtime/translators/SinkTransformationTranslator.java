...
public class SinkTransformationTranslator<InputT, CommT, WriterStateT, GlobalCommT> implements TransformationTranslator<Object, SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT>> {
    ...
    @Override
    public Collection<Integer> translateForBatch(SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> transformation, Context context) {
        ...
        try {
            internalTranslate(transformation, parallelism, PREVIOUS_SINK_STATE_NAME, new BatchCommitterOperatorFactory<>(transformation.getSink()), 1, 1,
    ...
    @Override
    public Collection<Integer> translateForStreaming(SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> transformation, Context context) {
        ...
        try {
            internalTranslate(transformation, parallelism, PREVIOUS_SINK_STATE_NAME, new StreamingCommitterOperatorFactory<>(transformation.getSink()),
                    parallelism, transformation.getMaxParallelism(), new StreamingGlobalCommitterOperatorFactory<>(transformation.getSink()), context);
        } catch(IOException e) {
            throw new FlinkRuntimeException("Could not add the Committer or GlobalCommitter to the stream graph.", e);
        }
     ...
     */
    private void internalTranslate(SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation, int writerParallelism,
            @SuppressWarnings("SameParameterValue") @Nullable String previousSinkStateName, OneInputStreamOperatorFactory<CommT, CommT> committerFactory,
            int committerParallelism, int committerMaxParallelism, OneInputStreamOperatorFactory<CommT, GlobalCommT> globalCommitterFactory,
            Context context) throws IOException {

        StreamGraphUtils.validateTransformationUid(context.getStreamGraph(), sinkTransformation);

        final int writerId = addWriter(sinkTransformation, writerParallelism, previousSinkStateName, context);

        final int committerId = addCommitter(writerId, sinkTransformation, committerFactory, committerParallelism, committerMaxParallelism, context);

        addGlobalCommitter(committerId > 0 ? committerId : writerId, sinkTransformation, globalCommitterFactory, context);
    }
     ...
     */
    private int addWriter(SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation, int parallelism,
            @Nullable String previousSinkStateName, Context context) {
        final boolean hasState = sinkTransformation.getSink().getWriterStateSerializer().isPresent();
        checkState(sinkTransformation.getInputs().size() == 1);
        @SuppressWarnings("unchecked") final Transformation<InputT> input = (Transformation<InputT>) sinkTransformation.getInputs().get(0);
        final TypeInformation<InputT> inputTypeInfo = input.getOutputType();

        final StreamOperatorFactory<CommT> writer = hasState ? new StatefulSinkWriterOperatorFactory<>(sinkTransformation.getSink(),
                previousSinkStateName) : new StatelessSinkWriterOperatorFactory<>(sinkTransformation.getSink());
        ...
        return addOperatorToStreamGraph(writer, context.getStreamNodeIds(input), inputTypeInfo, extractCommittableTypeInformation(sinkTransformation.getSink()),
                String.format("%s %s", prefix, sinkTransformation.getName()), sinkTransformation.getUid(), parallelism, sinkTransformation.getMaxParallelism(),
                sinkTransformation, context);
    }
     ...
     * committer.
     */
    private int addCommitter(int inputId, SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation,
            OneInputStreamOperatorFactory<CommT, CommT> committerFactory, int parallelism, int maxParallelism, Context context) throws IOException {
        ...
        final CommittableTypeInformation<CommT> committableTypeInfo = extractCommittableTypeInformation(sinkTransformation.getSink());
        checkNotNull(committableTypeInfo);

        return addOperatorToStreamGraph(committerFactory, Collections.singletonList(inputId), committableTypeInfo, committableTypeInfo,
                String.format("%s %s", prefix, sinkTransformation.getName()),
                sinkTransformation.getUid() == null ? null : String.format("%s %s", prefix, sinkTransformation.getUid()), parallelism, maxParallelism,
                sinkTransformation, context);
    }
     ...
     */
    private void addGlobalCommitter(int inputId, SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation,
            OneInputStreamOperatorFactory<CommT, GlobalCommT> globalCommitterFactory, Context context) throws IOException {
        ...
        addOperatorToStreamGraph(globalCommitterFactory, Collections.singletonList(inputId),
                checkNotNull(extractCommittableTypeInformation(sinkTransformation.getSink())), null,
                String.format("%s %s", prefix, sinkTransformation.getName()),
                sinkTransformation.getUid() == null ? null : String.format("%s %s", prefix, sinkTransformation.getUid()), 1, 1, sinkTransformation, context);
    }

    private int getParallelism(SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation, Context context) {
        return sinkTransformation.getParallelism() != ExecutionConfig.PARALLELISM_DEFAULT ? sinkTransformation.getParallelism() : context.getStreamGraph()
     ...
     */
    private <IN, OUT> int addOperatorToStreamGraph(StreamOperatorFactory<OUT> operatorFactory, Collection<Integer> inputs, TypeInformation<IN> inTypeInfo,
            TypeInformation<OUT> outTypInfo, String name, @Nullable String uid, int parallelism, int maxParallelism,
            SinkTransformation<InputT, CommT, WriterStateT, GlobalCommT> sinkTransformation, Context context) {
    ...
    private CommittableTypeInformation<CommT> extractCommittableTypeInformation(Sink<InputT, CommT, WriterStateT, GlobalCommT> sink) {
        if(sink.getCommittableSerializer().isPresent()) {
            ...
            return new CommittableTypeInformation<>(typeToClass(committableType), () -> sink.getCommittableSerializer().get());
        } else {
            return null;
        }
    }
