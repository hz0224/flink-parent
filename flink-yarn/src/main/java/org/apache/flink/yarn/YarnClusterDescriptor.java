...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    public YarnClusterDescriptor(Configuration flinkConfiguration, YarnConfiguration yarnConfiguration,
            YarnClient yarnClient, YarnClusterInformationRetriever yarnClusterInformationRetriever,
            boolean sharedYarnClient) {

        // TODO_MA 注释： yarn 配置
        this.yarnConfiguration = Preconditions.checkNotNull(yarnConfiguration);

        // TODO_MA 注释： yarn client
    ...
    private String getDecodedJarPath() {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Couldn't decode the encoded Flink dist jar path: " + encodedJarPath + " You can supply a path manually via the command line.");
        }
    }
    ...
    public void addShipFiles(List<File> shipFiles) {
        checkArgument(userJarInclusion != YarnConfigOptions.UserJarInclusion.DISABLED || isUsrLibDirIncludedInShipFiles(
                ...
                ConfigConstants.DEFAULT_FLINK_USR_LIB_DIR, YarnConfigOptions.CLASSPATH_INCLUDE_USER_JAR.key(),
                YarnConfigOptions.UserJarInclusion.DISABLED, ConfigConstants.DEFAULT_FLINK_USR_LIB_DIR);
    ...
    private static boolean isArchiveOnlyIncludedInShipArchiveFiles(List<File> shipFiles) {
        return shipFiles.stream().filter(File::isFile).map(File::getName).map(String::toLowerCase).allMatch(
                name -> name.endsWith(".tar.gz") || name.endsWith(".tar") || name.endsWith(".tgz") || name
                        .endsWith(".dst") || name.endsWith(".jar") || name.endsWith(".zip"));
    }
    ...
    private void isReadyForDeployment(ClusterSpecification clusterSpecification) throws Exception {
        ...
        if(configuredAmVcores > numYarnMaxVcores) {
            throw new IllegalConfigurationException(String.format(
                    "The number of requested virtual cores for application master %d" + " exceeds the maximum number of virtual cores %d available in the Yarn Cluster.",
                    configuredAmVcores, numYarnMaxVcores));
        }

        // TODO_MA 注释： yarn.containers.vcores = SlotsPerTaskManager，不能大于yarn的最大 core 个数
        int configuredVcores = flinkConfiguration
                .getInteger(YarnConfigOptions.VCORES, clusterSpecification.getSlotsPerTaskManager());
        // don't configure more than the maximum configured number of vcores
        if(configuredVcores > numYarnMaxVcores) {
            throw new IllegalConfigurationException(String.format(
                    "The number of requested virtual cores per node %d" + " exceeds the maximum number of virtual cores %d available in the Yarn Cluster." + " Please note that the number of virtual cores is set to the number of task slots by default" + " unless configured in the Flink config with '%s.'",
    ...
    public ClusterClientProvider<ApplicationId> retrieve(ApplicationId applicationId) throws ClusterRetrieveException {
        ...
        try {
            ...
            // TODO_MA 注释： YarnCluster 的时候，客户端实现也是： RestClusterClient
            return () -> {
                try {
    ...
    public ClusterClientProvider<ApplicationId> deploySessionCluster(
        ...
        try {
            return deployInternal(clusterSpecification, "Flink session cluster", getYarnSessionClusterEntrypoint(), null,
                    false);
        } catch(Exception e) {
    ...
    public ClusterClientProvider<ApplicationId> deployApplicationCluster(final ClusterSpecification clusterSpecification,
        ...
        if(YarnDeploymentTarget.APPLICATION != deploymentTarget) {
            throw new ClusterDeploymentException(
                    "Couldn't deploy Yarn Application Cluster." + " Expected deployment.target=" + YarnDeploymentTarget.APPLICATION
                            .getName() + " but actual one was \"" + deploymentTarget.getName() + "\"");
        }

        applicationConfiguration.applyToConfiguration(flinkConfiguration);

        final List<String> pipelineJars = flinkConfiguration.getOptional(PipelineOptions.JARS)
                .orElse(Collections.emptyList());
        Preconditions.checkArgument(pipelineJars.size() == 1, "Should only have one jar");

        try {
            return deployInternal(clusterSpecification, "Flink Application Cluster",
    ...
    public ClusterClientProvider<ApplicationId> deployJobCluster(ClusterSpecification clusterSpecification,
        ...
        try {
             ...
             *  注释： 调用内部实现
             *  注意第三个参数： YarnJobClusterEntrypoint 任务入口类
             */
            return deployInternal(clusterSpecification,
                    "Flink per-job cluster",    // TODO_MA 注释： 任务类型
                    getYarnJobClusterEntrypoint(),              // TODO_MA 注释： 任务入口类
                    jobGraph,
                    detached
            );
        } catch(Exception e) {
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(ClusterSpecification clusterSpecification,
            String applicationName, String yarnClusterEntrypoint, @Nullable JobGraph jobGraph,
            boolean detached) throws Exception {

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 获取提交 job 的用户信息，并根据配置决定是否校验权限
         */
        ...
        if(HadoopUtils.isKerberosSecurityEnabled(currentUser)) {
            ...
            if(!HadoopUtils.areKerberosCredentialsValid(currentUser, useTicketCache)) {
                throw new RuntimeException(
                        "Hadoop security with Kerberos is enabled but the login user " + "does not have Kerberos credentials or delegation tokens!");
            }
        }

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 做一些检查，比如 jar 包路径， conf 配置路径， yarn 的 cpu 个数校验等等
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建一个 YarnClientApplication 对象
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： yarn.scheduler.minimum-allocation-mb = 1G
         */
        ...
        if(yarnMinAllocationMB <= 0) {
            throw new YarnDeploymentException(
                    "The minimum allocation memory " + "(" + yarnMinAllocationMB + " MB) configured via '" + YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB + "' should be greater than 0.");
        }
        ...
        try {
            validClusterSpecification = validateClusterResources(clusterSpecification, yarnMinAllocationMB, maxRes,
                    freeClusterMem);
        ...
        // TODO_MA 注释： detach 模式，客户端提交完成之后，可以退出， flink -d 命令
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 AppMaster
         *  AppMaster = YarnJobClusterEntrypoint
         */
        ApplicationReport report = startAppMaster(flinkConfiguration, applicationName, yarnClusterEntrypoint, jobGraph,
                yarnClient, yarnApplication, validClusterSpecification);
    ...
    private ClusterSpecification validateClusterResources(ClusterSpecification clusterSpecification,
            int yarnMinAllocationMB, Resource maximumResourceCapability,
        ...
        if(jobManagerMemoryMb > maximumResourceCapability.getMemory()) {
            throw new YarnDeploymentException(
                    "The cluster does not have the requested resources for the JobManager available!\n" + "Maximum Memory: " + maximumResourceCapability
                            .getMemory() + "MB Requested: " + jobManagerMemoryMb + "MB. " + note);
        }
        ...
        if(taskManagerMemoryMb > maximumResourceCapability.getMemory()) {
            throw new YarnDeploymentException(
                    "The cluster does not have the requested resources for the TaskManagers available!\n" + "Maximum Memory: " + maximumResourceCapability
                            .getMemory() + " Requested: " + taskManagerMemoryMb + "MB. " + note);
        }
        ...
        if(taskManagerMemoryMb > freeClusterResources.containerLimit) {
            LOG.warn(
                    "The requested amount of memory for the TaskManagers (" + taskManagerMemoryMb + "MB) is more than " + "the largest possible YARN container: " + freeClusterResources.containerLimit + noteRsc);
        }
        if(jobManagerMemoryMb > freeClusterResources.containerLimit) {
            LOG.warn(
                    "The requested amount of memory for the JobManager (" + jobManagerMemoryMb + "MB) is more than " + "the largest possible YARN container: " + freeClusterResources.containerLimit + noteRsc);
        }

        return new ClusterSpecification.ClusterSpecificationBuilder().setMasterMemoryMB(jobManagerMemoryMb)
                .setTaskManagerMemoryMB(taskManagerMemoryMb)
                .setSlotsPerTaskManager(clusterSpecification.getSlotsPerTaskManager()).createClusterSpecification();
    }

    private void logIfComponentMemNotIntegerMultipleOfYarnMinAllocation(String componentName, int componentMemoryMB,
        ...
        if(componentMemoryMB != normalizedMemMB) {
            LOG.info(
                    ...
                    componentName, componentMemoryMB, normalizedMemMB, yarnMinAllocationMB,
    ...
    private void checkYarnQueues(YarnClient yarnClient) {
        try {
            ...
            if(queues.size() > 0 && this.yarnQueue != null) { // check only if there are queues configured in yarn and for
                // this session.
                boolean queueFound = false;
                for(QueueInfo queue : queues) {

                    // TODO_MA 注释： 如果指定的 yarn queue 存在，则记录一下
                    if(queue.getQueueName().equals(this.yarnQueue) || queue.getQueueName()
                ...
                if(!queueFound) {
                    ...
                    LOG.warn(
                            "The specified queue '" + this.yarnQueue + "' does not exist. " + "Available queues: " + queueNames);
                }
            } else {
    ...
    private ApplicationReport startAppMaster(Configuration configuration, String applicationName,
            String yarnClusterEntrypoint, JobGraph jobGraph, YarnClient yarnClient, YarnClientApplication yarnApplication,
            ClusterSpecification clusterSpecification) throws Exception {

        // TODO_MA 注释： 初始化文件系统相关
        ...
        if(!fs.getClass().getSimpleName().equals("GoogleHadoopFileSystem") && fs.getScheme().startsWith("file")) {
            LOG.warn("The file system scheme is '" + fs
                    .getScheme() + "'. This indicates that the " + "specified Hadoop configuration path is wrong and the system is using the default Hadoop configuration values." + "The Flink YARN client needs to store its files in a distributed file system");
        }
        ...
        final YarnApplicationFileUploader fileUploader = YarnApplicationFileUploader
                .from(fs, getStagingDir(fs), providedLibDirs, appContext.getApplicationId(), getFileReplication());
        ...
        if(jobGraph != null) {
            userJarFiles.addAll(jobGraph.getUserJars().stream().map(f -> f.toUri()).map(Path::new)
        ...
        // TODO_MA 注释： 将 JobGraph 依赖的 jar 存入分布式缓存
        // only for per job mode
        if(jobGraph != null) {
            for(Map.Entry<String, DistributedCache.DistributedCacheEntry> entry : jobGraph.getUserArtifacts()
                    .entrySet()) {
        ...
        final List<String> uploadedDependencies = fileUploader.registerMultipleLocalResources(
                systemShipFiles.stream().map(e -> new Path(e.toURI())).collect(Collectors.toSet()), Path.CUR_DIR,
        ...
        if(providedLibDirs == null || providedLibDirs.isEmpty()) {
            ...
            fileUploader.registerMultipleLocalResources(
                    shipOnlyFiles.stream().map(e -> new Path(e.toURI())).collect(Collectors.toSet()), Path.CUR_DIR,
                    LocalResourceType.FILE);
        }

        if(!shipArchives.isEmpty()) {
            fileUploader.registerMultipleLocalResources(
                    shipArchives.stream().map(e -> new Path(e.toURI())).collect(Collectors.toSet()), Path.CUR_DIR,
        ...
        // TODO_MA 注释： 将 JobGraph 写入到文件，然后上传
        ...
        if(jobGraph != null) {
            ...
            try {
                ...
                try(FileOutputStream output = new FileOutputStream(
                        tmpJobGraphFile); ObjectOutputStream obOutput = new ObjectOutputStream(output)) {
                    obOutput.writeObject(jobGraph);
                }
                final String jobGraphFilename = "job.graph";
                configuration.setString(JOB_GRAPH_FILE_PATH, jobGraphFilename);
                fileUploader.registerSingleLocalResource(jobGraphFilename, new Path(tmpJobGraphFile.toURI()), "",
        ...
        try {
            ...
            fileUploader.registerSingleLocalResource(flinkConfigKey, new Path(tmpConfigurationFile.getAbsolutePath()), "",
        ...
        if(System.getenv("IN_TESTS") != null) {
            ...
            remoteYarnSiteXmlPath = fileUploader
                    .registerSingleLocalResource(Utils.YARN_SITE_FILE_NAME, yarnSitePath, "", LocalResourceType.FILE,
                            false, false).getPath();
        ...
        if(!StringUtils.isNullOrWhitespaceOnly(krb5Config)) {
            ...
            remoteKrb5Path = fileUploader
                    .registerSingleLocalResource(Utils.KRB5_FILE_NAME, krb5ConfPath, "", LocalResourceType.FILE, false,
                            false).getPath();
            hasKrb5 = true;
        }
        ...
        if(keytab != null) {
            ...
            if(localizeKeytab) {
                ...
                remotePathKeytab = fileUploader
                        .registerSingleLocalResource(localizedKeytabPath, new Path(keytab), "", LocalResourceType.FILE,
                                false, false).getPath();
            } else {
        ...
        final JobManagerProcessSpec processSpec = JobManagerProcessUtils
                .processSpecFromConfigWithNewOptionToInterpretLegacyHeap(flinkConfiguration,
                        JobManagerOptions.TOTAL_PROCESS_MEMORY);

        // TODO_MA 注释： 构建 ApplicationMasterContainer
        final ContainerLaunchContext amContainer = setupApplicationMasterContainer(yarnClusterEntrypoint, hasKrb5,
                processSpec);
        ...
        if(UserGroupInformation.isSecurityEnabled()) {
            ...
            List<Path> yarnAccessList = ConfigUtils
                    .decodeListFromConfig(configuration, YarnConfigOptions.YARN_ACCESS, Path::new);
            Utils.setTokensFor(amContainer, ListUtils.union(yarnAccessList, fileUploader.getRemotePaths()),
                    yarnConfiguration);
        }
        ...
        // TODO_MA 注释： 给 ApplicationMaster 设置各种 环境变量参数
        // Setup CLASSPATH and environment variables for ApplicationMaster
        final Map<String, String> appMasterEnv = new HashMap<>();
        // set user specified app master environment variables
        appMasterEnv.putAll(ConfigurationUtils
                .getPrefixedKeyValuePairs(ResourceManagerOptions.CONTAINERIZED_MASTER_ENV_PREFIX, configuration));
        ...
        // TODO_MA 注释： 将 appMasterEnv 设置到 amContainer
        amContainer.setEnvironment(appMasterEnv);

        // TODO_MA 注释： 设置各种信息，比如 内存，cpu, 任务名称，任务类型等等
    ...
    private Path getStagingDir(FileSystem fileSystem) {
        ...
        return configuredStagingDir != null ? fileSystem.makeQualified(new Path(configuredStagingDir)) : fileSystem
                .getHomeDirectory();
    }

    private int getFileReplication() {
        final int yarnFileReplication = yarnConfiguration
                .getInt(DFSConfigKeys.DFS_REPLICATION_KEY, DFSConfigKeys.DFS_REPLICATION_DEFAULT);
        ...
        try {
            ...
            for(QueueInfo q : qInfo) {
                ps.println("Queue: " + q.getQueueName() + ", Current Capacity: " + q
                        .getCurrentCapacity() + " Max Capacity: " + q.getMaximumCapacity() + " Applications: " + q
    ...
    private void activateHighAvailabilitySupport(
            ApplicationSubmissionContext appContext) throws InvocationTargetException, IllegalAccessException {
    ...
    private void setApplicationTags(
            final ApplicationSubmissionContext appContext) throws InvocationTargetException, IllegalAccessException {
    ...
    private static class ApplicationSubmissionContextReflector {
        ...
        public void setApplicationTags(ApplicationSubmissionContext appContext,
            ...
            if(applicationTagsMethod != null) {
                LOG.debug("Calling method {} of {}.", applicationTagsMethod.getName(),
        ...
        public void setApplicationNodeLabel(ApplicationSubmissionContext appContext,
            ...
            if(nodeLabelExpressionMethod != null) {
                LOG.debug("Calling method {} of {}.", nodeLabelExpressionMethod.getName(),
                        appContext.getClass().getCanonicalName());
    ...
    void addLibFoldersToShipFiles(Collection<File> effectiveShipFiles) {
        ...
        if(libDir != null) {
            ...
            if(directoryFile.isDirectory()) {
                ...
                throw new YarnDeploymentException(
                        "The environment variable '" + ENV_FLINK_LIB_DIR + "' is set to '" + libDir + "' but the directory doesn't exist.");
            }
        } else if(shipFiles.isEmpty()) {
            LOG.warn(
                    "Environment variable '{}' not set and ship files have not been provided manually. " + "Not shipping any library files.",
                    ENV_FLINK_LIB_DIR);
        }
    }
    ...
    ContainerLaunchContext setupApplicationMasterContainer(String yarnClusterEntrypoint, boolean hasKrb5,
