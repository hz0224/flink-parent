...
class YarnApplicationFileUploader implements AutoCloseable {
    ...
    private YarnApplicationFileUploader(final FileSystem fileSystem, final Path homeDir, final List<Path> providedLibDirs,
            final ApplicationId applicationId, final int fileReplication) throws IOException {
     ...
     */
    YarnLocalResourceDescriptor registerSingleLocalResource(final String key, final Path resourcePath,
            final String relativeDstPath, final LocalResourceType resourceType, final boolean whetherToAddToRemotePaths,
        ...
        final YarnLocalResourceDescriptor descriptor = new YarnLocalResourceDescriptor(key, remoteFileInfo.f0,
                localFile.length(), remoteFileInfo.f1, LocalResourceVisibility.APPLICATION, resourceType);
        addToEnvShipResourceList(whetherToAddToEnvShipResourceList, descriptor);
        localResources.put(key, descriptor.toLocalResource());
        return descriptor;
    }

    Tuple2<Path, Long> uploadLocalFileToRemote(final Path localSrcPath, final String relativeDstPath) throws IOException {
        ...
        if(fss == null || fss.length <= 0) {
            LOG.debug("Failed to fetch remote modification time from {}, using local timestamp {}", dst,
     ...
     */
    List<String> registerMultipleLocalResources(final Collection<Path> shipFiles, final String localResourcesDirectory,
            ...
            if(Utils.isRemotePath(shipFile.toString())) {
                if(fileSystem.isDirectory(shipFile)) {
                    ...
                    while(iterable.hasNext()) {
                        ...
                        relativePaths
                                .add(new Path(localResourcesDirectory, parentURI.relativize(current.toUri()).getPath()));
                    }
                    continue;
                }
            } else {
        ...
        for(int i = 0; i < localPaths.size(); i++) {
            ...
            if(!isFlinkDistJar(relativePath.getName())) {
                ...
                final YarnLocalResourceDescriptor resourceDescriptor = registerSingleLocalResource(key, localPath,
                        relativePath.getParent().toString(), resourceType, true, true);
    ...
    public YarnLocalResourceDescriptor uploadFlinkDist(
            final Path localJarPath) throws IOException, ClusterDeploymentException {
        if(flinkDist != null) {
            return flinkDist;
        } else if(!providedSharedLibs.isEmpty()) {
            throw new ClusterDeploymentException("The \"" + YarnConfigOptions.PROVIDED_LIB_DIRS
                    .key() + "\"" + " has to also include the lib/, plugin/ and flink-dist jar." + " In other case, it cannot be used.");
        }

        flinkDist = registerSingleLocalResource(localJarPath.getName(), localJarPath, "", LocalResourceType.FILE, true,
                false);
        return flinkDist;
    }
    ...
    static YarnApplicationFileUploader from(final FileSystem fileSystem, final Path homeDirectory,
            final List<Path> providedLibDirs, final ApplicationId applicationId,
    ...
    private Path copyToRemoteApplicationDir(final Path localSrcPath, final String relativeDstPath,
        ...
        checkNotNull(providedLibDirs).forEach(FunctionUtils.uncheckedConsumer(path -> {
            if(!fileSystem.exists(path) || !fileSystem.isDirectory(path)) {
                ...
                while(iterable.hasNext()) {
                    ...
                    final String name = path.getParent().toUri().relativize(locatedFileStatus.getPath().toUri())
                            .toString();

                    final FileStatus prevMapping = allFiles.put(name, locatedFileStatus);
                    if(prevMapping != null) {
                        throw new IOException("Two files with the same filename exist in the shared libs: " + prevMapping
                                .getPath() + " - " + locatedFileStatus.getPath() + ". Please deduplicate.");
                    }
                }

                if(LOG.isDebugEnabled()) {
                    LOG.debug("The following files were found in the shared lib dir: {}",
                            allFiles.values().stream().map(fileStatus -> fileStatus.getPath().toString())
