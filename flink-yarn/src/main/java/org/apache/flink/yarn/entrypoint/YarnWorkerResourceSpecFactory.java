...
public class YarnWorkerResourceSpecFactory extends WorkerResourceSpecFactory {
    ...
    static CPUResource getDefaultCpus(final Configuration configuration) {
        ...
        double cpuCoresDouble = TaskExecutorProcessUtils.getCpuCoresWithFallback(configuration, fallback).getValue()
                .doubleValue();
        @SuppressWarnings("NumericCastThatLosesPrecision") long cpuCoresLong = Math
                .max((long) Math.ceil(cpuCoresDouble), 1L);
        ...
        if(cpuCoresLong != cpuCoresDouble) {
            LOG.info(
                    ...
                    cpuCoresDouble, cpuCoresLong);
        }
        if(cpuCoresLong > Integer.MAX_VALUE) {
            throw new IllegalConfigurationException(
                    String.format("The amount of cpu cores %d cannot exceed Integer.MAX_VALUE: %d", cpuCoresLong,
                            Integer.MAX_VALUE));
        }
