...
abstract class OperatorStateInputFormat<OT> extends RichInputFormat<OT, OperatorStateInputSplit> {
    ...
    private OperatorStateInputSplit[] getOperatorStateInputSplits(int minNumSplits) {
        ...
        reDistributePartitionableStates(singletonMap(operatorState.getOperatorID(), operatorState), minNumSplits,
                OperatorSubtaskState::getManagedOperatorState, RoundRobinOperatorStateRepartitioner.INSTANCE,
                newManagedOperatorStates);

        return CollectionUtil.mapWithIndex(newManagedOperatorStates.values(),
    ...
    public void open(OperatorStateInputSplit split) throws IOException {
        ...
        final BackendRestorerProcedure<OperatorStateBackend, OperatorStateHandle> backendRestorer = new BackendRestorerProcedure<>(
                (handles) -> createOperatorStateBackend(getRuntimeContext(), handles, registry), registry,
    ...
    private static OperatorStateBackend createOperatorStateBackend(RuntimeContext runtimeContext,
            Collection<OperatorStateHandle> stateHandles, CloseableRegistry cancelStreamRegistry) {

        try {
            return new DefaultOperatorStateBackendBuilder(runtimeContext.getUserCodeClassLoader(),
                    runtimeContext.getExecutionConfig(), false, stateHandles, cancelStreamRegistry).build();
