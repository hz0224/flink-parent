...
public class AccumulatorHelper {
     ...
     */
    public static void mergeInto(Map<String, OptionalFailure<Accumulator<?, ?>>> target,
        ...
        for(Map.Entry<String, Accumulator<?, ?>> otherEntry : toMerge.entrySet()) {
            ...
            if(ownAccumulator == null) {
                ...
                target.put(otherEntry.getKey(), wrapUnchecked(otherEntry.getKey(),
    ...
    private static <V, R extends Serializable> Accumulator<V, R> mergeSingle(Accumulator<?, ?> target,
            Accumulator<?, ?> toMerge) {
        @SuppressWarnings("unchecked") Accumulator<V, R> typedTarget = (Accumulator<V, R>) target;

        @SuppressWarnings("unchecked") Accumulator<V, R> typedToMerge = (Accumulator<V, R>) toMerge;
    ...
    public static void compareAccumulatorTypes(Object name, Class<? extends Accumulator> first,
        ...
        if(first != second) {
            if(!first.getName().equals(second.getName())) {
                throw new UnsupportedOperationException(
                        "The accumulator object '" + name + "' was created with two different types: " + first
                                .getName() + " and " + second.getName());
            } else {
                // damn, name is the same, but different classloaders
                throw new UnsupportedOperationException(
                        "The accumulator object '" + name + "' was created with two different classes: " + first + " and " + second + " Both have the same type (" + first
                                .getName() + ") but different classloaders: " + first.getClassLoader() + " and " + second
                                .getClassLoader());
            }
        }
    }
    ...
    public static Map<String, Object> deserializeAndUnwrapAccumulators(
        ...
        Map<String, OptionalFailure<Object>> deserializedAccumulators = deserializeAccumulators(serializedAccumulators,
                loader);
