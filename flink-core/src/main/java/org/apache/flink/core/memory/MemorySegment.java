...
/**
 * // TODO_MA 注释： Flink 管理的堆内内存 和 堆外内存，都被抽象成一个个的 MemorySegment
 * // TODO_MA 注释： 默认 32kb
 * // TODO_MA 注释： 提供 put 方法用来读取和写入
 ...
 *
 * // TODO_MA 注释： 跨两个内存段运行的所有方法都在此类中实现，以透明地处理内存段类型的混合。
 * All methods that operate across two memory
 * segments are implemented in this class, to transparently handle the mixing of memory segment types.
 *
...
public abstract class MemorySegment {
    ...
    public final void copyTo(int offset, MemorySegment target, int targetOffset, int numBytes) {
        ...
        if((numBytes | offset | targetOffset) >= 0 && thisPointer <= this.addressLimit - numBytes && otherPointer <= target.addressLimit - numBytes) {
            ...
            throw new IndexOutOfBoundsException(
                    String.format("offset=%d, targetOffset=%d, numBytes=%d, address=%d, targetAddress=%d", offset,
                            targetOffset, numBytes, this.address, target.address));
        }
    }
    ...
    public final void swapBytes(byte[] tempBuffer, MemorySegment seg2, int offset1, int offset2, int len) {
        ...
        // index is in fact invalid
        throw new IndexOutOfBoundsException(
                String.format("offset1=%d, offset2=%d, len=%d, bufferSize=%d, address1=%d, address2=%d", offset1, offset2,
                        len, tempBuffer.length, this.address, seg2.address));
    }
